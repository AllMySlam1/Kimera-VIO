  ...
  protected:
    ...
    void initializeData()
    {
        ...
        ref_stereo_frame = std::make_shared<StereoFrame>(
            ...
            Frame(id_ref, timestamp_ref, cam_params_left_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_ref_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_ref, timestamp_ref, cam_params_right_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_ref_right,
                      tp.stereo_matching_params_.equalize_image_)));
        ...
        cur_stereo_frame = std::make_shared<StereoFrame>(
            ...
            Frame(id_cur, timestamp_cur, cam_params_left_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_cur_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_cur, timestamp_cur, cam_params_right_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_cur_right,
                      tp.stereo_matching_params_.equalize_image_)));
    ...
    void fillStereoFrame(std::shared_ptr<StereoFrame>& sf)
    {
        ...
        // left.y == right.y
        // (right_keypoints_status[i] == valid && right_frame_.keypoints_[i] !=
        // 0) OR (right_keypoints_status[i] != valid)
        ...
        if (sf->right_keypoints_rectified_.size() != num_keypoints) {
            ...
            for (int i = 0; i < num_keypoints; i++) {
                sf->right_keypoints_rectified_.at(i).first =
        ...
        if (sf->right_frame_.keypoints_.size() != num_keypoints) {
            ...
            for (int i = 0; i < num_keypoints; i++) {
                if (sf->right_keypoints_rectified_[i].first ==
                    KeypointStatus::VALID) {
    ...
    std::vector<gtsam::Point2> loadCorners(const std::string& filepath)
    {
        ...
        for (int i = 0; i < num_corners; i++) {
            double x, y;
            fin >> x >> y;
            // Convert points coordinates from MATLAB convention to c++
            // convention:
    ...
    convertCornersAcrossCameras(const std::vector<gtsam::Point2>& corners_in,
...
TEST_F(StereoVisionImuFrontendFixture, getSmartStereoMeasurements)
{
    ...
    VIO::StereoCamera::ConstPtr stereo_camera =
        std::make_shared<VIO::StereoCamera>(cam_params_left_,
                                            cam_params_right_);

    StereoVisionImuFrontend st(imu_params_, ImuBias(), FrontendParams(),
                               stereo_camera);
    ...
    for (int i = 0; i < num_right_missing; i++) {
        ...
        ref_stereo_frame->left_frame_.scores_.push_back(1.0);
        ref_stereo_frame->left_keypoints_rectified_.push_back(StatusKeypointCV(
            KeypointStatus::NO_RIGHT_RECT, cv::Point2f(uL, v)));
        ref_stereo_frame->right_keypoints_rectified_.push_back(StatusKeypointCV(
            KeypointStatus::NO_RIGHT_RECT, cv::Point2f(uL, v)));
    }
    ...
    for (auto s : ssm) {
        ...
        EXPECT_EQ(
            ref_stereo_frame->left_keypoints_rectified_[landmark_id].second.x,
            s.second.uL());
        EXPECT_EQ(
            ref_stereo_frame->left_keypoints_rectified_[landmark_id].second.y,
            s.second.v());
        if (ref_stereo_frame->right_keypoints_rectified_[landmark_id].first ==
            KeypointStatus::VALID) {
            EXPECT_EQ(ref_stereo_frame->right_keypoints_rectified_[landmark_id]
                          .second.x,
                      s.second.uR());
        }
        else {
...
TEST_F(StereoVisionImuFrontendFixture, DISABLED_processFirstFrame)
{
    ...
    std::string  synthetic_stereo_path(FLAGS_test_data_path +
                                      "/ForStereoTracker");
    ...
    std::string img_name_right =
        synthetic_stereo_path + "/img_distort_right.png";
    ...
    // Same intrinsics
    for (int i = 0; i < 4; i++) {
        EXPECT_EQ(cam_params_left.intrinsics_[i],
                  cam_params_right.intrinsics_[i]);
    }
    ...
    for (size_t i = 0u; i < num_corners; i++) {
        Vector3 v_expect = UndistorterRectifier::UndistortKeypointAndGetVersor(
    ...
    std::vector<gtsam::Point2> left_rect_corners = convertCornersAcrossCameras(
        left_undistort_corners, gtsam_left_cam_calib,
    ...
    std::vector<gtsam::Point2> right_rect_corners = convertCornersAcrossCameras(
        right_undistort_corners, gtsam_right_cam_calib,
