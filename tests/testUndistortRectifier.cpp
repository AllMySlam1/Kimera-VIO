...
class UndistortRectifierFixture : public ::testing::Test {
  public:
    UndistortRectifierFixture()
        : cam_params_left(), cam_params_right(), undistorter_rectifier()
    {
        ...
        undistorter_rectifier = VIO::make_unique<VIO::UndistorterRectifier>(
            stereo_camera->getP1(), cam_params_left, stereo_camera->getR1());
    }
  ...
  protected:
    ...
    // Generate keypoints regularly spaced along a grid of size num_rows x
    // num_rows.
...
TEST_F(UndistortRectifierFixture, undistortRectifyKeypoints)
{
    ...
    // generate keypoints regularly spaced along a grid of size numRows x
    // numRows
...
TEST_F(UndistortRectifierFixture, distortUnrectifyKeypoints)
{
    ...
    VIO::CameraParams::createGtsamCalibration(
        cam_params_left.distortion_coeff_mat_, cam_params_left.intrinsics_,
        &gtsam_calib);
    ...
    for (int r = 0; r < numRows; r++) {
        for (int c = 0; c < numCols; c++) {
            ...
            keypoints_rectified.push_back(
                std::make_pair(st, cv::Point2f(x, y)));
        }
    }
    ...
    for (int i = 0; i < keypoints_unrectified.size();
         i++) {  // for each keypoint
        if (keypoints_rectified[i].first !=
            VIO::KeypointStatus::VALID) {  // if status is not valid, keypoint
                                           // is
                                           // conventionally
                                           // set to (0,0)
        ...
        VIO::Point2 pt_in = VIO::Point2(
            ...
            xn.at<double>(1, 0) /
                xn.at<double>(
                    2,
                    0));  // convert back to pixel (non-homogeneous coordinates)
...
TEST_F(UndistortRectifierFixture, DISABLED_undistortFisheye)
{
    ...
    cv::Mat left_fisheye_image_dist =
        VIO::UtilsOpenCV::ReadAndConvertToGrayScale(
    ...
    if (cam_params_left_fisheye.distortion_model_ ==
        ...
        fisheye_undistorter_left.undistortRectifyImage(
            left_fisheye_image_dist, &left_fisheye_image_undist);
    }
    else {
...
TEST_F(UndistortRectifierFixture, DISABLED_undistortFisheyeStereoFrame)
{
    ...
    cv::Mat left_fisheye_image_dist =
        VIO::UtilsOpenCV::ReadAndConvertToGrayScale(
            stereo_FLAGS_test_data_path + "left_fisheye_img_0.png", false);
    cv::Mat right_fisheye_image_dist =
        VIO::UtilsOpenCV::ReadAndConvertToGrayScale(
    ...
    undistorter_left->checkUndistortedRectifiedLeftKeypoints(
        left_frame_fish.img_, left_keypoints_rectified,
    ...
    fisheye_matcher->getRightKeypointsRectified(
        ...
        status_left_keypoints_rectified,
        left_undistRectCameraMatrix_fisheye.fx(), fisheye_camera->getBaseline(),
