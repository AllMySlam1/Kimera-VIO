...
TEST(testCameraParams, parseYAML)
{
    ...
    gtsam::Point3 T_expected(-0.0216401454975, -0.064676986768,
                             0.00981073058949);
...
TEST(testCameraParams, convertDistortionVectorToMatrix)
{
    ...
    for (size_t i = 0u; i < distortion_coeffs.size(); i++) {
        EXPECT_EQ(distortion_coeffs_mat.at<double>(0, i),
                  distortion_coeffs.at(i));
    }
    ...
    EXPECT_EQ(distortion_coeffs_mat.rows, 1u);
    for (size_t i = 0u; i < distortion_coeffs.size(); i++) {
        EXPECT_EQ(distortion_coeffs_mat.at<double>(0u, i),
                  distortion_coeffs.at(i));
    }
