...
class TestStereoDataProviderModule : public ::testing::Test {
  public:
    ...
    {
        // Set google flags to assume image is already rectified--makes dummy
        // params easier FLAGS_images_rectified = true;

        // Create the output queue
        output_queue_ =
            VIO::make_unique<VIO::VisionImuFrontendModule::InputQueue>(
                "output");
        ...
        bool                      parallel = false;
        stereo_data_provider_module_ =
            VIO::make_unique<VIO::StereoDataProviderModule>(
                dummy_queue_.release(), "Stereo Data Provider", parallel,
                dummy_params);
  ...
  protected:
    ...
    VIO::Frame::UniquePtr makeDummyFrame(const VIO::FrameId&   frame_id,
                                         const VIO::Timestamp& timestamp)
    {
        return VIO::make_unique<VIO::Frame>(frame_id, timestamp,
                                            makeDumyCameraParams(), cv::Mat());
    }
...
TEST_F(TestStereoDataProviderModule, basicSequentialCase)
{
    ...
    EXPECT_EQ(static_cast<VIO::FrameId>(2), result->getStereoFrame().id_);
...
TEST_F(TestStereoDataProviderModule, manyImuTest)
{
    ...
    for (size_t i = 0; i < num_valid_frames; i++) {
        ...
        CHECK(output_queue_->pop(result_base));
        VIO::StereoImuSyncPacket::UniquePtr result =
            VIO::safeCast<VIO::FrontendInputPacketBase,
                          VIO::StereoImuSyncPacket>(std::move(result_base));
        CHECK(result);
...
TEST_F(TestStereoDataProviderModule, imageBeforeImuTest)
{
    ...
    CHECK(result);
    EXPECT_EQ(current_time - 1, result->timestamp_);
    EXPECT_EQ(static_cast<VIO::FrameId>(3), result->getStereoFrame().id_);
    // +1 because it interpolates to the time frame
    EXPECT_EQ(kImuTestBundleSize + 1, result->getImuStamps().size());
}
