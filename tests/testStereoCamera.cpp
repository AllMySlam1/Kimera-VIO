...
class StereoCameraFixture : public ::testing::Test {
  public:
    StereoCameraFixture()
        : vio_params_(FLAGS_test_data_path + "/EurocParams"),
          stereo_camera_(nullptr), stereo_matcher_(nullptr),
          left_frame_queue_("left_frame_queue"),
          right_frame_queue_("right_frame_queue"), window_()
    {
        // Parse data
        parseEuroc();
  ...
  protected:
    ...
    void parseEuroc()
    {
        ...
        // Register Callbacks
        euroc_data_provider_->registerLeftFrameCallback(
            std::bind(&StereoCameraFixture::fillLeftFrameQueue, this,
                      std::placeholders::_1));
        euroc_data_provider_->registerRightFrameCallback(
            std::bind(&StereoCameraFixture::fillRightFrameQueue, this,
                      std::placeholders::_1));
...
TEST_F(StereoCameraFixture, backProjectDisparityTo3D)
{
    ...
    stereo_matcher_->denseStereoReconstruction(
        stereo_frame.getLeftImgRectified(), stereo_frame.getRightImgRectified(),
        &disp_img);
    ...
    for (size_t v = 0u; v < depth_map.rows; ++v) {
        for (size_t u = 0u; u < depth_map.cols; ++u) {
            ...
            if (isValidPoint(xyz) && xyz.z <= kMaxZ) {
                ...
                // transform from left cam frame of reference to body because
                // the stereo camera projection function expects landmarks in
                ...
                lmk_cv.y = pt_body.y();
                lmk_cv.z = pt_body.z();
                // Ok, now for real, project to camera and get pixel
                // coordinates.
                KeypointCV kp_left;
                KeypointCV kp_right;
...
{
    CHECK(stereo_camera_);
    // Rectification is already called in initializeData(), to avoid calling it
    // in every function
    ...
    EXPECT_TRUE(stereo_camera_->getBodyPoseLeftCamRect().equals(
        B_Pose_camLrect_expect));
