...
class OnlineAlignmentFixture : public ::testing::Test {
  public:
    OnlineAlignmentFixture()
        : dataset_(nullptr), estimated_poses_(), pims_(), ahrs_pim_(),
          delta_t_poses_(), imu_params_(), imu_bias_(), bias_acc_(),
          bias_gyr_(), init_navstate_()
    {
        // Set IMU params
  ...
  protected:
                                       ...
                                       const std::string& dataset_path)
    {
        ...
        dataset_ = VIO::make_unique<EurocDataProvider>(dataset_path, initial_k,
                                                       final_k, vio_params);
        ...
        while (it != dataset_->gt_data_.map_to_gt_.end()) {
            ...
            delta_t_poses_.push_back(UtilsNumerical::NsecToSec(
...
TEST_F(OnlineAlignmentFixture, DISABLED_GyroscopeBiasEstimation)
{
    ...
    OnlineGravityAlignment initial_alignment(estimated_poses_, delta_t_poses_,
                                             pims_, n_gravity);
...
TEST_F(OnlineAlignmentFixture, CreateTangentBasis)
{
    for (int i = 0; i < 20; i++) {
        ...
        gtsam::Vector3 basis_vec_y(tangent_basis(0, 0), tangent_basis(1, 0),
                                   tangent_basis(2, 0));
        gtsam::Vector3 basis_vec_z(tangent_basis(0, 1), tangent_basis(1, 1),
                                   tangent_basis(2, 1));
...
TEST_F(OnlineAlignmentFixture, DISABLED_GravityAlignmentRealData)
{
    for (int i = 0; i < 30; i++) {
        ...
        gtsam::Vector3 real_body_grav(
            init_navstate_.pose_.rotation().transpose() * n_gravity);
        gtsam::Pose3 real_init_pose(init_navstate_.pose_.rotation(),
                                    gtsam::Vector3::Zero());
