...
class StereoMatcherFixture : public ::testing::Test {
  public:
    StereoMatcherFixture() : cam_params_left(), cam_params_right()
    {
        ...
        stereo_camera  = std::make_shared<VIO::StereoCamera>(cam_params_left,
                                                            cam_params_right);
        ...
        for (size_t i = 0; i < sfnew->left_frame_.keypoints_.size(); i++) {
            ...
            sfnew->left_frame_.versors_.push_back(
                UndistorterRectifier::UndistortKeypointAndGetVersor(
                    ...
                    sfnew->left_frame_.cam_param_, stereo_camera->getR1()));
            ++landmark_count_;
        }
...
TEST_F(StereoMatcherFixture, sparseStereoReconstruction)
{
    ...
    CameraParams::createGtsamCalibration(
        ...
        sfnew->left_frame_.cam_param_.intrinsics_, &gtsam_calib);
    ...
    gtsam::Rot3 expected_camL_R_camLrect =
        sfnew->left_frame_.cam_param_.body_Pose_cam_.rotation().between(
            stereo_camera->getBodyPoseLeftCamRect()
                .rotation());  // camL_R_camLrect
    gtsam::Rot3 actual_camL_R_camLrect =
        UtilsOpenCV::cvMatToGtsamRot3(stereo_camera->getR1()).inverse();
    EXPECT_TRUE(gtsam::assert_equal(expected_camL_R_camLrect,
                                    actual_camL_R_camLrect, 1e-4));
    ...
    for (size_t i = 0; i < sfnew->keypoints_3d_.size(); i++) {
        if (sfnew->right_keypoints_rectified_.at(i).first ==
            ...
            EXPECT_TRUE(
                gtsam::assert_equal(versorExpected, versorActual, 1e-1));
        }
        else {
    ...
    for (size_t i = 0; i < sfnew->keypoints_3d_.size(); i++) {
        if (sfnew->right_keypoints_rectified_.at(i).first ==
            KeypointStatus::VALID) {
            // TEST: uncalibrateDistUnrect(versor) = original distorted
            // unrectified point (CHECK DIST UNRECT CALIBRATION WORKS)
            KeypointCV kp_i_distUnrect = sfnew->left_frame_.keypoints_.at(
                i);  // original distorted unrectified
            gtsam::Vector3 versor_i =
                sfnew->left_frame_.versors_.at(i);  // in rectified frame
            versor_i =
                actual_camL_R_camLrect.matrix() *
                versor_i;  // compensate for rotation due to rectification
            versor_i = versor_i /
                       versor_i(2);  // set last element to 1, instead of norm 1
            gtsam::Point2 kp_i_distUnrect_gtsam = gtsam_calib.uncalibrate(
                gtsam::Point2(versor_i(0), versor_i(1)));
            EXPECT_TRUE(gtsam::assert_equal(
                gtsam::Point2(kp_i_distUnrect.x, kp_i_distUnrect.y),
                kp_i_distUnrect_gtsam, 1));

            // TEST: uncalibrateUndistRect(versor) = original distorted
            // unrectified point (CHECK UNDIST RECT CALIBRATION WORKS)
            KeypointCV kp_i_undistRect =
                sfnew->left_keypoints_rectified_.at(i).second;
            Cal3_S2 KundistRect =
                stereo_camera->getStereoCalib()->calibration();
            versor_i = sfnew->left_frame_.versors_.at(i);
            versor_i = versor_i /
                       versor_i(2);  // set last element to 1, instead of norm 1
            gtsam::Point2 kp_i_undistRect_gtsam = KundistRect.uncalibrate(
                gtsam::Point2(versor_i(0), versor_i(1)));
            EXPECT_TRUE(gtsam::assert_equal(
            ...
            gtsam::PinholeCamera<Cal3_S2> leftCam_undistRect(
                gtsam::Pose3::identity(), KundistRect);
            gtsam::Point2 p2_undistRect = leftCam_undistRect.project(point3d);
            EXPECT_TRUE(gtsam::assert_equal(
                gtsam::Point2(kp_i_undistRect.x, kp_i_undistRect.y),
                p2_undistRect, 1));

            // TEST: projecting 3d point to left camera (distorted and
            // unrectified) = original distorted unrectified point (CHECK
            // BACKPROJECTION WORKS)
            gtsam::Point3 point3d_unrect = actual_camL_R_camLrect.rotate(
                point3d);  // compensate for the rotation induced by
                           // rectification
            gtsam::Cal3DS2                KdistUnrect = gtsam_calib;
            gtsam::PinholeCamera<Cal3DS2> leftCam_distUnrect(
                gtsam::Pose3::identity(), KdistUnrect);
            gtsam::Point2 p2_distUnrect =
                leftCam_distUnrect.project(point3d_unrect);
            EXPECT_TRUE(gtsam::assert_equal(
                Point2(kp_i_distUnrect.x, kp_i_distUnrect.y), p2_distUnrect,
                1));

            // TEST: projecting 3d point to stereo camera
            // reproject to camera and check that matches corresponding
            // rectified pixels Note: camera pose is the identity (instead of
            // sfnew->getBPoseCamLRect()) since the 3D point is in the left
            // camera frame
            ...
            EXPECT_NEAR(sp2.uL(),
                        sfnew->left_keypoints_rectified_.at(i).second.x, 1);
            EXPECT_NEAR(sp2.v(),
                        sfnew->left_keypoints_rectified_.at(i).second.y, 1);
            EXPECT_NEAR(sp2.uR(),
                        sfnew->right_keypoints_rectified_.at(i).second.x, 1);
            EXPECT_NEAR(sp2.v(),
...
TEST_F(StereoMatcherFixture, getRightKeypointsRectified)
{
    ...
    // disparity = left_px.x - right_px.x, hence we check: right_px.x <
    // left_px.x
    ...
    for (int i = 0; i < num_offset_test; i++) {
        ...
        for (int t = 0; t < 2; t++) {
            ...
            // matching error t = 1: test on rounded keypoints, expect very
            ...
            stereo_matcher->getRightKeypointsRectified(
                left_img, right_img, left_keypoints_rectified, 458.654,
                stereo_camera->getBaseline(), &right_keypoints_rectified);
            ...
            for (size_t i = 0; i < left_keypoints_rectified.size();
                ...
                if (y_left <= (stripe_rows - 1) / 2 ||
                    ...
                    countValid += 1;
                         ...
                         x_exp >=
                             left_img.cols) {  // if true right match is invalid
                    ...
                    // Corner case: The matching may be affected by the image
                    // border
                    EXPECT_TRUE((abs(x_exp - x_actual) < 0.5) ||
                                (abs(x_exp - x_actual) >= 0.5));
                    if (!((abs(x_exp - x_actual) < 0.5) ||
                          (abs(x_exp - x_actual) >= 0.5))) {
                        cout << "IF WE GET TO THIS POINT, THE TEST FAILED - we "
                                "visualize "
                                "wrong matches:"
                             << endl;
                        ...
                        cv::Mat canvas = UtilsOpenCV::DrawCornersMatches(
                            left_img, left_corners, right_img, right_corners,
                            matches);
    ...
    for (auto depth2baseline_ratio : depth2baseline_ratio_set) {
        for (auto x2depth : x2depth_set) {
            for (auto y2depth : y2depth_set) {
                double depth =
                    depth2baseline_ratio * stereo_camera->getBaseline();
                ...
                cv::Mat pt_mat_right =
                    stereo_camera->getP2() * (cv::Mat_<double>(4, 1) << x_loc,
                                              y_loc, depth,
                                              1);  // project to right
                pt_mat_left = pt_mat_left / pt_mat_left.at<double>(
                                                2, 0);  // express as pixels
                pt_mat_right = pt_mat_right / pt_mat_right.at<double>(
                                                  2, 0);  // express as pixels
                cv::Point2f pt_left(
                    pt_mat_left.at<double>(0, 0),
