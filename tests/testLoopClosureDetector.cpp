...
class LCDFixture : public ::testing::Test {
  ...
  public:
    LCDFixture()
        : lcd_test_data_path_(FLAGS_test_data_path +
          ...
          frontend_params_(), stereo_camera_(nullptr), stereo_matcher_(nullptr),
          lcd_detector_(nullptr), match1_T_query1_(), match2_T_query2_(),
          match1_stereo_frame_(nullptr), match2_stereo_frame_(nullptr),
          query1_stereo_frame_(nullptr), query2_stereo_frame_(nullptr),
          id_match1_(0), id_query1_(1), id_match2_(2), id_query2_(3),
          timestamp_match1_(1000), timestamp_query1_(2000),
          timestamp_match2_(3000), timestamp_query2_(4000)
    {
        // First set value of vocabulary path for LoopClosureDetector
        FLAGS_vocabulary_path = "../vocabulary/ORBvoc.yml";
        frontend_params_.parseYAML(lcd_test_data_path_ +
                                   "/FrontendParams.yaml");
        ...
        lcd_detector_ = VIO::make_unique<LoopClosureDetector>(
            params, stereo_camera_, frontend_params_.stereo_matching_params_,
            false);

        // Euroc V1_01_easy ts: 1403715386762142976
        world_T_match1_ =
            gtsam::Pose3(gtsam::Rot3(gtsam::Quaternion(0.338337, 0.608466,
                                                       -0.535476, 0.478082)),
                         gtsam::Point3(1.573832, 2.023348, 1.738755));

        // Euroc V1_01_easy ts: 1403715288312143104
        world_T_query1_ =
            gtsam::Pose3(gtsam::Rot3(gtsam::Quaternion(0.478634, 0.415595,
                                                       -0.700197, 0.328505)),
                         gtsam::Point3(1.872115, 1.786064, 1.586159));

        // Euroc V1_01_easy ts: 1403715400762142976
        world_T_match2_ =
            gtsam::Pose3(gtsam::Rot3(gtsam::Quaternion(0.3394, -0.672895,
                                                       -0.492724, -0.435018)),
                         gtsam::Point3(-0.662997, -0.495046, 1.347300));

        // Euroc V1_01_easy ts: 1403715400262142976
        world_T_query2_ =
            gtsam::Pose3(gtsam::Rot3(gtsam::Quaternion(0.39266, -0.590667,
                                                       -0.58023, -0.400326)),
  ...
  protected:
    void initializeData()
    {
        std::string img_name_match1_left =
            lcd_test_data_path_ + "/left_img_0.png";
        std::string img_name_match1_right =
            lcd_test_data_path_ + "/right_img_0.png";

        std::string img_name_query1_left =
            lcd_test_data_path_ + "/left_img_1.png";
        std::string img_name_query1_right =
            lcd_test_data_path_ + "/right_img_1.png";

        std::string img_name_match2_left =
            lcd_test_data_path_ + "/left_img_2.png";
        std::string img_name_match2_right =
            lcd_test_data_path_ + "/right_img_2.png";

        std::string img_name_query2_left =
            lcd_test_data_path_ + "/left_img_3.png";
        ...
        match1_stereo_frame_ = VIO::make_unique<StereoFrame>(
            id_match1_, timestamp_match1_,
            Frame(id_match1_, timestamp_match1_, cam_params_left_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_match1_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_match1_, timestamp_match1_, cam_params_right_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_match1_right,
                      tp.stereo_matching_params_.equalize_image_)));

        feature_detector.featureDetection(&match1_stereo_frame_->left_frame_);
        ...
        query1_stereo_frame_ = VIO::make_unique<StereoFrame>(
            id_query1_, timestamp_query1_,
            Frame(id_query1_, timestamp_query1_, cam_params_left_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_query1_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_query1_, timestamp_query1_, cam_params_right_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_query1_right,
                      tp.stereo_matching_params_.equalize_image_)));

        feature_detector.featureDetection(&query1_stereo_frame_->left_frame_);
        ...
        match2_stereo_frame_ = VIO::make_unique<StereoFrame>(
            id_match2_, timestamp_match2_,
            Frame(id_match2_, timestamp_match2_, cam_params_left_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_match2_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_match2_, timestamp_match2_, cam_params_right_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_match2_right,
                      tp.stereo_matching_params_.equalize_image_)));

        feature_detector.featureDetection(&match2_stereo_frame_->left_frame_);
        ...
        query2_stereo_frame_ = VIO::make_unique<StereoFrame>(
            id_query2_, timestamp_query2_,
            Frame(id_query2_, timestamp_query2_, cam_params_left_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_query2_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_query2_, timestamp_query2_, cam_params_right_,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_query2_right,
                      tp.stereo_matching_params_.equalize_image_)));

        feature_detector.featureDetection(&query2_stereo_frame_->left_frame_);
...
TEST_F(LCDFixture, geometricVerificationCheck)
{
    ...
    lcd_detector_->geometricVerificationCheck(1, 0, &camMatch1_T_camQuery1_mono,
                                              &i_query, &i_match);
...
TEST_F(LCDFixture, recoverPoseArun)
{
    ...
    lcd_detector_->recoverPose(1, 0, empty_pose,
                               &bodyMatch1_T_bodyQuery1_stereo, &i_query1,
                               &i_match1);
    ...
    lcd_detector_->recoverPose(3, 2, empty_pose,
                               &bodyMatch2_T_bodyQuery2_stereo, &i_query2,
                               &i_match2);
...
TEST_F(LCDFixture, recoverPoseGivenRot)
{
    ...
    gtsam::Pose3 bodyMatch1_T_bodyQuery1_stereo;
    lcd_detector_->recoverPose(1, 0, camMatch1_T_camQuery1_mono_gt,
                               &bodyMatch1_T_bodyQuery1_stereo, &i_query1,
                               &i_match1);
    ...
    gtsam::Pose3 bodyMatch2_T_bodyQuery2_stereo;
    lcd_detector_->recoverPose(3, 2, camMatch2_T_camQuery2_gt,
                               &bodyMatch2_T_bodyQuery2_stereo, &i_query2,
                               &i_match2);
...
TEST_F(LCDFixture, addOdometryFactorAndOptimize)
{
    ...
    lcd_detector_->initializePGO(
        OdometryFactor(0, gtsam::Pose3::identity(),
                       gtsam::noiseModel::Isotropic::Variance(6, 0.1)));

    OdometryFactor odom_factor(1, world_T_match1_,
                               gtsam::noiseModel::Isotropic::Variance(6, 0.1));
...
TEST_F(LCDFixture, spinOnce)
{
    ...
    StereoFrontendOutput::Ptr stereo_frontend_output =
        std::make_shared<StereoFrontendOutput>(
            match1_stereo_frame_->isKeyframe(), StatusStereoMeasurementsPtr(),
            TrackingStatus(), gtsam::Pose3::identity(),
            gtsam::Pose3::identity(), gtsam::Pose3::identity(),
            *match1_stereo_frame_, ImuFrontend::PimPtr(), ImuAccGyrS(),
            cv::Mat(), DebugTrackerInfo());
    ...
    LcdOutput::Ptr output_0 = lcd_detector_->spinOnce(LcdInput(
        timestamp_match1_, frontend_output_match1, FrameId(0), gtsam::Pose3()));

    CHECK(match2_stereo_frame_);
    stereo_frontend_output = std::make_shared<StereoFrontendOutput>(
        match2_stereo_frame_->isKeyframe(), StatusStereoMeasurementsPtr(),
        TrackingStatus(), gtsam::Pose3::identity(), gtsam::Pose3::identity(),
        gtsam::Pose3::identity(), *match2_stereo_frame_, ImuFrontend::PimPtr(),
        ImuAccGyrS(), cv::Mat(), DebugTrackerInfo());
    FrontendOutputPacketBase::Ptr frontend_output_match2 =
        VIO::safeCast<StereoFrontendOutput, FrontendOutputPacketBase>(
            stereo_frontend_output);
    LcdOutput::Ptr output_1 = lcd_detector_->spinOnce(LcdInput(
        timestamp_match2_, frontend_output_match2, FrameId(1), gtsam::Pose3()));

    CHECK(query1_stereo_frame_);
    stereo_frontend_output = std::make_shared<StereoFrontendOutput>(
        query1_stereo_frame_->isKeyframe(), StatusStereoMeasurementsPtr(),
        TrackingStatus(), gtsam::Pose3::identity(), gtsam::Pose3::identity(),
        gtsam::Pose3::identity(), *query1_stereo_frame_, ImuFrontend::PimPtr(),
        ImuAccGyrS(), cv::Mat(), DebugTrackerInfo());
    FrontendOutputPacketBase::Ptr frontend_output_query1 =
        VIO::safeCast<StereoFrontendOutput, FrontendOutputPacketBase>(
            stereo_frontend_output);
    LcdOutput::Ptr output_2 = lcd_detector_->spinOnce(LcdInput(
        timestamp_query1_, frontend_output_query1, FrameId(2), gtsam::Pose3()));
