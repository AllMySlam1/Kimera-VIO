  ...
  protected:
    ...
    void InitializeData()
    {
        ...
        ref_stereo_frame = std::make_shared<StereoFrame>(
            ...
            Frame(id_ref, timestamp_ref, cam_params_left,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_ref_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_ref, timestamp_ref, cam_params_right,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_ref_right,
                      tp.stereo_matching_params_.equalize_image_)));
        ...
        cur_stereo_frame = std::make_shared<StereoFrame>(
            ...
            Frame(id_cur, timestamp_cur, cam_params_left,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_cur_left,
                      tp.stereo_matching_params_.equalize_image_)),
            Frame(id_cur, timestamp_cur, cam_params_right,
                  UtilsOpenCV::ReadAndConvertToGrayScale(
                      img_name_cur_right,
                      tp.stereo_matching_params_.equalize_image_)));
                                 ...
                                 const int      inlierNum)
    {
        ...
        for (int i = 0; i < inlierNum; i++) {
            ...
            KeypointCV pt_ref(rand() % f_ref->img_.cols,
                              rand() % f_ref->img_.rows);
            // Calibrate the point
            Vector3 versor_ref =
            ...
            Vector3 versor_plane =
                IntersectVersorPlane(versor_ref, PlaneN, PlaneD);
            ...
            CameraParams::createGtsamCalibration(
                ...
                f_ref->cam_param_.intrinsics_, &gtsam_calib);
            Point2     pt_cur_gtsam = gtsam_calib.uncalibrate(Point2(
                versor_cur[0] / versor_cur[2], versor_cur[1] / versor_cur[2]));
            KeypointCV pt_cur(pt_cur_gtsam.x(), pt_cur_gtsam.y());

            AddKeypointsVersorsToFrames(f_ref, f_cur, pt_ref, pt_cur,
                                        versor_ref, versor_cur);
        }
    }
                            ...
                            int    outlierNum)
    {
        for (int i = 0; i < outlierNum; i++) {
            while (true) {
                ...
                KeypointCV pt_ref(rand() % f_ref->img_.cols,
                                  rand() % f_ref->img_.rows);
                KeypointCV pt_cur(rand() % f_cur->img_.cols,
                                  rand() % f_cur->img_.rows);
                ...
                Vector3 versor_proj =
                    camRef_pose_camCur.inverse().rotation() *
                        (versor_ref * depth) +
                    camRef_pose_camCur.inverse().translation();
                                  ...
                                  Vector3&     v_cur)
    {
        ...
        StereoPoint2 sp2 =
            stereoCam.project(camLrect_R_camL.rotate(Point3(v_ref)));
        sf_ref->left_keypoints_rectified_.push_back(StatusKeypointCV(
            KeypointStatus::VALID, KeypointCV(sp2.uL(), sp2.v())));
        sf_ref->right_keypoints_rectified_.push_back(StatusKeypointCV(
            KeypointStatus::VALID, KeypointCV(sp2.uR(), sp2.v())));

        // create cur stereo camera
        VIO::StereoCamera cur_stereo_camera(sf_cur->left_frame_.cam_param_,
                                            sf_cur->right_frame_.cam_param_);
        camLrect_R_camL =
            UtilsOpenCV::cvMatToGtsamRot3(cur_stereo_camera.getR1());
        stereoCam =
            gtsam::StereoCamera(Pose3(), cur_stereo_camera.getStereoCalib());

        sp2 = stereoCam.project(camLrect_R_camL.rotate(Point3(v_cur)));
        sf_cur->left_keypoints_rectified_.push_back(StatusKeypointCV(
            KeypointStatus::VALID, KeypointCV(sp2.uL(), sp2.v())));
        sf_cur->right_keypoints_rectified_.push_back(StatusKeypointCV(
            KeypointStatus::VALID, KeypointCV(sp2.uR(), sp2.v())));
    ...
    void
    AddNonPlanarInliersToStereoFrame(StereoFrame* sf_ref,
                                     StereoFrame* sf_cur,
        ...
        for (int i = 0; i < outlierNum; i++) {
            while (true) {
                ...
                Vector3 versor_proj =
                    ...
                    camRef_pose_camCur.inverse().translation();
                double cos_angle = versor_proj.dot(versor_cur) /
                                   versor_proj.norm() / versor_cur.norm();
                const double cos_angle_tol = 0.9;
                if (cos_angle > cos_angle_tol) {
                    // randomized "inliers"
                    continue;
                    ...
                    // true outliers
                    AddVersorsToStereoFrames(sf_ref, sf_cur, versor_ref,
                                             versor_cur);
                    break;
                }
            }
    ...
    pair<Vector3, Matrix3>
    monteCarloSampleCovariance(const gtsam::StereoCamera stereoCam,
                               ...
                               const Matrix3             stereoPtCov)
    {
        ...
        for (int i = 0; i < nrRuns; i++) {  // monte carlo runs
            ...
            normal_distribution<double> pixelNoise(
                0.0, stereoPtCov(0, 0));  // 0 mean and suitable std
    ...
    for (auto test_conf : test_configurations) {
        ...
        for (int t = 0; t < repeat_times; t++) {
            ...
            if (is_planar) {
                ...
                AddPlanarInliersToFrame(ref_frame.get(), cur_frame.get(),
                                        camRef_pose_camCur, PlaneN, PlaneD,
                                        inlier_num);
            }
            else {
                vector<double> depth_range;
                depth_range.push_back(camRef_pose_camCur.translation().norm());
                depth_range.push_back(10 *
                                      camRef_pose_camCur.translation().norm());
                AddNonPlanarInliersToFrame(ref_frame.get(), cur_frame.get(),
                                           camRef_pose_camCur, depth_range,
                                           inlier_num);
            }

            // add outliers
            AddOutliersToFrame(ref_frame.get(), cur_frame.get(),
                               camRef_pose_camCur, outlier_num);
            // add noise
            if (noise_sigma != 0) {
...
TEST_F(TestTracker, geometricOutlierRejectionStereo)
{
    ...
    for (size_t testId = 0; testId < test_configurations.size(); testId++) {
        ...
        for (int t = 0; t < repeat_times; t++) {
            ...
            vector<double> depth_range;
            depth_range.push_back(
                camLeftRef_pose_camLeftCur.translation().norm() * 10);
            depth_range.push_back(
            ...
            if (is_planar) {
                ...
                const double PlaneD =
                    camLeftRef_pose_camLeftCur.translation().norm() * 20;
                AddPlanarInliersToStereoFrame(
                    ref_stereo_frame.get(), cur_stereo_frame.get(),
                    camLeftRef_pose_camLeftCur, PlaneN, PlaneD, inlier_num);
            }
            else {
                AddNonPlanarInliersToStereoFrame(
                    ref_stereo_frame.get(), cur_stereo_frame.get(),
                    camLeftRef_pose_camLeftCur, depth_range, inlier_num);
            }

            // add outliers
            AddOutliersToStereoFrame(
                ref_stereo_frame.get(), cur_stereo_frame.get(),
            ...
            for (int i = 0; i < inlier_num; i++) {
                ...
                EXPECT_GT(
                    (ref_stereo_frame->keypoints_3d_.at(i) - Vector3::Zero())
                        .norm(),
                    tol);
                ...
                EXPECT_NE(ref_stereo_frame->keypoints_depth_.at(i), 0.0);
                EXPECT_GT(
                    (cur_stereo_frame->keypoints_3d_.at(i) - Vector3::Zero())
                        .norm(),
                    tol);
            }
            ...
            for (int i = 0; i < inlier_num; i++) {  // compare only inliers
                Vector3 right_pt_3d_exp =
                    estimated_pose.inverse().rotation() *
                        ref_stereo_frame->keypoints_3d_.at(i) +
                    estimated_pose.inverse().translation();
                double diff =
                    (right_pt_3d_exp - cur_stereo_frame->keypoints_3d_.at(i))
                        .norm();
...
TEST_F(TestTracker, findOutliers)
{
    // Normal case:
    {
        ...
        for (auto i : outliers_actual) {
            EXPECT_NE(
                find(outliers_expected.begin(), outliers_expected.end(), i),
...
TEST_F(TestTracker, FindMatchingStereoKeypoints)
{
    ...
    for (auto match_ref_cur : matches_ref_cur) {
        int l_ref =
            ref_stereo_frame->left_frame_.landmarks_[match_ref_cur.first];
        int l_cur =
...
TEST_F(TestTracker, FeatureTrackingRotationalOpticalFlowPredictionWithLargeRot)
{
}
