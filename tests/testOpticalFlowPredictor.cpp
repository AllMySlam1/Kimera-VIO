...
class OpticalFlowPredictorFixture : public ::testing::Test {
  public:
    ...
    OpticalFlowPredictorFixture()
        : lmks_(), simulated_calib_(), euroc_calib_(), cam_1_kpts_(),
          cam_2_kpts_(), cam_1_pose_(), cam_2_pose_(), cam_1_(), cam_2_(),
          camera_params_(VioParams(FLAGS_test_data_path + "/EurocParams")
                             .camera_params_.at(0)),
          optical_flow_predictor_(nullptr), window_(nullptr)
    {
        ...
        // but you need to account for distortion, since the projectSafe
        // function of GTSAM also uncalibrates the pixels using the distortion
        // params.
        ...
        simulated_calib_ =
            gtsam::Cal3DS2(fx, fx, 0.0, camera_params_.image_size_.width / 2u,
                           camera_params_.image_size_.height / 2u, 0.0, 0.0);
  ...
  protected:
                        ...
                        const KeypointsCV& predicted_kpts)
    {
        if (FLAGS_display) {
            ...
            cv::Matx33d K =
                UtilsOpenCV::gtsamMatrix3ToCvMat(simulated_calib_.K());
            cv::Mat cam_1_img = cv::Mat(camera_params_.image_size_, CV_8UC4,
                                        cv::Scalar(255u, 255u, 255u, 40u));
            cv::Mat cam_2_img = cv::Mat(camera_params_.image_size_, CV_8UC4,
                                        cv::Scalar(255u, 0u, 0u, 125u));

            // Visualize world coords.
            window_->showWidget("World Coordinates",
                                cv::viz::WCoordinateSystem(0.5));
            ...
            for (size_t i = 0u; i < lmks_.size(); i++) {
                cv::Point3f lmk_cv =
                    UtilsOpenCV::gtsamVector3ToCvPoint3(lmks_[i]);
                visualizeRay(lmk_cv, "lmk-cam1" + std::to_string(i),
                             cam_1_position);
                visualizeRay(lmk_cv, "lmk-cam2" + std::to_string(i),
                             cam_2_position, 0.2, cv::viz::Color::red());
            ...
            drawPixelsOnImg(cam_2_kpts_, cam_2_img, cv::viz::Color::green(), 3u,
                            125u);

            // Color image 2 with pixel prediction if no prediction is done
            // Expected result if using NoPredictionOpticalFlow
            drawPixelsOnImg(cam_1_kpts_, cam_2_img, cv::viz::Color::brown(), 6u,
                            125u);

            // Show the estimated kpt positions in red and smaller
            drawPixelsOnImg(predicted_kpts, cam_2_img, cv::viz::Color::red(),
                            1u, 125u);
            ...
            LOG(WARNING)
                << "Requested scene visualization but display gflag is "
                << "set to false.";
        }
    }
    ...
    void chessboardImgCreator()
    {
        ...
        for (int i = 0; i < img_size; i = i + block_size) {
            ...
            for (int j = 0; j < img_size; j = j + block_size) {
                cv::Mat ROI =
                    chessboard(cv::Rect(i, j, block_size, block_size));
...
TEST_F(OpticalFlowPredictorFixture, DefaultNoPredictionOpticalFlowPrediction)
{
    ...
    optical_flow_predictor_->predictSparseFlow(cam_1_kpts_, inter_frame_rot,
                                               &actual_kpts);
...
TEST_F(OpticalFlowPredictorFixture, NoPredictionOpticalFlowPredictionInvariance)
{
    optical_flow_predictor_ =
        buildOpticalFlowPredictor(OpticalFlowPredictorType::kNoPrediction);
    ASSERT_TRUE(optical_flow_predictor_);
    ...
    optical_flow_predictor_->predictSparseFlow(actual_kpts, inter_frame_rot,
                                               &actual_kpts);
...
TEST_F(OpticalFlowPredictorFixture, RotationalOpticalFlowPredictionRotationOnly)
{
    optical_flow_predictor_ =
        buildOpticalFlowPredictor(OpticalFlowPredictorType::kRotational);
    ASSERT_TRUE(optical_flow_predictor_);
...
TEST_F(OpticalFlowPredictorFixture, RotationalOpticalFlowPredictionBehindCam)
{
    ...
    //! are in the back of the camera, but still "project" inside of image of
    //! cam2
