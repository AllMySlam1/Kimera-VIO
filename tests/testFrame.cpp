...
TEST(testFrame, getNrValidKeypoints)
{
    ...
    const int outlier_rate =
        5;  // Insert one outlier every 5 valid landmark ids.
...
{
    // Perform a scan on the grid to verify the correctness of pixel
    // calibration!
    ...
    for (int r = 0; r < numTestRows; r++) {
        for (int c = 0; c < numTestCols; c++) {
            testPointsCV.push_back(
                KeypointCV(c * imgWidth / (numTestCols - 1),
    ...
    for (KeypointsCV::iterator iter = testPointsCV.begin();
        ...
        Vector3 versor = UndistorterRectifier::UndistortKeypointAndGetVersor(
            *iter, cam_params);
        ...
        gtsam::Point2 uncalibrated_px_expected =
            gtsam::Point2(iter->x, iter->y);
...
TEST(testFrame, DISABLED_UndistortKeypointAndGetVersor)
{
    ...
    gtsam::Cal3DS2 gtsam_calib;
    CameraParams::createGtsamCalibration(cam_params.distortion_coeff_mat_,
                                         cam_params.intrinsics_, &gtsam_calib);
...
TEST(testFrame, findLmkIdFromPixel)
{
    ...
    for (int i = 0; i < f.keypoints_.size(); i++) {
        ASSERT_EQ(
            f.findLmkIdFromPixel(f.keypoints_[i], f.keypoints_, f.landmarks_),
            i + 5);
    }
}
