...
struct functor
{
    ...
    void operator()() const
    {
        VLOG(1) << "In functor run for ";
    }
};

void func()
{
    VLOG(1) << "In function, run for ";
}
...
TEST(testTimer, testManualTimerEmptyFunction)
{
    ...
    EXPECT_LE(VIO::utils::Timer::toc<std::chrono::nanoseconds>(tic).count(),
              1e4);
}
...
TEST(testTimer, testFunctionMeasurer)
{
    ...
    auto delta_functor =
        VIO::utils::Measure<std::chrono::nanoseconds>::execution(
            functor(dummy));
