...
TEST_F(BackendLoggerFixture, logBackendOutput)
{
    ...
    logger_->logBackendOutput(BackendOutput(
        timestamp, state_values, gtsam::NonlinearFactorGraph(), W_Pose_Blkf,
        W_Vel_Blkf, imu_bias, gtsam::Matrix(), cur_kf_id, landmark_count,
        DebugVioInfo(), PointsWithIdMap(), LmkIdToLmkTypeMap()));
    ...
    std::vector<std::string> expected_results_header = {
        "#timestamp", "x",  "y",   "z",   "qw",  "qx",  "qy",  "qz", "vx",
    ...
    std::string smart_factors_csv =
        FLAGS_output_path + "output_smartFactors.csv";
    ...
    std::vector<std::string> expected_smart_factors_header = {
        ...
        "numValid",       "numDegenerate",      "numFarPoints",
        "numOutliers",    "numCheirality",      "numNonInitialized",
        "meanPixelError", "maxPixelError",      "meanTrackLength",
        "maxTrackLength", "nrElementsInMatrix", "nrZeroElementsInMatrix"};
    ...
    std::vector<std::string> expected_pim_header = {"#timestamp_kf",
                                                    "x",
                                                    "y",
                                                    "z",
                                                    "qw",
    ...
    std::vector<std::string> expected_factor_stats_header = {
        "#cur_kf_id",        "numAddedSmartF",    "numAddedImuF",
        "numAddedNoMotionF", "numAddedConstantF", "numAddedBetweenStereoF",
        "state_size",        "landmark_count"};
    ...
    std::vector<std::string> expected_timing_header = {
        "#cur_kf_id",     "factorsAndSlotsTime",
        "preUpdateTime",  "updateTime",
        "updateSlotTime", "extraIterationsTime",
        "linearizeTime",  "linearSolveTime",
        "retractTime",    "linearizeMarginalizeTime",
        "marginalizeTime"};
...
TEST_F(FrontendLoggerFixture, logFrontendStats)
{
    ...
    logger_->logFrontendStats(timestamp, VIO::DebugTrackerInfo(),
                              VIO::TrackerStatusSummary(), nrKeypoints);
...
TEST_F(LoopClosureDetectorLoggerFixture, logOptimizedTraj)
{
    ...
    logger_->logOptimizedTraj(LcdOutput(true, timestamp_kf, timestamp_query,
                                        timestamp_match, id_match, id_recent,
                                        relative_pose, w_pose_map, traj_values,
