    ...
    for (size_t test = 0; test < 5; test++) {
        ...
        switch (test) {
            ...
            case 4:
                ...
                n_gravity =
                    Rot3::Expmap(gtsam::Vector3(0.1, 1, 0.5)).matrix() * acc;
                break;
        }
        ...
        VioNavState initial_state_guess =
            InitializationFromImu::getInitialStateEstimate(accGyroRaw,
                                                           n_gravity, round);
        ...
        EXPECT_TRUE(gtsam::assert_equal(n_gravityDir_expected,
                                        n_gravityDir_actual, tol));
