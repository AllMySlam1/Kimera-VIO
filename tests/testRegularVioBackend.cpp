                              ...
                              const Vector3& v)
{
    ...
    for (int f_id = 0; f_id < num_keyframes; f_id++) {
        ...
        // the robot moves to the right, keeping the point in front of the
        // camera
...
TEST(testRegularVio, robotMovingWithConstantVelocity)
{
    ...
    for (int64_t k = 1; k < num_key_frames; k++) {
        ...
        cout << "at frame " << k << " nr factors: " << nrFactorsInSmoother
             << endl;
        ...
        if (k == 1) {
            ...
            // 3 priors, 1 imu + 1 between per time stamp, 8 * k projection
            // factors.
        ...
        for (size_t frame_id = 0; frame_id <= k; frame_id++) {
            ...
            EXPECT_TRUE(
                assert_equal(poses.at(frame_id).first, W_Pose_Blkf, tol));
...
TEST(testRegularVio, robotMovingWithConstantVelocitySmartAndProjFactor)
{
    ...
    for (int64_t k = 1; k < num_key_frames; k++) {
        ...
        if (k == 1) {
            ...
            // 3 priors, 1 imu + 1 between per time stamp,
            // Smart Factors: max (0, 8 - k). such that when k > 8 smart factors
            // = 0. Projection Factors: min ( , ). such that when k > 8 proj
            // factors = 8*k.
        ...
        for (int frame_id = 0; frame_id <= k; frame_id++) {
            ...
            ASSERT_TRUE(gtsam::assert_equal(poses.at(frame_id).first,
                                            W_Pose_Blkf, tol));
