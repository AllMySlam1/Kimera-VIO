  ...
  public:
    RgbdCameraFixture()
        : vio_params_(FLAGS_test_data_path + "/EurocParams"),
          rgbd_camera_(nullptr), visualizer_3d_(nullptr),
...
TEST_F(RgbdCameraFixture, convertToPoincloud)
{
    ...
    for (int v = 0u; v < depth_map.rows; v++) {
        for (int u = 0u; u < depth_map.cols; u++) {
            ...
            depth_map.at<uint16_t>(v, u) =
                1000.0f + 200.0f * std::sin(static_cast<float>(u) /
                                            static_cast<float>(depth_map.rows) *
                                            2.0f * M_PI);
        }
    }
    ...
    EXPECT_TRUE(UtilsOpenCV::compareCvMatsUpToTol(expected_cloud, actual_cloud,
                                                  0.00001));
    if (FLAGS_display) {
