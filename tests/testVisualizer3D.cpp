...
class VisualizerFixture : public ::testing::Test {
  public:
    VisualizerFixture()
        : camera_params_(), img_(), img_name_(), frame_(nullptr),
          viz_type_(VisualizationType::kMesh2dTo3dSparse),
          backend_type_(BackendType::kStereoImu), visualizer_(nullptr)
    {
...
TEST_F(VisualizerFixture, spinOnce)
{
    ...
    BackendOutput::Ptr backend_output = std::make_shared<BackendOutput>(
        timestamp, gtsam::Values(), gtsam::NonlinearFactorGraph(),
        gtsam::Pose3::identity(), Vector3::Zero(), ImuBias(), gtsam::Matrix(),
        FrameId(), 0, DebugVioInfo(), PointsWithIdMap(), LmkIdToLmkTypeMap());
    StereoFrontendOutput::Ptr frontend_output =
        std::make_shared<StereoFrontendOutput>(
            true, StatusStereoMeasurementsPtr(), TrackingStatus(),
            gtsam::Pose3::identity(), gtsam::Pose3::identity(),
            gtsam::Pose3::identity(),
            StereoFrame(FrameId(), timestamp,
                        Frame(FrameId(), timestamp, camera_params_, cv::Mat()),
                        Frame(FrameId(), timestamp, camera_params_, cv::Mat())),
            ImuFrontend::PimPtr(), ImuAccGyrS(), cv::Mat(), DebugTrackerInfo());
    LcdOutput::Ptr  lcd_output = std::make_shared<LcdOutput>(timestamp);
    VisualizerInput visualizer_input(timestamp, mesher_output, backend_output,
                                     frontend_output, lcd_output);
    // Visualize mesh.
