  ...
  public:
    CameraFixture()
        : vio_params_(FLAGS_test_data_path + "/EurocParams"),
          mono_camera_(nullptr), left_frame_queue_("left_frame_queue"),
          right_frame_queue_("right_frame_queue"), window_()
    {
        ...
        // Create Mono Camera
        mono_camera_ =
            VIO::make_unique<Camera>(vio_params_.camera_params_.at(0));
        CHECK(mono_camera_);
    }
...
TEST_F(CameraFixture, backProjectSingleRandom)
{
    ...
    LandmarkCV expected_lmk(depth / fx * (kpt.x - px),
                            depth / fy * (kpt.y - py), depth);
    EXPECT_NEAR(expected_lmk.x, actual_lmk.x, 0.0001);
    EXPECT_NEAR(expected_lmk.y, actual_lmk.y, 0.0001);
    EXPECT_NEAR(expected_lmk.z, actual_lmk.z, 0.0001);
}
...
TEST_F(CameraFixture, backProjectMultipleComplex)
{
    ...
    for (size_t i = 0u; i < kKeypoints; i++) {
        ...
        kpts.push_back(KeypointCV(
            i * (camera_params.image_size_.width - 1) / kKeypoints,
        ...
        // horizontal
        kpts.push_back(KeypointCV(
            i * (camera_params.image_size_.width - 1) / kKeypoints, py));
