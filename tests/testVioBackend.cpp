...
class BackendFixture : public ::testing::Test {
  ...
  protected:
    ...
    void createCameraPoses(StereoPoses* stereo_poses)
    {
        ...
        // The camera is assumed to face (0, 0, 1): z-forward, y down and x to
        // the right
        for (int f_id = 0; f_id < num_keyframes_; f_id++) {
            // constant velocity model along x: the robot moves to the right,
            // keeping the point in front of the camera
    ...
    void createImuBuffer(VIO::utils::ThreadsafeImuBuffer* imu_buf)
    {
        ...
        // Add a very old imu measurement to avoid the imu buffer complaining
        // that there was not a message before, when querying it i.e.
        // kDataNeverAvailable
        Timestamp t = 0u;
...
TEST_F(BackendFixture, robotMovingWithConstantVelocity)
{
    ...
    StereoCalibPtr stereo_calibration =
        boost::make_shared<gtsam::Cal3_S2Stereo>(
            cam_params.fx(), cam_params.fy(), cam_params.skew(),
            cam_params.px(), cam_params.py(), baseline);
    ...
    std::shared_ptr<VioBackend> vio_backend = std::make_shared<VioBackend>(
        B_pose_camLrect, stereo_calibration, backend_params_, imu_params_,
        BackendOutputParams(false, 0, false), false);
    vio_backend->registerImuBiasUpdateCallback(
        std::bind(&ImuFrontend::updateBias, std::ref(imu_frontend),
                  std::placeholders::_1));

    // For each frame, add landmarks and optimize.
    Timestamp timestamp_km1 =
        t_start_ - before_start_imu_msgs_ * imu_time_step_;
    for (FrameId k = 0u; k < num_keyframes_; k++) {
        ...
        switch (imu_status) {
            ...
            case utils::ThreadsafeImuBuffer::QueryResult::kQueueShutdown: {
                LOG(WARNING) << "IMU buffer was shutdown. Shutting down "
                                "DataProviderModule.";
                break;
            }
        ...
        BackendOutput::Ptr backend_output = vio_backend->spinOnce(BackendInput(
            ...
            tracker_status_valid.kfTrackingStatus_stereo_, pim, imu_accgyr));
        ...
        VLOG(1) << "Frame: " << k
                << ", # of factors: " << nr_factors_in_smoother;
        if (imu_params_.imu_preintegration_type_ ==
            ...
            if (k == 0) {
                ...
                EXPECT_EQ(
                    nr_factors_in_smoother,
                    3 + k +
                        8);  // 3 priors, 1 imu per time stamp, 8 smart factors
            }
        }
        else {
            if (k == 0) {
                ...
                EXPECT_EQ(
                    nr_factors_in_smoother,
                    3 + 2 * k);  // 3 priors, 1 imu + 1 between per time stamp:
                                 // we do not include smart factors of length 1
            }
            else {
                EXPECT_EQ(nr_factors_in_smoother,
                          3 + 2 * k + 8);  // 3 priors, 1 imu + 1 between per
                                           // time stamp, 8 smart factors
            }
        }
        ...
        for (FrameId f_id = 0u; f_id <= k; f_id++) {
            ...
            ImuBias imu_bias_lkf =
                results.at<ImuBias>(gtsam::Symbol('b', f_id));
