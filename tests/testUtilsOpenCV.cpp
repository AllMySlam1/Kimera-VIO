...
class UtilsOpenCVFixture : public ::testing::Test {
  public:
    UtilsOpenCVFixture()
        : chess_img_path_(std::string(FLAGS_test_data_path) +
                          "/chessboard.png"),
          real_img_path_(std::string(FLAGS_test_data_path) + "/realImage.png"),
          R_gtsam_(), T_gtsam_(), pose_gtsam_(), R_cvmat_(), T_cvmat_()
    {
        // clang-format off
    R_gtsam_ = gtsam::Rot3(
  ...
  protected:
    ...
    pair<cv::Mat, std::vector<KeypointCV>>
    cvCreateChessboard(int square_width,
                       int num_pattern_row,
                       int num_pattern_col) const
    {
        ...
        for (int pr = 1; pr < num_pattern_row; pr++) {
            for (int pc = 1; pc < num_pattern_col; pc++) {
                corners.push_back(KeypointCV(pc * square_width - 0.5,
                                             pr * square_width - 0.5));
            }
        }
...
TEST_F(UtilsOpenCVFixture, CvMatCmp)
{
    ...
    EXPECT_TRUE(
        UtilsOpenCV::compareCvMatsUpToTol(chessboardImg, chessboardImg));
...
TEST_F(UtilsOpenCVFixture, poseVectorToGtsamPose3)
{
    ...
    gtsam::Pose3 pose_actual =
        UtilsOpenCV::poseVectorToGtsamPose3(pose_vector_);
    // Comparison!
...
TEST_F(UtilsOpenCVFixture, gtsamPose3ToCvAffine3d)
{
    ...
    // Actual result
    cv::Affine3d affine_actual =
        UtilsOpenCV::gtsamPose3ToCvAffine3d(pose_gtsam_);

    // Comparison
    for (int r = 0; r < 3; r++) {
        EXPECT_NEAR(T_expected.at<double>(r, 0),
                    double(affine_actual.translation()(r)), tol_);
        for (int c = 0; c < 3; c++) {
            EXPECT_NEAR(R_expected.at<double>(r, c),
                        double(affine_actual.rotation()(r, c)), tol_);
        }
    }
}
...
TEST_F(UtilsOpenCVFixture, computeRTErrors_upToScale)
{
    ...
    {
        // translation of new pose is the same of the expected, but with unit
        // norm
    ...
    {
        // translation of new pose is the same of the expected, but with some
        // other norm
        double normT = pose_gtsam_.translation().norm();
...
TEST_F(UtilsOpenCVFixture, ReadAndConvertToGrayScale)
{
    // original image is already gray, hence it remains the same
    {
        ...
        std::tie(chessboardImg, keypoints_expected) =
            cvCreateChessboard(30, 10, 8);
        cv::Mat chessboardImgGray =
            UtilsOpenCV::ReadAndConvertToGrayScale("chessboard.png");
        EXPECT_EQ(chessboardImgGray.channels(), 1);
        EXPECT_TRUE(UtilsOpenCV::compareCvMatsUpToTol(chessboardImg,
                                                      chessboardImgGray));
    }
    // original image is in color and it is converted to gray
    {
        ...
        EXPECT_TRUE(
            UtilsOpenCV::compareCvMatsUpToTol(img_gray, img_gray_written));
    }
    // original image is already gray, hence it remains the same
    {
        cv::Mat img      = cv::imread(FLAGS_test_data_path + "/testImage.png",
                                 IMREAD_ANYCOLOR);
        cv::Mat img_gray = UtilsOpenCV::ReadAndConvertToGrayScale(
