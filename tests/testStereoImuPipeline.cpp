...
class VioPipelineFixture : public ::testing::Test {
  public:
    VioPipelineFixture()
        : dataset_parser_(nullptr), vio_pipeline_(nullptr),
  ...
  protected:
    ...
    void buildOnlinePipeline(const VioParams& vio_params)
    {
        ...
        dataset_parser_ = VIO::make_unique<EurocDataProvider>(
            FLAGS_test_data_path + "/MicroEurocDataset", initial_k, final_k,
            vio_params);
    ...
    void connectVioPipeline()
    {
        ...
        dataset_parser_->registerImuSingleCallback(
            std::bind(&VIO::StereoImuPipeline::fillSingleImuQueue,
                      vio_pipeline_.get(), std::placeholders::_1));
    ...
    void connectVioPipelineWithBlockingIfFullQueues()
    {
        ...
        dataset_parser_->registerRightFrameCallback(std::bind(
            &VIO::StereoImuPipeline::fillRightFrameQueueBlockingIfFull,
            vio_pipeline_.get(), std::placeholders::_1));
    }
...
TEST_F(VioPipelineFixture, OnlineParallelStartManualShutdown)
{
    ...
    auto handle_pipeline = std::async(
        std::launch::async, &VIO::StereoImuPipeline::spin, vio_pipeline_.get());
    vio_pipeline_->shutdown();
...
TEST_F(VioPipelineFixture, OnlineParallelSpinManualShutdown)
{
    ...
    ASSERT_TRUE(vio_pipeline_);
    auto handle =
        std::async(std::launch::async, &VIO::DataProviderInterface::spin,
                   dataset_parser_.get());
    auto handle_pipeline = std::async(
        std::launch::async, &VIO::StereoImuPipeline::spin, vio_pipeline_.get());
    vio_pipeline_->shutdown();
...
TEST_F(VioPipelineFixture, OnlineParallelSpinShutdownWhenFinished)
{
    ...
    auto handle_shutdown = std::async(
        std::launch::async, &VIO::StereoImuPipeline::shutdownWhenFinished,
        vio_pipeline_.get(), 500, true);
