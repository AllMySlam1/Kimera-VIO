...
class MeshUtilsFixture : public ::testing::Test {
  public:
    MeshUtilsFixture()
    {
        expected_bary_img_ = loadImage(
            FLAGS_test_data_path + '/' + folder_name_, bary_img_name_);
        expected_color_img_ = loadImage(
            FLAGS_test_data_path + '/' + folder_name_, color_img_name_);
    }
...
TEST_F(MeshUtilsFixture, testRayIntersectAlgorithm)
{
    ...
    for (uint32_t j = 0; j < height; ++j) {
        for (uint32_t i = 0; i < width; ++i) {
            // compute primary ray
            float x =
                (2 * (i + 0.5) / (float)width - 1) * imageAspectRatio * scale;
