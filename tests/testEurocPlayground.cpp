...
TEST(TestEurocPlayground, DISABLED_basicEurocPlayground)
{
    EurocPlayground euroc_playground(FLAGS_test_data_path + "/V1_01_easy/",
                                     FLAGS_test_data_path + "/EurocParams", 50,
                                     1000, 300);
    ...
    MeshOptimization mesh_opt(MeshOptimizerType::kGtsamMesh,
                              MeshColorType::kVertexFlatColor, mono_camera,
    ...
    if (FLAGS_display) {
        ...
        for (MeshPackets::const_iterator it =
            ...
            for (const auto& keypoint : keypoints) {
                cv::circle(mesh_2d_viz, keypoint, 1, cv::viz::Color::blue(),
                           CV_FILLED, CV_AA, 0);
            }
            ...
            // This doesn't make too much sense because x,y coordinates are
            // pixels! convertMesh2dTo3d(
            //     mesh_2d, cv::Mat::eye(cv::Size(3, 2), CV_32F),
            //     &input.mesh_3d);
            ...
            MeshOptimizationOutput::UniquePtr out_ptr =
                mesh_opt.spinOnce(input);
