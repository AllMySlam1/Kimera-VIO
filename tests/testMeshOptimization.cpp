...
TEST_F(MeshOptimizationFixture, DISABLED_testCollectTriangleDataPointsFast)
{
    ...
    MeshOptimization mesh_opt(MeshOptimizerType::kGtsamMesh,
                              MeshColorType::kVertexSupport, mono_camera);
    ...
    VIO::Depths depths = {0.5, std::numeric_limits<double>::max(), 0.5, 0.0003};
    ...
    // the point cloud shape must be the same as the image size which is 2x2
    // now. This is interpreted as: the pixel in {0,0} has a 3D point at
    // expected_lmk0 the pixel in {0,1} has a 3D point in expected_lmk1 etc...
  ...
  //  datapoint inside: x
    ...
    EXPECT_TRUE(MeshOptimization::pointInTriangle(pixel_inside, vtx1_xyz,
                                                  vtx2_xyz, vtx3_xyz));
    EXPECT_FALSE(MeshOptimization::pointInTriangle(pixel_outside, vtx1_xyz,
                                                   vtx2_xyz, vtx3_xyz));
    ...
    mesh_opt.collectTriangleDataPointsFast(point_cloud, mesh_2d, &corresp,
                                           &pixel_corresp,
