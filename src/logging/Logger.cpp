                                  ...
                                  bool               open_file_in_append_mode)
{
    ...
    OpenFile(output_path_ + '/' + output_file_name, &ofstream_,
    ...
    const Timestamp&      timestamp_k) const
{
    ...
    const gtsam::Vector3& rpy_gt =
        initial_state_gt.pose_.rotation()
    ...
    CHECK_LE(vioTranError, 1e-4) << "stereoVIOExample: wrong initialization "
...
void BackendLogger::logBackendResultsCSV(const BackendOutput& vio_output)
{
    ...
    const auto& w_pose_blkf_trans =
        cached_state.pose_.translation().transpose();
    ...
    const auto& imu_bias_acc =
        cached_state.imu_bias_.accelerometer().transpose();
...
void BackendLogger::logBackendPimNavstates(const BackendOutput& output)
{
    ...
    const gtsam::Velocity3&  velocity =
        output.debug_info_.navstate_k_.velocity();
...
VisualizerLogger::VisualizerLogger()
    : output_mesh_("output_mesh.ply"), output_landmarks_("output_landmarks.txt")
{
}
                               ...
                               bool           log_accumulated_mesh)
{
    ...
    // Second, log vertices.
    for (int i = 0; i < lmks.rows; i++) {
        output_mesh_stream << lmks.at<float>(i, 0)
                           << " "  // Log vertices x y z.
                           << lmks.at<float>(i, 1) << " "
                           << lmks.at<float>(i, 2) << " "
    ...
    const size_t&               nrKeypoints)
{
    ...
    output_stream_stats << timestamp_lkf
                        ...
                        << tracker_info.need_n_corners_ << std::endl;
}
    ...
    const gtsam::Pose3& relative_pose_body_stereo)
{
    ...
    // Log relative stereo poses; pose from previous keyframe to current
    // keyframe, in previous-keyframe coordinates. These are not cumulative
    // trajectories.
    ...
    const std::chrono::milliseconds& duration)
{
    ...
    std::ofstream& outputFile_timingOverall_ =
        output_pipeline_timing_.ofstream_;
...
LoopClosureDetectorLogger::LoopClosureDetectorLogger()
    : output_lcd_("output_lcd_result.csv"), output_traj_("traj_pgo.csv"),
      output_status_("output_lcd_status.csv"), ts_map_()
{
}
...
void LoopClosureDetectorLogger::logLoopClosure(const LcdOutput& lcd_output)
{
    ...
    if (!is_header_written) {
        output_stream_lcd
            << "#timestamp_kf,timestamp_query,timestamp_match,isLoop,"
...
void LoopClosureDetectorLogger::logOptimizedTraj(const LcdOutput& lcd_output)
{
    ...
    // TODO(marcus): set the append to false on this one and overwrite EVERY
    // TIME
...
void LoopClosureDetectorLogger::logDebugInfo(const LcdDebugInfo& debug_info)
{
    ...
    output_stream_status
        << debug_info.timestamp_ << ","
        << LoopResult::asString(debug_info.loop_result_.status_) << ","
        << debug_info.loop_result_.query_id_ << ","
        << debug_info.loop_result_.match_id_ << ","
        << debug_info.mono_input_size_ << "," << debug_info.mono_inliers_ << ","
        << debug_info.mono_iter_ << "," << debug_info.stereo_input_size_ << ","
        << debug_info.stereo_inliers_ << "," << debug_info.stereo_iter_ << ","
        << debug_info.pgo_size_ << "," << debug_info.pgo_lc_count_ << ","
