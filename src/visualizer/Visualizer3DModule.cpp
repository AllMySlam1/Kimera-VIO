...
VisualizerModule::VisualizerModule(OutputQueue*            output_queue,
    ...
    : MISOPipelineModule<VisualizerInput, DisplayInputBase>(output_queue,
      ...
      backend_queue_("visualizer_backend_queue"), mesher_queue_(nullptr),
      lcd_queue_(nullptr), visualizer_(std::move(visualizer))
{
    ...
    if (use_lcd) {
        lcd_queue_ = VIO::make_unique<ThreadsafeQueue<VizLcdInput>>(
            "visualizer_lcd_queue");
    }
}
...
{
    CHECK(lcd_queue_)
        << "Filling lcd queue without lcd_queue_ being "
           "initialized... Make sure you tell the visualizer that "
           "you want to use lcd...";
...
VisualizerModule::InputUniquePtr VisualizerModule::getInputPacket()
{
    ...
    if (PIO::parallel_run_) {
        // TODO(Toni): if the mesher_ is not running (it is optional) then there
        // is no module filling this queue, and the visualizer gets stuck at
        // this pop blocking :(
    ...
    if (!queue_state) {
        std::string msg = "Module: " + name_id_ + " - " +
                          backend_queue_.queue_id_ + " queue is down";
    ...
    if (mesher_queue_) {
        ...
        // Sync may fail is someone shuts down the pipeline, so no checks at
        // this level.
    ...
    return VIO::make_unique<VisualizerInput>(timestamp, mesher_payload,
                                             backend_payload, frontend_payload,
                                             lcd_payload);
}

VisualizerModule::OutputUniquePtr
VisualizerModule::spinOnce(VisualizerInput::UniquePtr input)
{
    CHECK(input);
...
{
    LOG_IF(WARNING, (mesher_queue_ ? mesher_queue_->empty() : false) &&
