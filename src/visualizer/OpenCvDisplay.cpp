...
OpenCv3dDisplay::OpenCv3dDisplay(
    ...
    : DisplayBase(display_params->display_type_),  // DANGEROUS
      window_data_(), shutdown_pipeline_cb_(shutdown_pipeline_cb),
      params_(
          VIO::safeCast<DisplayParams, OpenCv3dDisplayParams>(*display_params))
{
    if (VLOG_IS_ON(2)) {
...
void OpenCv3dDisplay::spin3dWindow(VisualizerOutput::UniquePtr&& viz_output)
{
    ...
    if (viz_output->visualization_type_ != VisualizationType::kNone) {
        if (window_data_.window_.wasStopped()) {
            // Shutdown the pipeline! This works because display is running in
            // the main thread, otherwise you would get a nasty:
            // ```
        ...
        for (auto it = widgets.begin(); it != widgets.end(); ++it) {
            CHECK(it->second);
            // This is to go around opencv issue #10829, new opencv should have
            // this fixed. cv::viz::Widget3D::getPose segfaults.
            it->second->updatePose(cv::Affine3d());
            window_data_.window_.showWidget(it->first, *(it->second),
                                            it->second->getPose());
        }
...
void OpenCv3dDisplay::setMeshProperties(WidgetsMap* widgets)
{
    ...
    if (mesh_iterator == widgets->end()) {
        LOG_EVERY_N(WARNING, 100)
            << "Missing Mesh in visualization's 3D widgets.";
        return;
    }
    ...
    switch (window_data_.mesh_representation_) {
        ...
        case 2: {
            mesh_widget->setRenderingProperty(
                cv::viz::REPRESENTATION, cv::viz::REPRESENTATION_WIREFRAME);
            break;
        }
        default: {
            break;
    ...
    WindowData*  window_data)
{
    ...
    if (code == 'w') {
        LOG(WARNING) << "Pressing " << code
                     << " displays current window size:\n"
                     << "\theight: "
                     << window_data->window_.getWindowSize().height
                     << "\twidth: "
                     << window_data->window_.getWindowSize().width;
    }
}
...
void OpenCv3dDisplay::recordVideo()
{
    ...
    if (i == 0u)
        CHECK(common::createDirectory(dir_path, dir_name));
