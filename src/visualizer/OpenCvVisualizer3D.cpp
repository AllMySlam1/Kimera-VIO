...
OpenCvVisualizer3D::spinOnce(const VisualizerInput& input)
{
    ...
    switch (visualization_type_) {
        ...
        // produced by cv::subdiv, which have uniform gradient (updateMesh3D
        // also filters out geometrically) Sparsity comes from filtering out
        // triangles corresponding to non planar obstacles which are assumed to
        // have non-uniform gradient.
        case VisualizationType::kMesh2dTo3dSparse: {
            ...
            if (FLAGS_visualize_mesh_2d || FLAGS_visualize_mesh_in_frustum) {
                const ImageToDisplay& mesh_display = ImageToDisplay(
                    "Mesh 2D", visualizeMesh2DStereo(
                                   input.mesher_output_->mesh_2d_for_viz_,
                ...
                if (FLAGS_visualize_semantic_mesh) {
                    ...
                    LOG_IF(WARNING,
                        ...
                        << "Both gflags visualize_semantic_mesh and "
                           "visualize_mesh_with_colored_polygon_cluster are "
                           "set to True,"
                           " but visualization of the semantic mesh has "
                           "priority over "
                           "visualization of the polygon clusters.";
                    visualizeMesh3D(vertices_mesh_prev,
                                    mesh_3d_viz_props_prev.colors_,
                                    polygons_mesh_prev, &output->widgets_,
                    ...
                    LOG_IF(ERROR, mesh_3d_viz_props_prev.colors_.rows > 0u)
                        << "The 3D mesh is being colored with semantic "
                           "information, but"
            ...
            if (!FLAGS_visualize_load_mesh_filename.empty()) {
                // TODO(Toni): remove static, never use static wo
                // const/constexpr
            ...
            if (backend_type_ == BackendType::kStructuralRegularities &&
                ...
                for (const auto& g : input.backend_output_->factor_graph_) {
                    ...
                    if (ppf) {
                        ...
                        CHECK(getEstimateOfKey(input.backend_output_->state_,
                                               point_key, &point));
                        // Visualize.
                        ...
                        for (const Plane& plane :
                             input.mesher_output_->planes_) {
                            if (ppf_plane_key == plane.getPlaneSymbol().key()) {
                                gtsam::OrientedPlane3 current_plane_estimate;
                                CHECK(getEstimateOfKey(  // This call makes
                                                         // visualizer unable to
                                                         // perform this in
                                                         // parallel.
                                    input.backend_output_->state_,
                                    ppf_plane_key, &current_plane_estimate));
                                // WARNING assumes the Backend updates normal
                                // and distance of plane and that no one
                                // modifies it afterwards...
                                visualizePlaneConstraints(
                                    plane.getPlaneSymbol().key(),
                                    current_plane_estimate.normal().point3(),
                                    current_plane_estimate.distance(), lmk_id,
                                    point, &output->widgets_);
                                // Stop since there are not multiple planes for
                                // one ppf.
                                break;
                            }
                        }
            ...
            if (FLAGS_visualize_planes || FLAGS_visualize_plane_constraints) {
                for (const Plane& plane : input.mesher_output_->planes_) {
                    ...
                    if (getEstimateOfKey<gtsam::OrientedPlane3>(
                            input.backend_output_->state_, plane_symbol.key(),
                        ...
                        // Visualize plane.
                        visualizePlane(
                            plane_index, plane_normal_estimate.x,
                            plane_normal_estimate.y, plane_normal_estimate.z,
                            current_plane_estimate.distance(),
                            &output->widgets_, FLAGS_visualize_plane_label,
                            plane.triangle_cluster_.cluster_id_);
                    }
                    else {
                        // We could not find the plane in the optimization...
                        // Careful cause we might enter here because there are
                        // new segmented planes. Delete the plane.
            ...
            mesh_3d_viz_props_prev =
                // Call semantic mesh segmentation if someone registered a
                // callback.
                mesh3d_viz_properties_callback_ ?
                    ...
                    (FLAGS_texturize_3d_mesh ?
                         OpenCvVisualizer3D::texturizeMesh3D(
                             left_stereo_keyframe.timestamp_,
                             left_stereo_keyframe.img_,
                             input.mesher_output_->mesh_2d_,
                             input.mesher_output_->mesh_3d_) :
        ...
        // Computes and visualizes a 3D point cloud with VIO points in current
        // time horizon of the optimization.
        case VisualizationType::kPointcloud: {
            // Do not color the cloud, send empty lmk id to lmk type map
            // TODO(Toni): don't use the backend's maps, instead, build these
            // maps, in the visualizer using the state and the factor graph.
    ...
    visualizePoseWithImgInFrustum(
        ...
        input.frontend_output_->frontend_type_ == FrontendType::kStereoImu ?
            ...
            VIO::safeCast<FrontendOutputPacketBase, MonoFrontendOutput>(
                input.frontend_output_)
                ->feature_tracks_,
        trajectory_poses_3d_.back(), &output->widgets_);
    ...
    visualizeFactorGraph(
        input.backend_output_->state_, input.backend_output_->factor_graph_,
        b_Pose_cam_Lrect,
        input.frontend_output_->frontend_type_ == FrontendType::kStereoImu ?
            VIO::safeCast<FrontendOutputPacketBase, StereoFrontendOutput>(
                input.frontend_output_)
    ...
    WidgetsMap*                        widgets_map)
{
    ...
    for (const gtsam::Values::ConstKeyValuePair& key_value : state) {
        ...
        if (variable_type == kLandmarkSymbolChar) {
            // const LandmarkId& lmk_id = variable_index;  // THIS IS NOT
            // LMK_ID! const gtsam::Point3& lmk_position =
        ...
        if (variable_type == kPoseSymbolChar) {
            ...
            if (draw_imu_to_left_cam_arrow) {
                drawImuToLeftCamArrow(imu_pose, world_pose_camLrect,
                                      variable_index, widgets_map);
            }
            continue;
        }
    ...
    for (const boost::shared_ptr<gtsam::NonlinearFactor>& factor :
         factor_graph) {
        if (draw_smart_stereo_factors) {
            ...
            if (smart_stereo_factor) {
                drawSmartStereoFactor(*smart_stereo_factor, state,
                                      body_pose_camLrect, widgets_map);
                continue;
            }
        }
        ...
        if (draw_linear_container_factors) {
            const auto& lcf =
                boost::dynamic_pointer_cast<gtsam::LinearContainerFactor>(
                    factor);
            if (lcf) {
                drawLinearContainerFactor(*lcf, state, body_pose_camLrect,
                                          widgets_map);
                continue;
            }
        }
        ...
        if (draw_pose_priors) {
            const auto& pose_prior =
                boost::dynamic_pointer_cast<gtsam::PriorFactor<gtsam::Pose3>>(
                    factor);
            if (pose_prior) {
                drawPosePrior(*pose_prior, state, body_pose_camLrect,
                              widgets_map);
                continue;
            }
        }
        ...
        if (draw_between_factors) {
            const auto& btw_factor =
                boost::dynamic_pointer_cast<gtsam::BetweenFactor<gtsam::Pose3>>(
                    factor);
            if (btw_factor) {
                drawBtwFactor(*btw_factor, state, body_pose_camLrect,
                              widgets_map);
                continue;
            }
        }
        ...
        if (draw_preintegrated_imu_factors) {
            ...
            if (imu_factor) {
                drawImuFactor(*imu_factor, state, body_pose_camLrect,
                              widgets_map);
                continue;
            }
        }
    ...
    WidgetsMap*         widgets_map)
{
    ...
    cv::Point3d arrow_start(imu_position.x(), imu_position.y(),
                            imu_position.z());
    cv::Point3d arrow_end(left_cam_position.x(), left_cam_position.y(),
                          left_cam_position.z());
    ...
    (*widgets_map)[imu_to_left_cam_id] = VIO::make_unique<cv::viz::WArrow>(
        arrow_start, arrow_end, imu_to_left_cam_vector_scale_,
                                           ...
                                           WidgetsMap*           widgets_map)
{
    ...
    (*widgets_map)["IMU vel " + std::to_string(variable_index)] =
        VIO::make_unique<cv::viz::WArrow>(arrow_start, arrow_end, 0.001,
                                          velocity_vector_color_);
}
    ...
    WidgetsMap*              widgets_map)
{
    ...
    if (result.valid()) {
        ...
        for (size_t i = 0u; i < stereo_cams.size(); i++) {
            ...
            cv::Point3d arrow_start(left_cam_pose.x(), left_cam_pose.y(),
                                    left_cam_pose.z());
            ...
            (*widgets_map)[cam_to_lmk_line_id] =
                VIO::make_unique<cv::viz::WArrow>(arrow_start, arrow_end,
                                                  0.0005, arrow_color);
        ...
        if (symbol.chr() == kPoseSymbolChar) {
            ...
            (*widgets_map)[left_cam_id] =
                VIO::make_unique<cv::viz::WCameraPosition>(
                    K_, cam_with_linear_prior_frustum_scale_,
    ...
    WidgetsMap*                               widgets_map)
{
    ...
    if (velocity_symbol.chr() == kVelocitySymbolChar) {
        gtsam::Pose3 imu_pose;
        CHECK(getEstimateOfKey(
            state, gtsam::Symbol(kPoseSymbolChar, velocity_symbol.index()),
            &imu_pose));

        // Print text saying that we got a velocity prior.
        const cv::Point3d text_position(imu_pose.x(), imu_pose.y(),
                                        imu_pose.z());
        std::string       info =
        ...
        (*widgets_map)[velocity_prior_cube_id] =
            VIO::make_unique<cv::viz::WCube>(min_point, max_point, true,
                                             velocity_prior_color_);
    ...
    WidgetsMap*                             widgets_map)
{
    ...
    (*widgets_map)[left_cam_id] = VIO::make_unique<cv::viz::WCameraPosition>(
        K_, cam_with_pose_prior_frustum_scale_,
    ...
    WidgetsMap*                               widgets_map)
{
    ...
    if (btw_factor.measured().equals(gtsam::Pose3::identity())) {
        ...
        (*widgets_map)[no_motion_prior_id + " (edge)"] =
            VIO::make_unique<cv::viz::WCylinder>(start_point, end_point,
                                                 kCylinderRadius, 20,
                                                 no_motion_prior_color_);

        // Add a sphere
        (*widgets_map)[no_motion_prior_id + " (factor)"] =
            VIO::make_unique<cv::viz::WSphere>(mid_point, kSphereRadius, 10,
                                               no_motion_prior_color_);
        ...
        (*widgets_map)[btw_factor_edge_id] =
            VIO::make_unique<cv::viz::WCylinder>(
        ...
        (*widgets_map)[btw_factor_pose_guess_id] =
            VIO::make_unique<cv::viz::WCameraPosition>(
                K_, btw_factor_pose_guess_active_frustum_scale_,
        ...
        (*widgets_map)[btw_factor_arrow_id] = VIO::make_unique<cv::viz::WArrow>(
            mid_point, cv::Point3d(left_cam_pose.translation()),
                                       ...
                                       WidgetsMap*         widgets_map)
{
    ...
    (*widgets_map)[imu_factor_pose_guess_id] =
        VIO::make_unique<cv::viz::WCameraPosition>(
            K_, imu_factor_to_guess_pose_scale_,
    ...
    const Frame&                  ref_frame)
{
    ...
    for (size_t i = 0; i < ref_frame.keypoints_.size(); i++) {
        ...
        if (ref_frame.landmarks_[i] != -1) {
            cv::circle(img_clone, ref_frame.keypoints_[i], 2,
                       kInvalidKeypointsColor, CV_FILLED, CV_AA, 0);
        }
    }
                                        ...
                                        const int&     cluster_id)
{
    ...
    if (visualize_plane_label) {
        ...
        (*widgets)[plane_id_for_viz + "_label"] =
            VIO::make_unique<cv::viz::WText3D>(plane_id_for_viz, text_position,
                                               0.07, true);
    }
...
void OpenCvVisualizer3D::visualizeMesh3D(const cv::Mat& map_points_3d,
                                         ...
                                         WidgetsMap*    widgets)
{
    cv::Mat colors(0, 1, CV_8UC3,
                   cv::viz::Color::gray());  // Do not color mesh.
    ...
    if (colors.rows != 0) {
        CHECK_EQ(map_points_3d.rows, colors.rows)
            << "Map points and Colors should have same number of rows. One"
               " color per map point.";
        LOG_IF(ERROR, !tcoords.empty()) << "Texture provided, but colors as "
                                           "well... Do not provide colors if "
                                             ...
                                             const cv::Mat&      normals)
{
    ...
    CHECK(pcl_type == CV_32FC3 || pcl_type == CV_32FC4 ||
          pcl_type == CV_64FC3 || pcl_type == CV_64FC4);
                                             ...
                                             WidgetsMap*    widgets_map)
{
    ...
    if (points_2d.size() != 0) {
        ...
        for (const int& idx : hull_idx) {
            hull_3d.push_back(cv::Point3f(points_2d.at(idx).x,
                                          points_2d.at(idx).y, z_s.at(idx)));
        }
        ...
        if (visualize_hull_as_polyline) {
            ...
            if (hull_3d.size() > 2) {
                cv::Mat polygon_hull =
                    cv::Mat(4 * (hull_3d.size() - 2), 1, CV_32SC1);
                size_t i = 1;
                ...
                (*widgets_map)["Convex Hull"] =
                    VIO::make_unique<cv::viz::WMesh>(hull_3d, polygon_hull,
                                                     colors.t(), normals.t());
            }
        }
    }
    ...
    for (PlaneIdMap::value_type& plane_id_pair : plane_id_map_) {
        ...
        for (LmkIdToLineIdMap::iterator lmk_id_to_line_id_it =
            ...
            if (std::find(lmk_ids.begin(), lmk_ids.end(),
                ...
                std::string line_id =
                    "Line " +
                    std::to_string(static_cast<int>(plane_id_pair.first)) +
                    std::to_string(
                        static_cast<int>(lmk_id_to_line_id_it->second));
                removeWidget(line_id);
                // Delete the corresponding entry in the map from lmk id to line
                // id.
        ...
        if (removeWidget(plane_id_for_viz)) {
            if (remove_plane_label) {
                if (!removeWidget(plane_id_for_viz + "_label")) {
                    LOG(WARNING)
                        << "Did you disable labels of planes?, then also"
...
void OpenCvVisualizer3D::addPoseToTrajectory(const cv::Affine3d& pose)
{
    ...
    if (FLAGS_displayed_trajectory_length > 0) {
        while (trajectory_poses_3d_.size() >
               FLAGS_displayed_trajectory_length) {
...
Mesh3DVizProperties
OpenCvVisualizer3D::texturizeMesh3D(const Timestamp& image_timestamp,
                                    ...
                                    const Mesh3D&    mesh_3d)
{
    ...
    for (size_t i = 0; i < mesh_2d.getNumberOfPolygons(); i++) {
        ...
        if (mesh_3d.getVertex(lmk0, nullptr, &p0_id) &&
            ...
            // These pixels correspond to the tcoords in the image for the 3d
            // mesh vertices.
            VLOG(100) << "Pixel: with id: " << p0_id << ", x: " << px0.x
                      << ", y: " << px0.y;
            // We divide by 2.0 to account for fake default texture padded to
            // the right of the texture_image.
            ...
            // If we did not find a corresponding 3D triangle for the 2D
            // triangle leave tcoords and colors to the default values.
                                 ...
                                 bool             log_accumulated_mesh)
{
    ...
    if ((timestamp - last_timestamp) >
        6500000000) {  // Log every 6 seconds approx. (a little bit more than
                       // time-horizon)
        LOG(WARNING) << "Logging mesh every (ns) = "
                     << timestamp - last_timestamp;
        CHECK(logger_);
        logger_->logMesh(map_points_3d, colors, polygons_mesh, timestamp,
                                                    ...
                                                    WidgetsMap*   widgets)
{
    ...
    drawLineFromPlaneToPoint(line_id, plane_n_x, plane_n_y, plane_n_z, plane_d,
                             point_x, point_y, point_z, widgets);
}
