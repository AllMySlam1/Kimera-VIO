...
LoopClosureDetector::LoopClosureDetector(
    ...
    bool                             log_output)
    : lcd_state_(LcdState::Bootstrap), lcd_params_(lcd_params),
      log_output_(log_output), orb_feature_detector_(), orb_feature_matcher_(),
      db_BoW_(nullptr), db_frames_(), timestamp_map_(),
      lcd_tp_wrapper_(nullptr), latest_bowvec_(), B_Pose_camLrect_(),
      stereo_camera_(stereo_camera), stereo_matcher_(nullptr), pgo_(nullptr),
      W_Pose_Blkf_estimates_(), logger_(nullptr)
{
    ...
    orb_feature_detector_ = cv::ORB::create(
        ...
        lcd_params_.edge_threshold_, lcd_params_.first_level_,
        lcd_params_.WTA_K_, lcd_params_.score_type_, lcd_params_.patch_sze_,
...
LcdOutput::UniquePtr LoopClosureDetector::spinOnce(const LcdInput& input)
{
    ...
    switch (lcd_state_) {
        case LcdState::Bootstrap: {
            ...
            // addOdometryFactorAndOptimize. Not doing it now bcs code assumes
            // that addOdometryFactorAndOptimize does that...
    ...
    if (input.frontend_output_->frontend_type_ == FrontendType::kStereoImu) {
        ...
        if (detectLoop(stereo_frontend_output->stereo_frame_lkf_,
                       &loop_result)) {
            LoopClosureFactor lc_factor(
            ...
            VLOG(1)
                << "LoopClosureDetector: LOOP CLOSURE detected from keyframe "
        ...
        LOG(ERROR) << "LoopClosureDetector: Not using StereoFrontend! Change "
                      "frontend.";
    }
    ...
    if (loop_result.isLoop()) {
        output_payload = VIO::make_unique<LcdOutput>(
            true, input.timestamp_, timestamp_map_.at(loop_result.query_id_),
            timestamp_map_.at(loop_result.match_id_), loop_result.match_id_,
...
/* ------------------------------------------------------------------------ */
FrameId LoopClosureDetector::processAndAddFrame(const StereoFrame& stereo_frame)
{
    ...
    db_frames_.push_back(LCDFrame(
        cp_stereo_frame.timestamp_, db_frames_.size(), cp_stereo_frame.id_,
        keypoints, cp_stereo_frame.keypoints_3d_, descriptors_vec,
        descriptors_mat, cp_stereo_frame.left_frame_.versors_,
                                     ...
                                     LoopResult*        result)
{
    ...
    if (query_result.empty()) {
        ...
        if (lcd_params_.use_nss_) {
            nss_factor =
                db_BoW_->getVocabulary()->score(bow_vec, latest_bowvec_);
        }
        else {
            ...
            if (query_result.empty()) {
                ...
                if (islands.empty()) {
                    ...
                    bool pass_temporal_constraint =
                        lcd_tp_wrapper_->checkTemporalConstraint(frame_id,
                                                                 best_island);
                    ...
                    if (!pass_temporal_constraint) {
                        ...
                        computeMatchedIndices(result->query_id_,
                                              result->match_id_, &i_query,
                                              &i_match, true);

                        bool pass_geometric_verification =
                            geometricVerificationCheck(
                                result->query_id_, result->match_id_,
                                &camMatch_T_camQuery_mono, &i_query, &i_match);

                        if (!pass_geometric_verification) {
                            result->status_ =
                                LCDStatus::FAILED_GEOM_VERIFICATION;
                        }
                        else {
                            gtsam::Pose3 bodyMatch_T_bodyQuery_stereo;
                            bool         pass_3d_pose_compute = recoverPose(
                                result->query_id_, result->match_id_,
                                camMatch_T_camQuery_mono,
                                &bodyMatch_T_bodyQuery_stereo, &i_query,
                                &i_match);

                            if (!pass_3d_pose_compute) {
                                result->status_ =
                                    LCDStatus::FAILED_POSE_RECOVERY;
                            }
                            else {
    ...
    switch (lcd_params_.geom_check_) {
        case GeomVerifOption::NISTER: {
            return geometricVerificationNister(
                query_id, match_id, camMatch_T_camQuery_mono,
                inlier_id_in_query_frame, inlier_id_in_match_frame);
        }
        ...
        default: {
            LOG(FATAL) << "LoopClosureDetector: Incorrect geom_check_ option: "
                       << std::to_string(
                              static_cast<int>(lcd_params_.geom_check_));
        }
    }
    ...
    std::vector<FrameId>* inlier_id_in_match_frame)
{
    ...
    switch (lcd_params_.pose_recovery_option_) {
        case PoseRecoveryOption::RANSAC_ARUN: {
            passed_pose_recovery = recoverPoseArun(
                query_id, match_id, &camMatch_T_camQuery_stereo,
                inlier_id_in_query_frame, inlier_id_in_match_frame);
            break;
        }
        case PoseRecoveryOption::GIVEN_ROT: {
            passed_pose_recovery = recoverPoseGivenRot(
                query_id, match_id, camMatch_T_camQuery_mono,
                &camMatch_T_camQuery_stereo, inlier_id_in_query_frame,
                inlier_id_in_match_frame);
            break;
        }
        default: {
            LOG(FATAL)
                << "LoopClosureDetector: Incorrect pose recovery option: "
                << std::to_string(
    ...
    if (lcd_params_.refine_pose_) {
        camMatch_T_camQuery_stereo =
            refinePoses(query_id, match_id, camMatch_T_camQuery_stereo,
                        *inlier_id_in_query_frame, *inlier_id_in_match_frame);
    }
    ...
    if (lcd_params_.use_mono_rot_ &&
        ...
        *bodyMatch_T_bodyQuery_stereo = gtsam::Pose3(
            bodyMatch_R_bodyQuery_stereo, bodyMatch_t_bodyQuery_stereo);
    }
    ...
    const std::vector<FrameId>& inlier_id_in_match_frame)
{
    ...
    nfg.add(gtsam::PriorFactor<gtsam::Pose3>(key_match,
    ...
    for (size_t i = 0; i < inlier_id_in_query_frame.size(); i++) {
        ...
        gtsam::StereoPoint2 sp_query_i(
            undistorted_rectified_left_query_keypoint.x,
        ...
        stereo_factor_i.add(sp_query_i, key_query,
        ...
        gtsam::StereoPoint2 sp_match_i(
            undistorted_rectified_left_match_keypoint.x,
            undistorted_rectified_right_match_keypoint.x,
            undistorted_rectified_left_match_keypoint.y);

        stereo_factor_i.add(sp_match_i, key_match,
                            stereo_camera_->getStereoCalib());
    ...
    try {
        values =
            gtsam::LevenbergMarquardtOptimizer(nfg, values, params).optimize();
    }
    ...
    bool           cut_matches) const
{
    ...
    cv::drawMatches(query_img, db_frames_.at(query_id).keypoints_, match_img,
                    db_frames_.at(match_id).keypoints_, good_matches,
                    img_matches, cv::Scalar(255, 0, 0), cv::Scalar(255, 0, 0));
    ...
    std::vector<FrameId>* inlier_id_in_match_frame)
{
    ...
    if (static_cast<int>(match_versors.size()) >=
        ...
        ransac.sac_model_ = std::make_shared<SacProblemMono>(
            adapter, SacProblemMono::Algorithm::NISTER,
        ...
        if (!ransac_success) {
            ...
            if (inlier_percentage >=
                lcd_params_.ransac_inlier_threshold_mono_) {
                if (ransac.iterations_ <
                    lcd_params_.max_ransac_iterations_mono_) {
                    opengv::transformation_t transformation =
                        ransac.model_coefficients_;
                    // Remove the outliers based on ransac.inliers (modify
                    // i_query and i_match AND pass the result out)
    ...
    if (!ransac_success) {
        ...
        if (inlier_percentage >= lcd_params_.ransac_inlier_threshold_stereo_) {
            if (ransac.iterations_ <
                ...
                // Remove the outliers based on ransac.inliers (modify i_query
                // and i_match AND pass the result out)
                inlier_id_in_query_frame->clear();
                inlier_id_in_match_frame->clear();
                for (const auto i : ransac.inliers_) {
                    inlier_id_in_query_frame->push_back(i_query[i]);
    ...
    std::vector<FrameId>* inlier_id_in_match_frame)
{
    ...
    const gtsam::Rot3& camMatch_R_camQuery =
        camMatch_T_camQuery_mono.rotation();
    ...
    if (n_matches > 0) {
        ...
        *camMatch_T_camQuery = gtsam::Pose3(camMatch_R_camQuery, scaled_t);
        ...
        inlier_id_in_match_frame->clear();
        for (size_t i = 0; i < n_matches; i++) {
            gtsam::Vector3 residual_error_vector = gtsam::Vector3(
                x_coord[i] - scaled_t.x(), y_coord[i] - scaled_t.y(),
    ...
    const OdometryFactor& factor)
{
    ...
    nfg.add(gtsam::BetweenFactor<gtsam::Pose3>(
        gtsam::Symbol(factor.cur_key_ - 1), gtsam::Symbol(factor.cur_key_),
