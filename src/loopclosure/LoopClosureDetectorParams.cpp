...
LoopClosureDetectorParams::LoopClosureDetectorParams(
    ...
    : PipelineParams("Loop Closure Parameters"), image_width_(image_width),
      image_height_(image_height), focal_length_(focal_length),
      principle_point_(principle_point),

      use_nss_(use_nss), alpha_(alpha),
      min_temporal_matches_(min_temporal_matches),
      recent_frames_window_(recent_frames_window),
      max_db_results_(max_db_results), min_nss_factor_(min_nss_factor),
      ...
      geom_check_(geom_check), min_correspondences_(min_correspondences),
      ...
      use_mono_rot_(use_mono_rot), refine_pose_(refine_pose),

      lowe_ratio_(lowe_ratio), matcher_type_(matcher_type),

      nfeatures_(nfeatures), scale_factor_(scale_factor), nlevels_(nlevels),
      edge_threshold_(edge_threshold), first_level_(first_level), WTA_K_(WTA_K),
      score_type_(score_type), patch_sze_(patch_sze),
...
bool LoopClosureDetectorParams::parseYAML(const std::string& filepath)
{
    ...
    yaml_parser.getYamlParam("min_matches_per_island",
    ...
    switch (geom_check_id) {
        ...
        default:
            throw std::runtime_error(
                "LCDparams parseYAML: wrong geom_check_id");
            break;
    }
    ...
    switch (score_type_id) {
        ...
        default:
            throw std::runtime_error(
                "LCDparams parseYAML: wrong score_type_id");
            break;
    }
...
void LoopClosureDetectorParams::print() const
{
    ...
    PipelineParams::print(
        out, "image_width_: ", image_width_, "image_height_: ", image_height_,
        "focal_length_: ", focal_length_,
        "principle_point_: ", principle_point_,

        "use_nss_: ", use_nss_, "alpha_: ", alpha_,
        "min_temporal_matches_: ", min_temporal_matches_,
        "recent_frames_window_: ", recent_frames_window_,
        "max_db_results_: ", max_db_results_,
        "max_db_results_: ", max_db_results_,
        "min_nss_factor_: ", min_nss_factor_,
        "min_matches_per_island_: ", min_matches_per_island_,
        "max_intraisland_gap_: ", max_intraisland_gap_,
        "max_nrFrames_between_islands_: ", max_nrFrames_between_islands_,

        "max_nrFrames_between_queries_: ", max_nrFrames_between_queries_,
        ...
        "use_mono_rot_:", use_mono_rot_, "refine_pose_:", refine_pose_,
        "lowe_ratio_: ", lowe_ratio_,
        "matcher_type_:", static_cast<unsigned int>(matcher_type_),

        "nfeatures_: ", nfeatures_, "scale_factor_: ", scale_factor_,
        "nlevels_: ", nlevels_, "edge_threshold_: ", edge_threshold_,
        "first_level_: ", first_level_, "WTA_K_: ", WTA_K_,
        "score_type_: ", static_cast<unsigned int>(score_type_),
        "patch_sze_: ", patch_sze_, "fast_threshold_: ", fast_threshold_,
