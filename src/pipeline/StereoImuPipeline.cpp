    ...
    : Pipeline(params), stereo_camera_(nullptr)
{
    ...
    data_provider_module_ = VIO::make_unique<StereoDataProviderModule>(
        &frontend_input_queue_, "Stereo Data Provider", parallel_run_,
    ...
    vio_frontend_module_ = VIO::make_unique<VisionImuFrontendModule>(
        &frontend_input_queue_, parallel_run_,
        VisionImuFrontendFactory::createFrontend(
            params.frontend_type_, params.imu_params_,
            gtsam::imuBias::ConstantBias(), params.frontend_params_,
            stereo_camera_, FLAGS_visualize ? &display_input_queue_ : nullptr,
            FLAGS_log_output));
        ...
        [&backend_input_queue](const FrontendOutputPacketBase::Ptr& output) {
            ...
            if (converted_output && converted_output->is_keyframe_) {
                ...
                backend_input_queue.push(VIO::make_unique<BackendInput>(
                    ...
                    converted_output->tracker_status_, converted_output->pim_,
                    converted_output->imu_acc_gyrs_,
                    converted_output->relative_pose_body_stereo_));
            }
            else {
                VLOG(5) << "Frontend did not output a keyframe, skipping "
                           "Backend input.";
            }
        });
    ...
    vio_backend_module_ = VIO::make_unique<VioBackendModule>(
        &backend_input_queue_, parallel_run_,
        BackendFactory::createBackend(
            ...
            stereo_camera_->getStereoCalib(), *backend_params_, imu_params_,
            backend_output_params, FLAGS_log_output));
    ...
    if (static_cast<VisualizationType>(FLAGS_viz_type) ==
        ...
        vio_frontend_module_->registerOutputCallback(
            [&mesher_module](const FrontendOutputPacketBase::Ptr& output) {
                StereoFrontendOutput::Ptr converted_output =
                    VIO::safeCast<FrontendOutputPacketBase,
                                  StereoFrontendOutput>(output);
    ...
    if (FLAGS_use_lcd) {
        lcd_module_ = VIO::make_unique<LcdModule>(
            parallel_run_,
            LcdFactory::createLcd(
                LoopClosureDetectorType::BoW, params.lcd_params_,
                stereo_camera_, params.frontend_params_.stereo_matching_params_,
                FLAGS_log_output));
        //! Register input callbacks
        vio_backend_module_->registerOutputCallback(
    ...
    if (FLAGS_visualize) {
        visualizer_module_ = VIO::make_unique<VisualizerModule>(
            ...
            &display_input_queue_, parallel_run_, FLAGS_use_lcd,
        ...
        display_module_ = VIO::make_unique<DisplayModule>(
            &display_input_queue_, nullptr, parallel_run_,
