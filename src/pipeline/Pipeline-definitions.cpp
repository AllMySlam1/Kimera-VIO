...
VioParams::VioParams(const std::string& params_folder_path,
    ...
    : PipelineParams("VioParams"),
      ...
      imu_params_(), camera_params_(), frontend_params_(),
      backend_params_(std::make_shared<BackendParams>()), lcd_params_(),
      display_params_(std::make_shared<DisplayParams>(DisplayType::kOpenCV)),
      frontend_type_(FrontendType::kStereoImu),
      backend_type_(BackendType::kStructuralRegularities), parallel_run_(true),
...
void VioParams::print() const
{
    ...
    imu_params_.print();
    for (const auto& cam : camera_params_)
        cam.print();
    ...
    LOG(INFO) << "Running VIO in "
              << (parallel_run_ ? "parallel" : "sequential") << " mode.";
}
