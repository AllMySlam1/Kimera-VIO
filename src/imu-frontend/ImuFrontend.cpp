...
// preintegration??
ImuFrontend::PimPtr
ImuFrontend::preintegrateImuMeasurements(const ImuStampS&  imu_stamps,
                                         const ImuAccGyrS& imu_accgyr)
{
    ...
    // This can be a feature instead of a bug, in the sense that the user can
    // then either integrate or not the last measurement (typically
    // fake/interpolated) measurement. Nevertheless the imu_stamps, should be
    // shifted one step back I would say.
    ...
    // pim. All of this is to deal with gtsam's idiosyncracies with base
    // classes.
...
/* -------------------------------------------------------------------------- */
gtsam::Rot3
ImuFrontend::preintegrateGyroMeasurements(const ImuStampS&  imu_stamps,
                                          const ImuAccGyrS& imu_accgyr)
{
    CHECK(imu_stamps.cols() >= 2) << "No Imu data found.";
    CHECK(imu_accgyr.cols() >= 2) << "No Imu data found.";
...
ImuFrontend::convertVioImuParamsToGtsam(const ImuParams& imu_params)
{
    CHECK_GT(imu_params.acc_noise_density_, 0.0);
    CHECK_GT(imu_params.gyro_noise_density_, 0.0);
    CHECK_GT(imu_params.imu_integration_sigma_, 0.0);
...
{
    boost::shared_ptr<gtsam::PreintegratedCombinedMeasurements::Params>
        combined_imu_params = boost::make_shared<
    ...
    if (gtsam_imu_params.omegaCoriolis) {
        combined_imu_params->setOmegaCoriolis(
            *gtsam_imu_params.getOmegaCoriolis());
    }
...
ImuFrontend::generateRegularImuParams(const ImuParams& imu_params)
{
    ...
    if (gtsam_imu_params.omegaCoriolis) {
        regular_imu_params->setOmegaCoriolis(
            *gtsam_imu_params.getOmegaCoriolis());
    }
