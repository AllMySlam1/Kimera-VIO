...
RgbdDataProviderModule::InputUniquePtr RgbdDataProviderModule::getInputPacket()
{
    ...
    if (!PIO::syncQueue(timestamp, &depth_frame_queue_, &depth_frame_payload)) {
        // Dropping this message because of missing left/depth stereo synced
        // frames.
        LOG(ERROR) << "Missing depth frame for left frame with id "
                   << left_frame_id << ", dropping this frame.";
        return nullptr;
    }
    ...
    if (!shutdown_) {
        ...
        vio_pipeline_callback_(VIO::make_unique<RgbdImuSyncPacket>(
            timestamp,
            VIO::make_unique<RgbdFrame>(left_frame_id, timestamp,
