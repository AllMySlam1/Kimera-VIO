...
EurocDataProvider::EurocDataProvider(const std::string& dataset_path,
    ...
    : DataProviderInterface(), dataset_path_(dataset_path),
      current_k_(std::numeric_limits<FrameId>::max()), initial_k_(initial_k),
      final_k_(final_k), vio_params_(vio_params), imu_measurements_(),
...
bool EurocDataProvider::spin()
{
    if (dataset_parsed_) {
        if (!is_imu_data_sent_) {
            // First, send all the IMU data. The flag is to avoid sending it
            // several times if we are running in sequential mode.
            if (imu_single_callback_) {
                sendImuData();
            }
            else {
                LOG(ERROR)
                    << "Imu callback not registered! Not sending IMU data.";
            }
        ...
        CHECK_GT(final_k_, initial_k_);
        // We log only the first one, because we may be running in sequential
        // mode.
...
bool EurocDataProvider::spinOnce()
{
    ...
    if (available_left_img && available_right_img) {
        ...
        left_frame_callback_(
            VIO::make_unique<Frame>(current_k_, timestamp_frame_k,
        ...
        LOG(ERROR)
            << "Missing left/right stereo pair, proceeding to the next one.";
    }
                                     ...
                                     const std::string& imuName)
{
    ...
    while (std::getline(fin, line)) {
        ...
        if (previous_timestamp != -1) {
            ...
            imu_rate_maxMismatch =
                std::max(imu_rate_maxMismatch, deltaMismatch);
            deltaCount += 1u;
        }
                                    ...
                                    const std::string& gt_sensor_name)
{
    ...
    while (std::getline(fin, line)) {
        ...
        if (previous_timestamp == -1) {
            ...
            sumOfDelta += (timestamp - previous_timestamp);
            // std::cout << "time diff (sec): " << (timestamp -
            // previous_timestamp) * 1e-9 << std::endl;
            delta_count += 1u;
            previous_timestamp = timestamp;
        }
        ...
        if (std::fabs(q(0) + gt_data_raw[3]) <
            std::fabs(q(0) - gt_data_raw[3])) {
            q = -q;
        }

        LOG_IF(FATAL, (fabs(q(0) - gt_data_raw[3]) > 1e-3) ||
...
bool EurocDataProvider::parseDataset()
{
    ...
    if (VLOG_IS_ON(1))
        left_cam_image_list.print();
    parseCameraData(kRightCamName, &right_cam_image_list);
    if (VLOG_IS_ON(1))
        right_cam_image_list.print();
    ...
    // WARNING Use [x] not .at() because we are adding entries that do not
    // exist.
    ...
    if (logger_) {
        if (is_gt_available_) {
            ...
            LOG(ERROR)
                << "Requested ground-truth data logging but no ground-truth "
                   "data available.";
        }
    }
    ...
    std::map<std::string, CameraImageLists>* camera_image_lists) const
{
    ...
    auto&       left_img_lists =
        camera_image_lists->at(camera_names.at(0)).img_lists_;
    auto& right_img_lists =
        camera_image_lists->at(camera_names.at(1)).img_lists_;
    ...
    const CameraParams&               left_cam_info) const
{
    ...
    for (size_t i = 0; i < left_img_lists.size(); i++) {
        if (i > 0) {
            ...
            frame_rate_maxMismatch =
                std::max(frame_rate_maxMismatch, deltaMismatch);
        }
...
/* -------------------------------------------------------------------------- */
bool EurocDataProvider::isGroundTruthAvailable(const Timestamp& timestamp) const
{
...
/* -------------------------------------------------------------------------- */
VioNavState
EurocDataProvider::getGroundTruthState(const Timestamp& timestamp) const
{
    const auto& it_low = gt_data_.map_to_gt_.equal_range(timestamp)
                             .first;  // closest, non-lesser
    ...
    const gtsam::Vector3&            init_gravity)
{
    ...
    const InitializationPerformance init_performance(
        timestamps.at(1), init_n_frames, avg_relativeRotError,
        avg_relativeTranError, init_nav_state, init_gravity, gt_state,
        gt_gravity);
    // Log performance
...
/* -------------------------------------------------------------------------- */
size_t
EurocDataProvider::getNumImagesForCamera(const std::string& camera_name) const
{
