...
/* -------------------------------------------------------------------------- */
Histogram::Histogram()
    : n_images_(0), channels_(nullptr), mask_(cv::Mat()), dims_(0),
      hist_size_(nullptr), ranges_(nullptr), uniform_(false), accumulate_(false)
{
}
...
Histogram::Histogram(int                                      n_images,
                     ...
                     bool                                     uniform,
                     bool                                     accumulate)
    : n_images_(n_images), channels_(nullptr), mask_(mask), dims_(dims),
      hist_size_(nullptr), ranges_(nullptr), uniform_(uniform),
                           ...
                           bool                         display_image) const
{
    ...
    for (size_t i = 0; i < peaks.size(); i++) {
        cv::line(*hist_image,
                 cv::Point(bin_w * peaks[i].pos_, hist_image->rows),
        ...
        case 1:
            for (int i = 0; i < hist_image.cols; i++) {
                ...
                cv::line(hist_image, cv::Point(bin_w * x, y),
                         cv::Point(bin_w * x, y - cvRound(hist->at<float>(i))),
                         color);
            }
            break;
        case 2:
            for (int i = 1; i < hist_size; ++i) {
                ...
                cv::Point pt4 = cv::Point(
                    bin_w * (i - 1), hist_h - cvRound(hist->at<float>(i - 1)));
...
/* -------------------------------------------------------------------------- */
Histogram::PeakInfo
Histogram::peakInfo(int pos, int left_size, int right_size, float support) const
{
    CHECK_EQ(dims_, 1) << "This function is meant for 1D histograms.";
    PeakInfo output;
    output.pos_        = pos;
                                                      ...
                                                      int window_size) const
{
    ...
    while (i < src2.cols - size) {
        ...
        if ((pre_state == 1 && cur_state == 2) ||
            ...
            PeakInfo peak_info =
                peakInfo(max_pos, up_hill.size(), down_hill.size(),
    ...
    bool            log_histogram) const
{
    ...
    if (log_histogram) {
        cv::FileStorage file("histogram_" + std::to_string(dims_) +
                                 "_smoothed.yaml",
                             cv::FileStorage::WRITE);
    ...
    if (display_histogram) {
        ...
        cv::Mat hist_img =
            drawHistogram1D(&src, 400, 1024,
                            hist_size,  // WARNING: this number has to coincide
                                        // with nr of bins...
        ...
        drawPeaks1D(&hist_img, output, hist_size, peak_color,
                    display_histogram);
    }
    return output;
}
                                  ...
                                  bool            log_histogram) const
{
    ...
    int nr_of_maximums =
        imgRegionalMax(&histogram_smoothed, number_of_local_max, min_support,
                       min_dist_btw_loc_max, peaks);
    if (nr_of_maximums > 0) {
        ...
        for (int t = 0; t < hist_size_[0]; t++) {
            for (int d = 0; d < hist_size_[1]; d++) {
                ...
                cv::rectangle(*img_output,
                              cv::Point(t * scale_theta, d * scale_distance),
                              ...
                              std::vector<PeakInfo2D>* locations) const
{
    ...
    for (int i = 0; i < number_of_local_max; i++) {
        ...
        if (max_val > min_support) {
            ...
            int    col = location.x;
            double x_value =
                (row * (ranges_[0][1] - ranges_[0][0]) / hist_size_[0]) +
                ranges_[0][0];
            double y_value =
                (col * (ranges_[1][1] - ranges_[1][0]) / hist_size_[1]) +
                ranges_[1][0];
            locations->push_back(PeakInfo2D(cv::Point(location.y, location.x),
                                            max_val, x_value, y_value));
            int r0 =
                (row - min_dist_btw_loc_max > -1 ? row - min_dist_btw_loc_max :
                                                   0);
            int r1 = (row + min_dist_btw_loc_max < input->rows ?
                          row + min_dist_btw_loc_max :
                          input->rows - 1);
            int c0 =
                (col - min_dist_btw_loc_max > -1 ? col - min_dist_btw_loc_max :
                                                   0);
            int c1 = (col + min_dist_btw_loc_max < input->cols ?
                          col + min_dist_btw_loc_max :
                          input->cols - 1);
            for (int r = r0; r <= r1; r++) {
                for (int c = c0; c <= c1; c++) {
                    if (std::sqrt(std::pow(r - row, 2) +
                                  std::pow(c - col, 2)) <=
