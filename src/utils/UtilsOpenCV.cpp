...
gtsam::Pose3
UtilsOpenCV::poseVectorToGtsamPose3(const std::vector<double>& vector_pose)
{
...
gtsam::Pose3 UtilsOpenCV::cvMatsToGtsamPose3(const cv::Mat& R, const cv::Mat& T)
{
...
gtsam::Pose3
UtilsOpenCV::openGvTfToGtsamPose3(const opengv::transformation_t& RT)
{
                                 ...
                                 const bool&               useHarrisDetector)
{
    ...
    try {
        // Extract the corners
        cv::goodFeaturesToTrack(img, *corners, max_n_corners, qualityLevel,
                                 ...
                                 bool                  round)
{
    ...
    if (std::fabs(1 - c) < 1e-3) {
        ...
        R = gtsam::Rot3::AlignPair(cross_product, globalGravityDir,
                                   localGravityDir);
    }
...
gtsam::Unit3 UtilsOpenCV::RoundUnit3(const gtsam::Unit3& x)
{
    ...
    for (size_t i = 0; i < 3; i++) {
        if (fabs(fabs(x_vect(i)) - max_x) < 1e-4) {  // found max element
            ...
            break;  // tie breaker for the case in which multiple elements
                    // attain the max
        }
    }
                                     ...
                                     const int&              rem_id)
{
    ...
    for (size_t i = 0u; i < image_points.size(); i++) {
        ...
        if (point_ids.size() == image_points.size()) {
            // We also have text
            cv::putText(img, std::to_string(point_ids[i] % rem_id),
                        image_points[i] + text_offset, CV_FONT_HERSHEY_COMPLEX,
                        0.5, color);
        }
    }
}
    ...
    for (size_t i = 0; i < imagePoints.size(); i++) {
        ...
        if (pointIds.size() == imagePoints.size())  // we also have text
            cv::putText(img, std::to_string(pointIds[i] % remId),
                        imagePoints[i] + textOffset, CV_FONT_HERSHEY_COMPLEX,
                        0.5, color);
    }
}
/* -------------------------------------------------------------------------- */
                                          ...
                                          const cv::Mat& right_img)
{
    ...
    cv::Mat right(dual_img,
                  cv::Rect(left_img_size.width, 0, right_img_size.width,
...
cv::Mat
UtilsOpenCV::DrawCornersMatches(const cv::Mat&           img1,
                                ...
                                const bool&              random_color)
{
    ...
    return DrawCornersMatches(img1, keypoints_1, img2, keypoints_2, matches,
                              random_color);
}
                                 ...
                                 const std::vector<double>& circle_sizes)
{
    ...
    return UtilsOpenCV::DrawCircles(img, valid_img_points, circle_colors,
                                    circle_sizes);
}

cv::Scalar
UtilsOpenCV::getColorFromKeypointStatus(const KeypointStatus& kpt_status)
{
    ...
    for (size_t i = 0u; i < image_points.size(); i++) {
        ...
        if (display_with_text && circle_sizes.size() == image_points.size() &&
            ...
            cv::putText(
                ...
                image_points[i] + text_offset, CV_FONT_HERSHEY_COMPLEX, 0.4,
                circle_color);
        }
    }
    return img_color;
                                         ...
                                         const float      intensityThreshold)
{
    ...
    for (int r = topLeft_y; r < botRight_y; r++) {
        ...
        // check triangle 01:
        if (y0 !=
            y1) {  // in this case segment is horizontal and we can skip it
            double lambda01 = double(r - y1) / double(y0 - y1);
            if (lambda01 >= 0 &&
                lambda01 <= 1) {  // intersection belongs to segment
                int x = std::round((lambda01) * double(x0) +
                                   (1 - lambda01) * double(x1));
                min_x =
                    std::min(min_x, x);  // try to expand segment to the left
                max_x =
                    std::max(max_x, x);  // try to expand segment to the right
            }
        }

        // check triangle 12:
        if (y1 !=
            y2) {  // in this case segment is horizontal and we can skip it
            double lambda12 = double(r - y2) / double(y1 - y2);
            if (lambda12 >= 0 &&
                lambda12 <= 1) {  // intersection belongs to segment
        ...
        // check triangle 20:
        if (y2 !=
            y0) {  // in this case segment is horizontal and we can skip it
            double lambda20 = double(r - y0) / double(y2 - y0);
            if (lambda20 >= 0 &&
                lambda20 <= 1) {  // intersection belongs to segment
                                        ...
                                        const bool         check_opened)
{
    ...
    if (check_opened) {
        CHECK(fs->isOpened())
            << "Cannot open file: " << file_path << " (remember "
