...
namespace utils {
    ...
    ThreadsafeImuBuffer::isDataAvailableUpToImpl(
        ...
        const Timestamp& timestamp_ns_to) const
    {
        ...
        if (buffer_.getNewestValue(&value) &&
            value.timestamp_ < timestamp_ns_to) {
            // This is triggered if the timestamp_ns_to requested exceeds the
            // newest IMU measurement, meaning that there is data still to
            // arrive to reach the requested point in time.
            return QueryResult::kDataNotYetAvailable;
        }

        if (buffer_.getOldestValue(&value) &&
            timestamp_ns_from < value.timestamp_) {
            // This is triggered if the user requests data previous to the
            // oldest IMU measurement present in the buffer, meaning that there
            // is missing data from the timestamp_ns_from requested to the
        ...
        bool             get_lower_bound)
    {
        ...
        // Copy the data with timestamp_up_to <= timestamps_buffer from the
        // buffer.
        ...
        if (between_values.empty()) {
            LOG(WARNING)
                << "No IMU measurements available strictly between time "
        ...
        ImuAccGyrS*      imu_measurements)
    {
        ...
        interpolateValueAtTimestamp(timestamp_ns_to,
                                    &interpolated_upper_border);
        ...
        ImuAccGyrS*      imu_measurements)
    {
        ...
        interpolateValueAtTimestamp(timestamp_ns_from,
                                    &interpolated_lower_border);
        // Interpolate upper border.
        ImuAccGyr interpolated_upper_border;
        interpolateValueAtTimestamp(timestamp_ns_to,
                                    &interpolated_upper_border);
        ...
        DCHECK_EQ(imu_timestamps->rows(), 1);
        DCHECK_EQ(imu_measurements->rows(), 6);
        // The first and last measurements will correspond to the interpolated
        // data.
        ...
        // Add measurements.
        imu_timestamps->middleCols(1, imu_timestamps_tmp.cols()) =
            imu_timestamps_tmp;
        ...
        ImuAccGyr*       interpolated_imu_measurement)
    {
        ...
        CHECK(buffer_.getValueAtOrBeforeTime(
            ...
            << "The IMU buffer seems not to contain measurements at or before "
               "time: "
            << timestamp_ns;
        ...
        CHECK(buffer_.getValueAtOrAfterTime(
            ...
            << "The IMU buffer seems not to contain measurements at or after "
               "time: "
            << timestamp_ns;
        CHECK_EQ(post_border_timestamp, post_border_value.timestamp_);
        linearInterpolate(
            pre_border_value.timestamp_, pre_border_value.acc_gyr_,
        ...
        ImuAccGyrS*      imu_measurements)
    {
        ...
        QueryResult query_result;
        {
            ...
            while ((query_result = isDataAvailableUpToImpl(timestamp_ns_from,
                                                           timestamp_ns_to)) !=
                ...
                // Check if we hit the max. time allowed to wait for the
                // required data.
