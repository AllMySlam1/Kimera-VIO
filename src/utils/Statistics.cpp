...
namespace utils {
    ...
    size_t Statistics::GetHandle(std::string const& tag)
    {
        ...
        if (i == Instance().tag_map_.end()) {
            ...
            // Track the maximum tag length to help printing a table of values
            // later.
    ...
    void Statistics::WriteToYamlFile(const std::string& path)
    {
        ...
        if (!output_file) {
            LOG(ERROR) << "Could not write statistics: Unable to open file: "
                       << path;
            return;
        }
        ...
        for (const map_t::value_type& tag : tag_map) {
            ...
            if (GetNumSamples(index) > 0) {
                ...
                // We do not want colons or hashes in a label, as they might
                // interfere with reading the yaml later.
