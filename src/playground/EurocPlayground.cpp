...
EurocPlayground::EurocPlayground(const std::string& dataset_path,
    ...
    : dataset_path_(dataset_path), subsample_n(subsample_n),
      vio_params_(params_path), feature_detector_(nullptr),
      euroc_data_provider_(nullptr), visualizer_3d_(nullptr),
      display_module_(nullptr), display_input_queue_("display_input_queue"),
      imu_data_(), left_frame_queue_("left_frame_queue"),
      right_frame_queue_("right_frame_queue"), stereo_camera_(nullptr),
      stereo_matcher_(nullptr)
{
    ...
    while (euroc_data_provider_->spin()) {};  // Fill queues.
    ...
    display_module_ = VIO::make_unique<DisplayModule>(
        &display_input_queue_, nullptr, vio_params_.parallel_run_,
                                      ...
                                      const bool& viz_pointcloud)
{
    ...
    if (viz_img_in_frustum) {
        ...
        while (left_frame_queue_.pop(left_frame) &&
            ...
            StereoFrame stereo_frame(left_frame->id_, left_frame->timestamp_,
            ...
            if ((left_frame->id_ % subsample_n) == 0u) {
                ...
                cv::resize(left_frame->img_, smaller_img, cv::Size(),
                ...
                visualizer_3d_->visualizePoseWithImgInFrustum(
                    smaller_img, left_cam_rect_pose, &output->widgets_,
                    "Camera id: " + std::to_string(left_frame->id_), resized_K);

                // Compute depth map just to see.
                cv::Mat disp_img = cv::Mat(left_frame->img_.rows,
                                           left_frame->img_.cols, CV_32F);
                ...
                stereo_matcher_->denseStereoReconstruction(
                    ...
                    stereo_frame.getRightImgRectified(), &disp_img);
                cv::Mat disp_viz_img;
                UtilsOpenCV::getDisparityVis(disp_img, disp_viz_img, 1.0);
                cv::imshow("Left Image Rectified",
                           stereo_frame.getLeftImgRectified());
                ...
                mesh_packet_.world_pose_body_ =
                    euroc_data_provider_->getGroundTruthPose(
                ...
                for (int32_t v = 0; v < depth_map.rows; ++v) {
                    for (int32_t u = 0; u < depth_map.cols; ++u) {
                        ...
                        if (isValidPoint(xyz)) {
                            ...
                            // We recolor the depth map for disambiguation of
                            // original viewpoint
                            valid_colors.push_back(cv::Vec3b(
                                color_counter == 0u ? grey_value : 0u,
                ...
                visualizer_3d_->visualizePointCloud(
                    valid_depth, &output->widgets_, left_cam_rect_pose,
                    valid_colors);
            }
        }
    }
