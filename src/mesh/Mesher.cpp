...
Mesher::Mesher(const MesherParams& mesher_params, const bool& serialize_meshes)
    : mesher_params_(mesher_params), mesh_2d_(), mesh_3d_(),
      mesher_logger_(nullptr), serialize_meshes_(serialize_meshes)
{
...
MesherOutput::UniquePtr Mesher::spinOnce(const MesherInput& input)
{
    ...
    updateMesh3D(input,
                 // TODO REMOVE THIS FLAG MAKE MESH_2D Optional!
                 FLAGS_return_mesh_2d ? &(mesher_output_payload->mesh_2d_) :
                                        nullptr,
    ...
    const gtsam::Pose3& leftCameraPose) const
{
    ...
    // get 3D points
    gtsam::Point3 p1_C =
        gtsam::Point3(static_cast<double>(p1.x), static_cast<double>(p1.y),
                      static_cast<double>(p1.z));
    points.push_back(leftCameraPose.transformTo(
        p1_C));  // checks elongation in *camera frame*

    gtsam::Point3 p2_C =
        gtsam::Point3(static_cast<double>(p2.x), static_cast<double>(p2.y),
                      static_cast<double>(p2.z));
    points.push_back(leftCameraPose.transformTo(
        p2_C));  // checks elongation in *camera frame*

    gtsam::Point3 p3_C =
        gtsam::Point3(static_cast<double>(p3.x), static_cast<double>(p3.y),
                      static_cast<double>(p3.z));
    points.push_back(leftCameraPose.transformTo(
        p3_C));  // checks elongation in *camera frame*

    return UtilsGeometry::getRatioBetweenTangentialAndRadialDisplacement(
        points);
}
                                   ...
                                   double maxTriangleSide)
{
    ...
    for (size_t i = 0; i < mesh_3d_.getNumberOfPolygons(); i++) {
        CHECK(mesh_3d_.getPolygon(i, &polygon))
            << "Could not retrieve polygon.";
    ...
    const double&          max_triangle_side) const
{
    ...
    if (min_elongation_ratio > 0.0) {
        ratioTangentialRadial_i =
            getRatioBetweenTangentialAndRadialDisplacement(p1, p2, p3,
                                                           left_camera_pose);
    }
    ...
    Mesh2D*                       mesh_2d)
{
    ...
    populate3dMesh(mesh_2d_pixels, points_with_id_map, keypoints, landmarks,
                   left_cam_pose, min_ratio_largest_smallest_side,
                   min_elongation_ratio, max_triangle_side, mesh_2d);
    ...
    updatePolygonMeshToTimeHorizon(
        points_with_id_map, left_cam_pose, min_ratio_largest_smallest_side,
                            ...
                            Mesh2D* mesh_2d)
{
    ...
    for (size_t i = 0u; i < mesh_2d_pixels.size(); i++) {
        ...
        for (size_t j = 0u; j < triangle_2d.rows / 2u; j++) {
            // Extract pixel.
            const cv::Point2f pixel(triangle_2d[j * 2u],
                                    triangle_2d[j * 2u + 1u]);
            ...
            if (lmk_it != points_with_id_map.end()) {
                ...
                // Add landmark as one of the vertices of the current polygon in
                // 3D.
                ...
                if (j == loop_end) {
                    ...
                    if (!isBadTriangle(polygon, left_cam_pose,
                                       min_ratio_largest_smallest_side,
                                       min_elongation_ratio,
                                       max_triangle_side)) {
                        // Save the valid triangular polygon, since it has all
                        // vertices in points_with_id_map.
                ...
                // Do not save current polygon, since it has at least one vertex
                // that is not in points_with_id_map.
    ...
    const bool&            reduce_mesh_to_time_horizon)
{
    ...
    for (size_t i = 0; i < mesh_3d_.getNumberOfPolygons(); i++) {
        ...
        for (Mesh3D::VertexType& vertex : polygon) {
            const auto& point_with_id_it =
                points_with_id_map.find(vertex.getLmkId());
            if (point_with_id_it == end) {
                ...
                // addPolygonToMesh only updates the positions of the vertices
                // in the visible frame.
                vertex.setVertexPosition(Vertex3D(
                    point_with_id_it->second.x(), point_with_id_it->second.y(),
...
void Mesher::calculateNormals(std::vector<cv::Point3f>* normals)
{
    ...
    // TODO there are far too many loops over the total number of Polygon
    // faces... Should put them all in the same loop!
    Mesh3D::Polygon polygon;
    for (size_t i = 0; i < mesh_3d_.getNumberOfPolygons(); i++) {
        CHECK(mesh_3d_.getPolygon(i, &polygon))
            << "Could not retrieve polygon.";
...
bool Mesher::isPointAtDistanceFromPlane(const Vertex3D&    point,
                                        const double&      plane_distance,
                                        const cv::Point3f& plane_normal,
                                        const double& distance_tolerance) const
{
                                   ...
                                   const PointsWithIdMap& points_with_id_vio)
{
    ...
    if (new_non_associated_planes.size() > 0) {
        ...
        // Very unefficient.
        VLOG(10)
            << "Starting update plane lmk ids for new non-associated planes.";
        updatePlanesLmkIdsFromMesh(
            ...
            points_with_id_vio);
        VLOG(10)
            << "Finished update plane lmk ids for new non-associated planes.";
        ...
        VLOG(10) << "Avoid extra loop over mesh, since there are no new "
                    "non-associated"
    ...
    const double&          normal_tolerance_walls)
{
    ...
    for (size_t i = 0; i < mesh_3d_.getNumberOfPolygons(); i++) {
        ...
        if (calculateNormal(p1, p2, p3, &triangle_normal)) {
            ////////////////////////// Update seed planes
            ///////////////////////////////
            // Update seed_planes lmk_ids field with ids of vertices of polygon
            ...
            /// Values for Z
            /// Histogram.///////////////////////////////////////////////
            // Collect z values of vertices of polygon which is not already on a
            // plane and which has the normal aligned with the vertical
            // direction so that we can build an histogram.
            ...
            if ((FLAGS_only_use_non_clustered_points ? !is_polygon_on_a_plane :
                ...
                // We have a triangle with a normal aligned with gravity, which
                // is not already clustered in a plane. Store z components to
                // build histogram.
                // TODO instead of storing z_components, use the accumulate flag
                // in calcHist and add them straight.
                ...
                /// Values for walls
                /// Histogram./////////////////////////////////////////
                // WARNING if we do not normalize, we'll have two peaks for the
                // same plane, no? Store theta.
                ...
                // NORMALIZE if a theta is positive and distance negative, it is
                // the same as if theta is 180 deg from it and distance
                // positive...
            }
        }
    }
    ...
    for (Plane& seed_plane : *seed_planes) {
        ...
        if (isNormalAroundAxis(seed_plane.normal_, triangle_normal,
            ...
            isPolygonAtDistanceFromPlane(polygon, seed_plane.distance_,
                                         seed_plane.normal_,
                                         distance_tolerance)) {
            // I guess we can comment out the check below since it can happen
            // that a poygon is segmented in two very close planes? Better we
            // enable it again! CHECK(!is_polygon_on_a_plane) << "Polygon was
            // already in a plane,"
                          ...
                          const cv::Mat&      walls)
{
    ...
    for (const Histogram::PeakInfo2D& peak : peaks2) {
        ...
        cv::Point3f plane_normal(std::cos(plane_theta), std::sin(plane_theta),
                                 0);
                                     ...
                                     const cv::Mat&       z_components)
{
    ...
    static const cv::Size            kernel_size(1,
                                      FLAGS_z_histogram_gaussian_kernel_size);
    ...
    for (std::vector<Histogram::PeakInfo>::iterator peak_it = peaks.begin();
        ...
        // Remove duplicates, and, for peaks that are too close, take the one
        // with maximum support. Assuming repeated peaks are ordered...
        if (i > 0 && *peak_it == peaks.at(i - 1)) {
            // Repeated element, delete it.
            LOG(WARNING) << "Deleting repeated peak for peak # " << i
                         << " in bin " << peak_it->pos_;
            ...
            // Not enough separation between planes, delete the one with less
            // support.
            if (previous_peak_it->support_ < peak_it->support_) {
                // Delete previous_peak.
                LOG(WARNING)
                    << "Deleting peak in bin " << previous_peak_it->pos_;
                // Iterators, pointers and references pointing to position (or
                // first) and beyond are invalidated, with all iterators,
                // pointers and references to elements before position (or
                // first) are guaranteed to keep referring to the same elements
    ...
    for (int peak_nr = 0;
        ...
        if (it != peaks.end()) {
            ...
            // WARNING we are not giving lmk ids to this plane!
            // We should either completely customize the histogram calc to pass
            // lmk ids or do another loop over the mesh to cluster new
            // triangles.
            const gtsam::Symbol  plane_symbol('P', *plane_id);
            static constexpr int cluster_id =
                             ...
                             const double&             distance_tolerance) const
{
    ...
    if (planes.size() == 0) {
        ...
        *non_associated_planes = segmented_planes;
        ...
        // Both the ones that could be associated, in which case only the
        // landmark ids are updated (symbol, norm, distance remain the same).
        // WARNING maybe we should do the Union of both the lmk ids of the new
        // plane and the old plane........................ And the ones that
        // could not be associated, in which case they are added as new planes,
        // with a new symbol.
        ...
        for (const Plane& segmented_plane : segmented_planes) {
            ...
            for (const Plane& plane_backend : planes) {
                ...
                if (plane_backend.geometricEqual(segmented_plane,
                    ...
                    uint64_t backend_plane_index =
                        plane_backend.getPlaneSymbol().index();
                    ...
                    if (std::find(associated_plane_ids.begin(),
                        ...
                        // Update plane.
                        // WARNING maybe do the union between previous and
                        // current lmk_ids? Or just don't do anything, cause the
                        // plane should be there with its own lmk ids already...
                        // Actually YES DO IT, so we can spare one loop over the
                        // mesh! the first one ! (aka go back to the so-called
                        // naive implementation! Not entirely true, since we
                        // still need to loop over the mesh to get the points
                        // for extracting new planes... plane_backend.lmk_ids_ =
                        // segmented_plane.lmk_ids_; WARNING TODO should we also
                        // update the normal & distance?? Acknowledge that we
                        // have an association.
                        ...
                        // Continue, to see if we can associate the current
                        // segmented plane to another Backend plane.
                        LOG(ERROR)
                        ...
                        if (FLAGS_do_double_association) {
                            LOG(ERROR) << "Doing double plane association of "
                                          "Backend plane.";
                            is_segmented_plane_associated = true;
                            break;
                        }
                        else {
                            LOG(ERROR) << "Avoiding double plane association "
                                          "of Backend plane. "
                                       << "Searching instead for another "
                                          "possible Backend plane for "
                                          "this"
                                          " segmented plane.";
                            continue;
                        }
                    ...
                    VLOG(0)
                        ...
                        << gtsam::DefaultKeyFormatter(
                               plane_backend.getPlaneSymbol().key())
                        << " from Backend not associated to new segmented "
                           "plane "
                        << gtsam::DefaultKeyFormatter(
            ...
            if (!is_segmented_plane_associated) {
                // The segmented plane could not be associated to any existing
                // plane in the Backend... Add it as a new plane.
                VLOG(0) << "Add plane with id "
                        << gtsam::DefaultKeyFormatter(
                               segmented_plane.getPlaneSymbol())
                        << " as a new plane for the Backend.";
                // WARNING by pushing we are also associating segmented planes
                // between them, do we want this? Maybe yes because we have some
                // repeated segmented planes that are very similar, but then
                // what's up with the lmk ids, which should we keep?
        ...
        // Which is not necessarily good, because the segmenter might differ
        // from Backend and we will not do expectation maximization anymore...
        // plus it makes the visualizer not work...
        //*planes = planes_tmp;
    }
}
                          ...
                          std::vector<cv::Vec6f>*            mesh_2d_for_viz)
{
    ...
    if (FLAGS_add_extra_lmks_from_stereo) {
        ...
        appendNonVioStereoPoints(landmarks, keypoints_status, keypoints_3d,
                                 left_camera_pose, &points_with_id_stereo);
    ...
    createMesh2dVIO(&mesh_2d_pixels, landmarks, keypoints_status, keypoints,
                    mesher_params_.img_size_, *points_with_id_all);
    if (mesh_2d_for_viz)
        *mesh_2d_for_viz = mesh_2d_pixels;
    LOG_IF(WARNING, mesh_2d_pixels.size() == 0) << "2D Mesh is empty!";

    populate3dMeshTimeHorizon(
        mesh_2d_pixels, *points_with_id_all, keypoints, landmarks,
        left_camera_pose, FLAGS_min_ratio_btw_largest_smallest_side,
        FLAGS_min_elongation_ratio, FLAGS_max_triangle_side, mesh_2d);

    // Calculate 3d mesh normals.
    if (FLAGS_compute_per_vertex_normals)
        mesh_3d_.computePerVertexNormals();
                          ...
                          std::vector<cv::Vec6f>* mesh_2d_for_viz)
{
    ...
    updateMesh3D(mesher_payload.backend_output_->landmarks_with_id_map_,
                 stereo_frame.left_frame_.keypoints_, right_keypoint_status,
    ...
    PointsWithIdMap*                   points_with_id_stereo) const
{
    ...
    for (size_t i = 0; i < landmarks.size(); i++) {
        ...
        if (keypoints_status.at(i) == KeypointStatus::VALID &&
            ...
            // Use insert() instead of [] operator, to make sure that if there
            // is already a point with the same lmk_id, we do not override it.
            points_with_id_stereo->insert(
                std::make_pair(landmark_id, p_i_global));
        }
    }
}
    ...
    for (const Mesh3D::VertexType& vertex : polygon) {
        ...
        if (it == lmk_ids->end()) {
            ...
            if (FLAGS_add_extra_lmks_from_stereo) {
                ...
                // We are assuming lmk_ids has already only points in
                // time-horizon, so no need to check them as well.
    ...
    const PointsWithIdMap&             pointsWithIdVIO)
{
    ...
    for (const auto& point_with_id : pointsWithIdVIO) {
        for (size_t j = 0u; j < landmarks.size(); j++) {
            // If we are seeing a VIO point in left and right frame, add to
            // keypoints to generate the mesh in 2D.
...
std::vector<cv::Vec6f>
Mesher::createMesh2dImpl(const cv::Size&    img_size,
                         ...
                         MeshIndices*       vtx_indices)
{
    ...
    try {
        ...
        LOG(ERROR)
            << "CreateMesh2D: subdiv.insert error (2). "
            ...
            << "Keypoints to triangulate: " << keypoints_to_triangulate.size()
            << '\n'
    ...
    // If requested, also return the unique ids of the vertices of each
    // triangle. subdiv.locate has a bug, let us just use hashes of vertices...
    if (vtx_indices) {
        vtx_indices->clear();
        ...
        if (rect.contains(cv::Point2f(tri[0], tri[1])) &&
            ...
            if (vtx_indices) {
                ...
                for (size_t j = 0u; j < tri.rows / 2u; j++) {
                    ...
                    // Find vertex ids
                    tri_vtx_indices[j] = UtilsNumerical::hashPair(
                        std::make_pair(pixel.x, pixel.y));
                }
...
// Create a 2D mesh from 2D corners in an image
std::vector<cv::Vec6f>
Mesher::createMesh2D(const Frame&               frame,
                     const std::vector<size_t>& selected_indices)
{
    ...
    for (const size_t& i : selected_indices) {
        ...
        if (frame.landmarks_.at(i) != -1 && rect.contains(keypoint_i)) {
            ...
            // end up outside image after tracking which causes subdiv to
            // crash).
    ...
    std::vector<std::pair<LandmarkId, gtsam::Point3>>* lmk_with_id_stereo)
{
    ...
    for (size_t i = 0u; i < landmarks.size(); i++) {
        if (keypoints_status.at(i) == KeypointStatus::VALID &&
            ...
            // These points are in stereo camera and are not in VIO, but have
            // lmk id.
