...
Mesh<VertexPositionType>::Mesh(const size_t& polygon_dimension)
    : vertex_to_lmk_id_map_(), lmk_id_to_vertex_map_(),
      vertices_mesh_(0, 1, CV_32FC3), vertices_mesh_normal_(),
      normals_computed_(false),
      vertices_mesh_color_(0, 0, CV_8UC3, cv::viz::Color::blue()),
      polygons_mesh_(0, 1, CV_32SC1),
      adjacency_matrix_(1, 1, CV_8UC1, cv::Scalar(0u)), face_hashes_(),
...
/* -------------------------------------------------------------------------- */
template <typename VertexPositionType>
Mesh<VertexPositionType>&
Mesh<VertexPositionType>::operator=(const Mesh<VertexPositionType>& rhs_mesh)
{
...
void Mesh<VertexPositionType>::addPolygonToMesh(const Polygon& polygon)
{
    ...
    for (const VertexType& vertex : polygon) {
        ...
        const VertexId& vtx_id = updateMeshDataStructures(
            lmk_id, vertex.getVertexPosition(), vertex.getVertexColor(),
            vertex.getVertexNormal(), &vertex_to_lmk_id_map_,
            &lmk_id_to_vertex_map_, &vertices_mesh_, &vertices_mesh_normal_,
    ...
    if (triangle_maybe_already_in_mesh) {
        ...
        if (it != face_hashes_.end()) {
            ...
            CHECK(it->second);
            CHECK_EQ(adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[0],
                                                   sorted_vtx_ids[1]),
                     1u);
            CHECK_EQ(adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[1],
                                                   sorted_vtx_ids[2]),
                     1u);
            CHECK_EQ(adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[2],
                                                   sorted_vtx_ids[0]),
                     1u);
        }
        else {
    ...
    if (!triangle_in_mesh) {
        ...
        CHECK(it == face_hashes_.end())
            << "Hash collision? This can happen but "
        ...
        if (!triangle_maybe_already_in_mesh) {
            ...
            for (const auto& sorted_vtx_id : sorted_vtx_ids) {
                if (sorted_vtx_id >= adjacency_matrix_.rows) {
                    // Non-existing vertex! Add row/col. Careful! vtx_ids
                    // ordering matters! First order them!
                    CHECK(adjacency_matrix_.rows != 0 &&
                          adjacency_matrix_.cols != 0);
                    cv::Mat row =
                        cv::Mat::zeros(1, adjacency_matrix_.cols, CV_8UC1);
                    cv::vconcat(adjacency_matrix_, row, adjacency_matrix_);

                    cv::Mat col =
                        cv::Mat::zeros(adjacency_matrix_.rows, 1, CV_8UC1);
        ...
        adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[0], sorted_vtx_ids[1]) =
            1u;
        adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[1], sorted_vtx_ids[0]) =
            1u;
        adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[0], sorted_vtx_ids[2]) =
            1u;
        ...
        adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[1], sorted_vtx_ids[2]) =
            1u;
        adjacency_matrix_.at<uint8_t>(sorted_vtx_ids[2], sorted_vtx_ids[1]) =
            1u;
    }
    else {
        // No need to update connectivity, since the triangle is in the mesh
        // already
        CHECK(it != face_hashes_.end());
                                          ...
                                          Polygon*      polygon) const
{
    ...
    for (size_t j = 0; j < polygon_dimension_; j++) {
        ...
        polygon->at(j) = Vertex<VertexPositionType>(
            ...
            has_normals ? vertices_mesh_normal_.at(row_id_pt_j) :
                          VertexNormal());
    }
    return true;
}
...
{
    CHECK_EQ(polygon_dimension_, 3)
        << "Normals are only valid for dim 3 meshes.";
    ...
    for (size_t i = 0; i < getNumberOfPolygons(); i++) {
        ...
        /// Indices of vertices
        const VertexId& p1_idx =
            lmk_id_to_vertex_map_.at(polygon.at(0).getLmkId());
        const VertexId& p2_idx =
            lmk_id_to_vertex_map_.at(polygon.at(1).getLmkId());
        const VertexId& p3_idx =
            lmk_id_to_vertex_map_.at(polygon.at(2).getLmkId());
...
LandmarkIds Mesh<VertexPositionType>::getLandmarkIds() const
{
    ...
    for (LmkIdToVertexMap::const_iterator it = lmk_id_to_vertex_map_.begin();
         it != lmk_id_to_vertex_map_.end(); ++it) {
        lmk_ids[i] = it->first;
        i++;
    }
