...
MesherModule::MesherModule(bool parallel_run, Mesher::UniquePtr mesher)
    : MIMOPipelineModule<MesherInput, MesherOutput>("Mesher", parallel_run),
      ...
      backend_payload_queue_("mesher_backend"), mesher_(std::move(mesher))
{
}
...
MesherModule::InputUniquePtr MesherModule::getInputPacket()
{
    ...
    return VIO::make_unique<MesherInput>(timestamp, frontend_payload,
                                         backend_payload);
}

MesherModule::OutputUniquePtr
MesherModule::spinOnce(MesherInput::UniquePtr input)
{
    CHECK(input);
