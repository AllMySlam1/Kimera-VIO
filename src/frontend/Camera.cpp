...
Camera::Camera(const CameraParams& cam_params)
    : cam_params_(cam_params), calibration_(cam_params.intrinsics_.at(0),
                                            cam_params.intrinsics_.at(1),
                                            0.0,  // No skew
                                            cam_params.intrinsics_.at(2),
                                            cam_params.intrinsics_.at(3)),
      undistorter_(nullptr), camera_impl_(nullptr)
{
...
void Camera::project(const LandmarkCV& lmk, KeypointCV* kpt) const
{
    ...
    // through a cascade of calls... Only useful if you want gradients I
    // guess...
    CHECK(camera_impl_);
                                ...
                                StatusKeypointsCV* status_keypoints) const
{
    ...
    CHECK(undistorter_);
    undistorter_->undistortRectifyKeypoints(keypoints, &undistorted_keypoints);
