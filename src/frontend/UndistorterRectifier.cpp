    ...
    boost::optional<cv::Mat> P)
{
    switch (cam_param.distortion_model_) {
        case DistortionModel::RADTAN: {
            cv::undistortPoints(keypoints, *undistorted_keypoints, cam_param.K_,
        ...
        case DistortionModel::EQUIDISTANT: {
            ...
            cv::fisheye::undistortPoints(
                keypoints, *undistorted_keypoints, cam_param.K_,
                cam_param.distortion_coeff_mat_, R ? R.get() : cv::noArray(),
                P ? P.get() : cv::noArray());
        } break;
        default: {
    ...
    boost::optional<cv::Mat> R)
{
    ...
    gtsam::Vector3 versor(undistorted_keypoint.at(0).x,
                          undistorted_keypoint.at(0).y, 1.0);
    ...
    //  std::endl; std::cout << "distorted_keypoint_gtsam: \n" <<
    //  distorted_keypoint_gtsam << std::endl; std::cout << "px_mismatch: \n" <<
    //  px_mismatch << std::endl; throw
    //  std::runtime_error("UndistortKeypointAndGetVersor: possible calibration
    //  mismatch");
    //}
                                                 ...
                                                 cv::Mat* undistorted_img) const
{
    ...
    cv::remap(img, *undistorted_img, map_x_, map_y_, remap_interpolation_type_,
    ...
    const float&       pixel_tol) const
{
    ...
    for (size_t i = 0u; i < undistorted_kps.size(); i++) {
        ...
        if (cropped) {
            ...
            if (std::fabs(distorted_kp.x - expected_distorted_kp_x) >
                    pixel_tol ||
                std::fabs(distorted_kp.y - expected_distorted_kp_y) >
                    pixel_tol) {
                // Mark as invalid all pixels that were undistorted out of the
                // frame and for which the undistorted rectified keypoint remaps
                // close to the distorted unrectified pixel.
                VLOG(5)
                    << "Pixel mismatch when checking undistortRectification! \n"
                ...
                // Invalid points.
                invalid_count += 1;
                status_kps->push_back(std::make_pair(
                    KeypointStatus::NO_LEFT_RECT, undistorted_kp));
            }
            else {
                // Point is valid!
                status_kps->push_back(
    ...
    switch (cam_params.distortion_model_) {
        ...
        case DistortionModel::RADTAN: {
            cv::initUndistortRectifyMap(
                // Input
                cam_params.K_, cam_params.distortion_coeff_mat_, R, P,
                cam_params.image_size_, kImageType,
                // Output:
