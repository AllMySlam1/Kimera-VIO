...
StereoMatcher::StereoMatcher(const StereoCamera::ConstPtr& stereo_camera,
    ...
    : stereo_camera_(stereo_camera),
      stereo_matching_params_(stereo_matching_params), dense_stereo_params_()
{
}
    ...
    cv::Mat*       disparity_img)
{
    ...
    if (dense_stereo_params_.use_sgbm_) {
        ...
        cv_stereo_matcher = cv::StereoSGBM::create(
            dense_stereo_params_.min_disparity_,
            dense_stereo_params_.num_disparities_,
            dense_stereo_params_.sad_window_size_, dense_stereo_params_.p1_,
            dense_stereo_params_.p2_, dense_stereo_params_.disp_12_max_diff_,
            dense_stereo_params_.pre_filter_cap_,
            dense_stereo_params_.uniqueness_ratio_,
            dense_stereo_params_.speckle_window_size_,
            dense_stereo_params_.speckle_range_, mode);
    }
    else {
    ...
    // Reconstruct scene
    cv_stereo_matcher->compute(left_img_rectified, right_img_rectified,
                               *disparity_img);
...
void StereoMatcher::sparseStereoReconstruction(StereoFrame* stereo_frame)
{
    ...
    //   rectified using this function! That's why the above check doesn't
    //   work...
    ...
    for (size_t i = 0; i < stereo_frame->right_keypoints_rectified_.size();
         i++) {
        if (stereo_frame->right_keypoints_rectified_[i].first ==
            ...
            // keypoints_depth_ is not the norm of the vector, it is the z
            // component.
    ...
    StatusKeypointsCV*       right_keypoints_rectified)
{
    ...
    switch (stereo_matching_params_.vision_sensor_type_) {
        case VisionSensorType::STEREO: {
            getRightKeypointsRectified(left_img_rectified, right_img_rectified,
                                       left_keypoints_rectified, fx, baseline,
    ...
    StatusKeypointsCV*       right_keypoints_rectified) const
{
    ...
    for (const StatusKeypointCV& left_keypoint_rectified :
        ...
        if (left_keypoint_rectified.first != KeypointStatus::VALID) {
            ...
            right_keypoints_rectified->push_back(std::make_pair(
                left_keypoint_rectified.first, KeypointCV(0.0, 0.0)));
            continue;
        }
        ...
        searchRightKeypointEpipolar(
            left_img_rectified, left_rectified_i, right_img_rectified,
            stripe_cols, stripe_rows, stereo_matching_params_,
            &right_rectified_i_candidate, &matching_val_LR);
    ...
    if (verbosity > 0) {
        cv::Mat left_img_with_keypoints = UtilsOpenCV::DrawCircles(
            left_img_rectified, left_keypoints_rectified);
    ...
    double*                     score) const
{
    ...
    int temp_corner_y = rounded_left_rectified_i_y -
                        (stereo_matching_params.templ_rows_ - 1) / 2;
    if (temp_corner_y < 0 ||
    ...
    int offset_temp   = 0;
    int temp_corner_x = rounded_left_rectified_i_x -
    ...
    // Create template
    cv::Rect templ_selector(temp_corner_x, temp_corner_y,
    ...
    // Create stripe
    cv::Rect stripe_selector(stripe_corner_x, stripe_corner_y, stripe_cols,
                             stripe_rows);
    ...
    matchLoc.x += stripe_corner_x +
                  (stereo_matching_params.templ_cols_ - 1) / 2 + offset_temp;
    // From result to image
    matchLoc.y +=
        stripe_corner_y + (stereo_matching_params.templ_rows_ - 1) / 2;
    ...
    *score = min_val;
    if (min_val < stereo_matching_params.tolerance_template_matching_) {
        ...
        *right_keypoint_rectified =
            std::make_pair(KeypointStatus::VALID, match_px);
    }
    else {
        *right_keypoint_rectified =
    ...
    for (i = 0; i < left_keypoints_rectified.size(); i++) {
        if (left_keypoints_rectified[i].first == KeypointStatus::VALID &&
            ...
            if (left_keypoints_rectified[i].first != KeypointStatus::VALID &&
                ...
                LOG(WARNING)
                    << "Cannot have a valid right kpt without also a valid "
                       "left kpt!"
                    << "\nLeft kpt status: "
