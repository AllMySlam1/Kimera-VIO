...
StereoCamera::StereoCamera(const CameraParams& left_cam_params,
    ...
    : original_left_camera_(nullptr), original_right_camera_(nullptr),
      undistorted_rectified_stereo_camera_impl_(), stereo_calibration_(nullptr),
      stereo_baseline_(0.0), left_cam_undistort_rectifier_(nullptr),
      right_cam_undistort_rectifier_(nullptr)
{
    computeRectificationParameters(left_cam_params, right_cam_params, &R1_,
                                   &R2_, &P1_, &P2_, &Q_, &ROI1_, &ROI2_);
    ...
    B_Pose_camRrect_ =
        left_cam_params.body_Pose_cam_.compose(camR_Pose_camRrect);
                                        ...
                                        LandmarkCV*       lmk) const
{
    CHECK_NOTNULL(lmk);
    CHECK(stereo_calibration_ != nullptr);
    CHECK_GT(disparity, 0.0)
        << "Requested back projection of a keypoint :" << kp
                               ...
                               LandmarksCV*       lmks) const
{
    ...
    for (const KeypointCV& kp : kps) {
        // Some algorithms, like StereoBM or StereoSGBM compute 16-bit
        // fixed-point disparity map (where each disparity value has 4
        // fractional bits), whereas other algorithms output 32-bit
                                            ...
                                            cv::Mat*       depth) const
{
    ...
    cv::reprojectImageTo3D(disparity_img, *depth, Q_, kHandleMissingValues,
                           CV_32F);
    ...
    cv::Rect*           ROI2)
{
    ...
    switch (left_cam_params.distortion_model_) {
        case DistortionModel::RADTAN: {
            cv::stereoRectify(
                // Input
                left_cam_params.K_, left_cam_params.distortion_coeff_mat_,
                right_cam_params.K_, right_cam_params.distortion_coeff_mat_,
                left_cam_params.image_size_, camL_Rot_camR, camL_Tran_camR,
                // Output
                *R1, *R2, *P1, *P2, *Q, cv::CALIB_ZERO_DISPARITY, kAlpha,
                cv::Size(), ROI1, ROI2);
        } break;
