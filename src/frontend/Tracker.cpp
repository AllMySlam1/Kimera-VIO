...
Tracker::Tracker(const FrontendParams&   tracker_params,
    ...
    : landmark_count_(0), tracker_params_(tracker_params), camera_(camera),
      // Only for debugging and visualization:
      optical_flow_predictor_(nullptr), display_queue_(display_queue),
      output_images_path_("./outputImages/")
{
    ...
    optical_flow_predictor_ =
        OpticalFlowPredictorFactory::makeOpticalFlowPredictor(
            ...
            camera_->getCamParams().K_, camera_->getCamParams().image_size_);
                              ...
                              boost::optional<cv::Mat> R)
{
    ...
    const cv::TermCriteria kTerminationCriteria(
        ...
        tracker_params_.klt_max_iter_, tracker_params_.klt_eps_);
    ...
    KeypointsCV px_cur;
    CHECK(
        optical_flow_predictor_->predictSparseFlow(px_ref, ref_R_cur, &px_cur));
    ...
    cv::calcOpticalFlowPyrLK(
        ...
        klt_window_size, tracker_params_.klt_max_level_, kTerminationCriteria,
    ...
    if (display_queue_ && FLAGS_visualize_feature_predictions) {
        displayImage(
            cur_frame->timestamp_, "feature_tracks_with_predicted_keypoints",
...
Tracker::geometricOutlierRejectionMono(Frame* ref_frame, Frame* cur_frame)
{
    ...
    if (!mono_ransac_.computeModel(0)) {
        ...
        return std::make_pair(TrackingStatus::INVALID,
    ...
    removeOutliersMono(mono_ransac_.inliers_, ref_frame, cur_frame,
                       &matches_ref_cur);
    ...
    double disparity;
    bool   median_disparity_success =
        computeMedianDisparity(ref_frame->keypoints_, cur_frame->keypoints_,
                               matches_ref_cur, &disparity);
    ...
    const gtsam::Rot3& camLrectlkf_R_camLrectkf)
{
    ...
    std::shared_ptr<ProblemMonoGivenRot> problem =
        std::make_shared<ProblemMonoGivenRot>(
    ...
    removeOutliersMono(mono_ransac_given_rot_.inliers_, ref_frame, cur_frame,
                       &matches_ref_cur);
...
std::pair<Vector3, Matrix3>
Tracker::getPoint3AndCovariance(const StereoFrame&              stereoFrame,
                                ...
                                boost::optional<gtsam::Matrix3> Rmat)
{
    ...
    if (Rmat) {
        point3_i =
            (*Rmat) * point3_i;  // optionally rotated to another ref frame
    ...
    const gtsam::Rot3&          camLrectlkf_R_camLrectkf)
{
    ...
    for (const KeypointMatch& it : matches_ref_cur) {
        ...
        std::tie(R_f_cur_i, cov_R_cur_i) = Tracker::getPoint3AndCovariance(
            cur_stereoFrame, stereoCam, it.second, stereoPtCov,
    ...
    for (size_t i = 0; i < nrMatches; i++)
        coherentSet.at(i).reserve(nrMatches);
    ...
    // Remove outliers.
    removeOutliersStereo(inliers, &ref_stereoFrame, &cur_stereoFrame,
                         &matches_ref_cur);
    ...
    if (VLOG_IS_ON(10)) {
        double time_translation_computation_p =
            utils::Timer::toc(start_time_tic).count();
        VLOG(5) << " Time MatchingAndAllocation: "
                << timeMatchingAndAllocation_p
    ...
    debug_info_.stereoRansacIters_ =
        1;  // this is bcs we use coherent sets here.

    return std::make_pair(
        std::make_pair(
            status, gtsam::Pose3(camLrectlkf_R_camLrectkf, gtsam::Point3(t))),
                                         ...
                                         StereoFrame& cur_stereoFrame)
{
    ...
    if (!stereo_ransac_.computeModel(0)) {
        LOG(WARNING) << "failure: (Arun) RANSAC could not find a solution."
                     << "\n  size of matches_ref_cur: "
                     << matches_ref_cur.size()
    ...
    // Remove outliers.
    removeOutliersStereo(stereo_ransac_.inliers_, &ref_stereoFrame,
                         &cur_stereoFrame, &matches_ref_cur);
    ...
    KeypointMatches*   matches_ref_cur_stereo)
{
    ...
    findMatchingKeypoints(ref_stereoFrame.left_frame_,
                          cur_stereoFrame.left_frame_, &matches_ref_cur_mono);
    findMatchingStereoKeypoints(ref_stereoFrame, cur_stereoFrame,
    ...
    for (size_t i = 0; i < matches_ref_cur_mono.size(); ++i) {
        ...
        if (ref_stereoFrame.right_keypoints_rectified_[ind_ref].first ==
            ...
            VLOG(5) << "Failed match status: "
                    << to_underlying(
                           ref_stereoFrame.right_keypoints_rectified_[ind_ref]
                               .first)
                    << " "
                    << to_underlying(
                           cur_stereoFrame.right_keypoints_rectified_[ind_cur]
                               .first);
        }
    }
}
                                     ...
                                     double*                median_disparity)
{
    ...
    for (const KeypointMatch& rc : matches_ref_cur) {
        KeypointCV px_diff =
            cur_frame_kpts[rc.second] - ref_frame_kpts[rc.first];
    ...
    std::nth_element(disparity_sq.begin(), disparity_sq.begin() + center,
                     disparity_sq.end());
    ...
    for (size_t i = 0; i < cur_frame.keypoints_.size(); ++i) {
        ...
        if (cur_frame.landmarks_.at(i) == -1) {  // Untracked landmarks are red.
            ...
            if (it != ref_frame.landmarks_.end()) {
                // If feature was in previous frame, display tracked feature
                // with green circle/line:
