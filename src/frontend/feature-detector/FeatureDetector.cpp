...
FeatureDetector::FeatureDetector(
    ...
    : feature_detector_params_(feature_detector_params),
      non_max_suppression_(nullptr), feature_detector_()
{
...
#endif
            static constexpr int patch_size =
                2;  // We don't use descriptors (yet).
            feature_detector_ = cv::ORB::create(
                feature_detector_params_.max_features_per_frame_, scale_factor,
                n_levels, edge_threshold, first_level, WTA_K, score_type,
                patch_size, feature_detector_params.fast_thresh_);
            break;
        }
                                       ...
                                       boost::optional<cv::Mat> R)
{
    ...
    const KeypointsCV& corners =
        featureDetection(*cur_frame, nr_corners_needed);
    ...
    if (n_corners > 0u) {
        ...
        VLOG(10) << "featureExtraction: frame " << cur_frame->id_
                 ...
                 << ",  total: " << cur_frame->keypoints_.size() << "  (max: "
                 << feature_detector_params_.max_features_per_frame_ << ")";
    }
    else {
...
FeatureDetector::rawFeatureDetection(const cv::Mat& img, const cv::Mat& mask)
{
                                              ...
                                              const int&   need_n_corners)
{
    ...
    if (non_max_suppression_) {
        ...
        max_keypoints                    = non_max_suppression_->suppressNonMax(
            keypoints, need_n_corners, tolerance, cur_frame.img_.cols,
            cur_frame.img_.rows);
    }
    ...
    if (new_corners.size() > 0) {
        if (feature_detector_params_.enable_subpixel_corner_refinement_) {
            ...
            cv::cornerSubPix(
                cur_frame.img_, new_corners, subpixel_params.window_size_,
                subpixel_params.zero_zone_, subpixel_params.term_criteria_);
