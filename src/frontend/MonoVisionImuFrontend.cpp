...
MonoVisionImuFrontend::MonoVisionImuFrontend(
    ...
    : VisionImuFrontend(imu_params,
                        ...
                        display_queue,
                        log_output),
      mono_frame_k_(nullptr), mono_frame_km1_(nullptr),
      mono_frame_lkf_(nullptr), keyframe_R_ref_frame_(gtsam::Rot3::identity()),
      feature_detector_(nullptr), mono_camera_(camera),
      frontend_params_(frontend_params)
{
    CHECK(mono_camera_);

    tracker_ = VIO::make_unique<Tracker>(frontend_params_, mono_camera_,
                                         display_queue);
    ...
    MonoFrontendInputPayload::UniquePtr&& input)
{
    ...
    return VIO::make_unique<MonoFrontendOutput>(
        mono_frame_lkf_->isKeyframe_, nullptr, TrackingStatus::DISABLED,
        gtsam::Pose3::identity(),  // no stereo!
        mono_camera_->getBodyPoseCam(), *mono_frame_lkf_, nullptr,
    ...
    if (mono_frame_km1_->isKeyframe_) {
        ...
        if (logger_) {
            logger_->logFrontendStats(mono_frame_lkf_->timestamp_,
                                      getTrackerInfo(), tracker_status_summary_,
        ...
        return VIO::make_unique<MonoFrontendOutput>(
            ...
            *mono_frame_lkf_,  //! This is really the current keyframe in this
                               //! if
        ...
        return VIO::make_unique<MonoFrontendOutput>(
            false, status_mono_measurements,
            TrackingStatus::DISABLED,  // This is a stereo status only
            gtsam::Pose3::identity(),  // don't pass stereo pose to Backend!
            mono_camera_->getBodyPoseCam(),
            *mono_frame_lkf_,  //! This is really the current keyframe in this
...
MonoVisionImuFrontend::processFrame(const Frame&       cur_frame,
                                    ...
                                    cv::Mat*           feature_tracks)
{
    ...
    tracker_->featureTracking(mono_frame_km1_.get(), mono_frame_k_.get(),
    ...
    if (max_time_elapsed || nr_features_low || mono_frame_k_->isKeyframe_) {
        ...
        if (tracker_->tracker_params_.useRANSAC_) {
            ...
            outlierRejectionMono(keyframe_R_cur_frame, mono_frame_lkf_.get(),
                                 mono_frame_k_.get(), &status_pose_mono);
        ...
        // Undistort keypoints:
        mono_camera_->undistortKeypoints(
            mono_frame_k_->keypoints_, &mono_frame_k_->keypoints_undistorted_);
        ...
        if (display_queue_ && FLAGS_visualize_feature_tracks) {
            displayImage(
                mono_frame_k_->timestamp_, "feature_tracks",
                tracker_->getTrackerImage(*mono_frame_lkf_, *mono_frame_k_),
                display_queue_);
        }
