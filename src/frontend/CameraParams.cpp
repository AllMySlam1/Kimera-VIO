...
CameraParams::stringToDistortion(const std::string& distortion_model,
        ...
        if (lower_case_camera_model == std::string("none")) {
            ...
            LOG(FATAL)
                << "Unrecognized distortion model for pinhole camera. Valid "
                   "pinhole "
                   "distortion model options are 'none', 'radtan', "
                   "'equidistant'.";
        }
    }
    else {
    ...
    cv::Mat*                   distortion_coeffs_mat)
{
    ...
    *distortion_coeffs_mat =
        cv::Mat::zeros(1, distortion_coeffs.size(), CV_64F);
                                         ...
                                         Intrinsics*       intrinsics_)
{
    ...
    std::copy_n(std::make_move_iterator(intrinsics.begin()),
                intrinsics_->size(), intrinsics_->begin());
}
...
void CameraParams::print() const
{
    ...
    PipelineParams::print(
        out, "Camera ID ", camera_id_, "Intrinsics: \n- fx", intrinsics_[0],
        "- fy", intrinsics_[1], "- cu", intrinsics_[2], "- cv", intrinsics_[3],
        "frame_rate_: ", frame_rate_, "image_size_: \n - width",
        image_size_.width, "- height", image_size_.height);
    LOG(INFO) << out.str();
