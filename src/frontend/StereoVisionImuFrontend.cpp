...
StereoVisionImuFrontend::StereoVisionImuFrontend(
    ...
    : VisionImuFrontend(imu_params,
                        ...
                        display_queue,
                        log_output),
      stereoFrame_k_(nullptr), stereoFrame_km1_(nullptr),
      stereoFrame_lkf_(nullptr), keyframe_R_ref_frame_(gtsam::Rot3::identity()),
      feature_detector_(nullptr), frontend_params_(frontend_params),
...
{  // Only for debugging and visualization.
    ...
    tracker_ = VIO::make_unique<Tracker>(
        frontend_params_, stereo_camera_->getOriginalLeftCamera(),
        display_queue);

    if (VLOG_IS_ON(1))
        tracker_->tracker_params_.print();
}
    ...
    StereoFrontendInputPayload::UniquePtr&& input)
{
    ...
    return VIO::make_unique<StereoFrontendOutput>(
        stereoFrame_lkf_->isKeyframe(), nullptr, TrackingStatus::DISABLED,
        getRelativePoseBodyStereo(), stereo_camera_->getBodyPoseLeftCamRect(),
        stereo_camera_->getBodyPoseRightCamRect(), *stereoFrame_lkf_, nullptr,
        input->getImuAccGyrs(), cv::Mat(), getTrackerInfo());
}
    ...
    StereoFrontendInputPayload::UniquePtr&& input)
{
    ...
    // But note that we are using interpolated "fake" values when doing the
    // preint egration!! Should we remove them?? Actually, currently does not
    // integrate fake interpolated meas as it does not take the last measurement
    // into account (although it takes its stamp into account!!!).
    ...
    // utils::StatsCollector stats_full_preint("IMU Preintegration Timing
    // [us]"); stats_full_preint.AddSample(full_preint_duration);
    VLOG_IF(1, full_preint_duration != 0.0)
        << "Current IMU Preintegration frequency: "
        << 10e6 / full_preint_duration << " Hz. (" << full_preint_duration
        << " us).";
    ...
    if (stereoFrame_km1_->isKeyframe()) {
        ...
        if (logger_) {
            logger_->logFrontendStats(
                stereoFrame_lkf_->timestamp_, getTrackerInfo(),
        ...
        timing_stats_keyframe_rate.AddSample(
            utils::Timer::toc(start_time).count());

        // Return the output of the Frontend for the others.
        VLOG(2)
            << "Frontend output is a keyframe: pushing to output callbacks.";
        return VIO::make_unique<StereoFrontendOutput>(
            true, status_stereo_measurements,
            ...
            *stereoFrame_lkf_,  //! This is really the current keyframe in this
                                //! if
            pim, input->getImuAccGyrs(), feature_tracks, getTrackerInfo());
    }
    else {
        // Record frame rate timing
        timing_stats_frame_rate.AddSample(
            utils::Timer::toc(start_time).count());

        // We don't have a keyframe.
        VLOG(2)
            << "Frontend output is not a keyframe. Skipping output queue push.";
        return VIO::make_unique<StereoFrontendOutput>(
            false, status_stereo_measurements, TrackingStatus::INVALID,
            tracker_->tracker_params_.useStereoTracking_ ?
                getRelativePoseBodyStereo() :
                gtsam::Pose3::identity(),
            stereo_camera_->getBodyPoseLeftCamRect(),
            stereo_camera_->getBodyPoseRightCamRect(), *stereoFrame_lkf_, pim,
            input->getImuAccGyrs(), feature_tracks, getTrackerInfo());
    }
}
    ...
    const StereoFrame& firstFrame)
{
    ...
    stereoFrame_k_ = std::make_shared<StereoFrame>(
        firstFrame);  // TODO this can be optimized!
    ...
    cv::Mat*           feature_tracks)
{
    ...
    tracker_->featureTracking(&stereoFrame_km1_->left_frame_, left_frame_k,
                              ref_frame_R_cur_frame, stereo_camera_->getR1());
    if (feature_tracks) {
        ...
        *feature_tracks = tracker_->getTrackerImage(
            stereoFrame_lkf_->left_frame_, stereoFrame_k_->left_frame_);
    }
    ...
    if (max_time_elapsed || nr_features_low || stereoFrame_k_->isKeyframe()) {
        ...
        if (tracker_->tracker_params_.useRANSAC_) {
            ...
            outlierRejectionMono(keyframe_R_cur_frame, left_frame_lkf,
                                 left_frame_k, &status_pose_mono);
            tracker_status_summary_.kfTrackingStatus_mono_ =
                status_pose_mono.first;
            ...
            if (tracker_->tracker_params_.useStereoTracking_) {
                outlierRejectionStereo(keyframe_R_cur_frame, stereoFrame_lkf_,
                                       stereoFrame_k_, &status_pose_stereo);
                tracker_status_summary_.kfTrackingStatus_stereo_ =
                    status_pose_stereo.first;

                if (status_pose_stereo.first == TrackingStatus::VALID) {
                    tracker_status_summary_.lkf_T_k_stereo_ =
                        status_pose_stereo.second;
                }
            }
            else {
        ...
        if (VLOG_IS_ON(2)) {
            printTrackingStatus(tracker_status_summary_.kfTrackingStatus_mono_,
                                "mono");
            printTrackingStatus(
                tracker_status_summary_.kfTrackingStatus_stereo_, "stereo");
        }
        ...
        if (display_queue_ && FLAGS_visualize_feature_tracks) {
            displayImage(
                stereoFrame_k_->timestamp_, "feature_tracks",
        ...
        double get_smart_stereo_meas_time =
            utils::Timer::toc(start_time).count();
    ...
    stereoFrame_k_.reset();
    ++frame_count_;
    return std::make_shared<StatusStereoMeasurements>(std::make_pair(
        tracker_status_summary_,
    ...
    if (tracker_->tracker_params_.ransac_use_1point_stereo_ &&
        ...
        std::tie(*status_pose_stereo, infoMatStereoTranslation) =
            tracker_->geometricOutlierRejectionStereoGivenRotation(
                *stereoFrame_lkf_, *stereoFrame_k_, stereo_camera_,
        ...
        *status_pose_stereo = tracker_->geometricOutlierRejectionStereo(
            *stereoFrame_lkf_, *stereoFrame_k_);
        LOG_IF(WARNING, force_53point_ransac_)
            << "3-point RANSAC was enforced!";
    }
...
void StereoVisionImuFrontend::sendFeatureTracksToLogger() const
{
    ...
    logger_->logFrontendImg(left_frame_k.id_, img_left, "monoFeatureTracksLeft",
...
void StereoVisionImuFrontend::sendStereoMatchesToLogger() const
{
    ...
    logger_->logFrontendImg(
        left_frame_k.id_, img_left_right, "stereoMatchingUnrectified",
        "/stereoMatchingUnrectifiedImg/", FLAGS_visualize_frontend_images,
        FLAGS_save_frontend_images);
    //############################################################################
    ...
    cv::Mat img_left_right_rectified      = UtilsOpenCV::DrawCornersMatches(
        ...
        stereoFrame_k_->right_keypoints_rectified_, matches, kUseRandomColor);
    ...
    logger_->logFrontendImg(
        left_frame_k.id_, img_left_right_rectified, "stereoMatchingRectified",
        "/stereoMatchingRectifiedImg/", FLAGS_visualize_frontend_images,
        FLAGS_save_frontend_images);
}
...
void StereoVisionImuFrontend::sendMonoTrackingToLogger() const
{
    ...
    for (size_t i = 0; i < cur_left_frame.keypoints_.size(); ++i) {
        if (cur_left_frame.landmarks_.at(i) != -1) {  // if landmark is valid
            ...
            if (it !=
                ref_left_frame.landmarks_.end()) {  // if landmark was found
    ...
    cv::Mat img_left_lkf_kf = UtilsOpenCV::DrawCornersMatches(
        ...
        cur_left_frame.keypoints_, matches,
        false);  // true: random color
    ...
    logger_->logFrontendImg(
        cur_left_frame.id_, img_left_lkf_kf, "monoTrackingUnrectified",
        "/monoTrackingUnrectifiedImg/", FLAGS_visualize_frontend_images,
        FLAGS_save_frontend_images);
    //############################################################################
    ...
    logger_->logFrontendImg(
        cur_left_frame.id_, img_left_lkf_kf_rectified, "monoTrackingRectified",
        "/monoTrackingRectifiedImg/", FLAGS_visualize_frontend_images,
        FLAGS_save_frontend_images);
