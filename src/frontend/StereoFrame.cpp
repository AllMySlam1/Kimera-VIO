...
StereoFrame::StereoFrame(const FrameId&   id,
    ...
    : is_keyframe_(false), is_rectified_(false), left_img_rectified_(),
      right_img_rectified_(), id_(id), timestamp_(timestamp),
      // TODO(Toni): remove these copies
      left_frame_(left_frame), right_frame_(right_frame),
      left_keypoints_rectified_(), right_keypoints_rectified_(),
      keypoints_depth_(), keypoints_3d_()
{
...
void StereoFrame::checkStatusRightKeypoints(DebugTrackerInfo* debug_info) const
{
...
cv::Mat StereoFrame::drawCornersMatches(const StereoFrame& stereo_frame_1,
                                        ...
                                        const bool&        random_color)
{
    return UtilsOpenCV::DrawCornersMatches(
        ...
        stereo_frame_2.left_keypoints_rectified_, matches, random_color);
}

cv::Mat StereoFrame::drawLeftRightCornersMatches(const DMatchVec& matches,
                                                 const bool& random_color) const
{
    return UtilsOpenCV::DrawCornersMatches(
        left_img_rectified_, left_keypoints_rectified_, right_img_rectified_,
        right_keypoints_rectified_, matches, random_color);
}
...
void StereoFrame::showRectified(const bool& visualize, const bool& write) const
{
    CHECK(is_rectified_);
                                       ...
                                       const int&         verbosity) const
{
    ...
    if (verbosity == 1) {
        cv::namedWindow(
            title,
            cv::WINDOW_AUTOSIZE);  // moved in here to allow saving images
                                       ...
                                       const bool&   write) const
{
    ...
    for (int l = 0; l < num_lines; l++) {
        ...
        cv::line(canvas, cv::Point2f(0.0f, y_pos),
                 cv::Point2f(canvas.cols - 1, y_pos), cv::Scalar(0, 255, 0));
    }
