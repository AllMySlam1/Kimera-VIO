...
VioBackend::VioBackend(const Pose3&               B_Pose_leftCam,
                       ...
                       bool                       log_output)
    : backend_params_(backend_params), imu_params_(imu_params),
      backend_output_params_(backend_output_params),
      backend_state_(BackendState::Bootstrap), timestamp_lkf_(-1),
      imu_bias_lkf_(ImuBias()), W_Vel_B_lkf_(gtsam::Vector3::Zero()),
      W_Pose_B_lkf_(gtsam::Pose3::identity()), imu_bias_prev_kf_(ImuBias()),
      B_Pose_leftCam_(B_Pose_leftCam), stereo_cal_(stereo_calibration),
      last_kf_id_(-1), curr_kf_id_(0), landmark_count_(0),
...
// Initialize smoother.
    ...
    setFactorsParams(backend_params, &smart_noise_, &smart_factors_params_,
...
BackendOutput::UniquePtr VioBackend::spinOnce(const BackendInput& input)
{
    ...
    // Fill ouput_payload (it will remain nullptr if the backend_status is not
    // ok)
    ...
    if (backend_status) {
        ...
        if (kOutputLmkMap) {
            ...
            // WARNING this also cleans the lmks inside the old_smart_factors
            // map!
        ...
        output_payload = VIO::make_unique<BackendOutput>(
            VioNavStateTimestamped(input.timestamp_, W_Pose_B_lkf_,
                                   W_Vel_B_lkf_, imu_bias_lkf_),
            // TODO(Toni): Make all below optional!!
            state_, smoother_->getFactors(), getCurrentStateCovariance(),
            curr_kf_id_, landmark_count_, debug_info_,
            lmk_ids_to_3d_points_in_time_horizon, lmk_id_to_lmk_type_map);
    ...
    boost::optional<gtsam::Pose3>    stereo_ransac_body_pose)
{
    ...
    return optimize(timestamp_kf_nsec, curr_kf_id_,
                    backend_params_.numOptimize_);
}
...
bool VioBackend::addVisualInertialStateAndOptimize(const BackendInput& input)
{
    ...
    LOG_IF(WARNING, use_stereo_btw_factor &&
        ...
        << "User set useStereoBetweenFactor = true, but "
           "stereo_ransac_body_pose_ not available!";
    ...
    const std::pair<FrameId, StereoPoint2>& new_measurement)
{
    ...
    if (slot != -1) {
        ...
        // If it's slot in the graph is still -1, it means that the factor has
        // not been inserted yet in the graph...
        LOG(FATAL)
            << "When updating the smart factor, its slot should not be -1!"
               " Offensive lmk_id: "
            << lmk_id;
    }
    ...
    const size_t&                      min_age)
{
    ...
    for (SmartFactorMap::iterator old_smart_factor_it =
        ...
        if (smart_factor_ptr != graph.at(slot_id)) {
            ...
            // ERROR: if the pointers don't match, then the code that follows
            // does not make any sense, since we are using lmk_id which comes
            // from smart_factor and result which comes from graph[slot_id], we
            // should use smart_factor_ptr instead then...
            LOG(ERROR)
                << "The factor with slot id: " << slot_id
        ...
        if (result.valid()) {
            ...
            if (gsf->measured().size() >= min_age) {
                ...
                VLOG(20) << "Rejecting lmk with id: " << lmk_id
                         ...
                         << gsf->measured().size() << ", vs min_age of "
                         << min_age << ".";
    ...
    for (size_t i = 0u; i < n_stereo_measurements; ++i) {
        ...
        if (feature_track_it == feature_tracks_.end()) {
            ...
            feature_tracks_.insert(std::make_pair(
                lmk_id_in_kf_i, FeatureTrack(frame_num, stereo_px_i)));
            ++landmark_count_;
        }
        else {
            // @TODO: It seems that this else condition does not help --
            // conjecture that it creates long feature tracks with low
            // information (i.e. we're not moving) This is problematic in
            // conjunction with our landmark selection mechanism which
            // prioritizes long feature tracks
            ...
            // TODO(Toni):
            // Mark feature tracks that have been re-observed, so that we can
            // delete the broken feature tracks efficiently.
        }
    }
}
        ...
        case ImuPreintegrationType::kPreintegratedImuMeasurements: {
            ...
            new_imu_prior_and_other_factors_.push_back(
                boost::make_shared<
                    ...
                    gtsam::Symbol(kImuBiasSymbolChar, from_id),
                    gtsam::Symbol(kImuBiasSymbolChar, to_id), zero_bias,
                    bias_noise_model));
            break;
        }
        default: {
    ...
    const gtsam::FactorIndices& extra_factor_slots_to_delete)
{
    ...
    for (const auto& new_smart_factor : new_smart_factors_) {
        ...
        if (slot != -1) {
            // Smart factor Slot is different than -1, therefore the factor
            // should be already in the factor graph.
            DCHECK_GE(slot, 0);
            if (smoother_->getFactors().exists(slot)) {
                ...
                // (longer than factor graph's time horizon), than the factor
                // has been removed from the optimization. Erase this factor and
                // feature track, as it has gone past the horizon.
                old_smart_factors_.erase(old_smart_factor_it);
                CHECK(deleteLmkFromFeatureTracks(lmk_id));
                // TODO(Toni): we should as well remove it from
                // new_smart_factors_!!
            }
        }
        else {
            // We just add the new smart factor to the graph, as it has never
            // been there before.
            new_factors_tmp.push_back(new_smart_factor.second);
            // Store lmk id of the smart factor to add to the graph.
            lmk_ids_of_new_smart_factors_tmp.push_back(lmk_id);
    ...
    if (VLOG_IS_ON(10)) {
        printSmootherInfo(new_factors_tmp, delete_slots,
                          "Smoother status before update:", VLOG_IS_ON(10));
    }
    ...
    // Compute iSAM update.
    VLOG(10) << "iSAM2 update with " << new_factors_tmp.size()
             << " new factors "
    ...
    if (is_smoother_ok) {
        ...
        // TODO(Toni): shouldn't we be doing this after each updateSmoother
        // call?
...
void VioBackend::addInitialPriorFactors(const FrameId& frame_id)
{
    ...
    prior_biasSigmas.head<3>().setConstant(
        backend_params_.initialAccBiasSigma_);
    prior_biasSigmas.tail<3>().setConstant(
                                           ...
                                           const FrameId& to_id)
{
    ...
    new_imu_prior_and_other_factors_.push_back(
        boost::make_shared<gtsam::BetweenFactor<gtsam::Vector3>>(
            gtsam::Symbol(kVelocitySymbolChar, from_id),
            gtsam::Symbol(kVelocitySymbolChar, to_id), gtsam::Vector3::Zero(),
            constant_velocity_prior_noise_));
                                ...
                                const gtsam::FactorIndices&        delete_slots)
{
    ...
    try {
        ...
        *result = smoother_->update(new_factors, new_values, timestamps,
        ...
        if (debug_smoother_) {
            printSmootherInfo(new_factors, delete_slots, "CATCHING EXCEPTION",
                              false);
        ...
        LOG(ERROR) << "ERROR: Variable has type '"
                   << lmk_symbol_cheirality.chr() << "' "
        ...
        if (got_cheirality_exception) {
            ...
            *smoother_ = smoother_backup;
            ...
            if (VLOG_IS_ON(5) && FLAGS_debug_graph_before_opt) {
                ...
                debug_info_.graphToBeDeleted = gtsam::NonlinearFactorGraph();
                debug_info_.graphToBeDeleted.resize(
                    delete_slots_cheirality.size());
                for (size_t i = 0; i < delete_slots_cheirality.size(); i++) {
                    ...
                    // delete, extend graphToBeDeleted to have both the factor
                    // and the slot.
                    debug_info_.graphToBeDeleted.at(i) =
    ...
    const gtsam::FactorIndices&        delete_slots)
{
    ...
    *delete_slots_cheirality = delete_slots;
    ...
    findSlotsOfFactorsWithKey(lmk_key, graph,
    ...
    const bool&                        showDetails) const
{
    ...
    ////////////////////// Print all factors.
    ///////////////////////////////////////
    LOG(INFO) << "Nr of factors in graph " + *which_graph << ": "
              << graph->size() << ", with factors:" << std::endl;
    LOG(INFO) << "[\n";
    ...
    if (debug_info_.graphToBeDeleted.size() != 0) {
        ...
        for (size_t i = 0u; i < delete_slots.size(); ++i) {
            ...
            if (print_point_plane_factors) {
                printSelectedFactors(
                    debug_info_.graphToBeDeleted.at(i), delete_slots.at(i),
                    false, print_point_plane_factors, false, false, false);
            }
            else {
    ...
    for (const auto& g : graph) {
        if (g) {
            ...
            if (gsf) {
                ...
                // LOW_DISPARITY at some frame, we do not add the measurement to
                // the smart factor, hence keys are not necessarily consecutive
                // auto keys = g->keys();
                // Key last_key;
                // bool first_key = true;
                // for (Key key : keys)
                ...
                //    std::cout << " Last: " <<
                //    gtsam::DefaultKeyFormatter(last_key)
    ...
    const std::chrono::high_resolution_clock::time_point& start_time)
{
    ...
    if (VLOG_IS_ON(10)) {
        ...
        VLOG(10) << "Optimization Errors:\n"
                 << " - Error before :"
                 << graph.error(debug_info_.stateBeforeOpt) << '\n'
            ...
            if (g->find(key) != g->end()) {
                ...
                CHECK(
                    !boost::dynamic_pointer_cast<gtsam::LinearContainerFactor>(
                        g));
                CHECK(!boost::dynamic_pointer_cast<
                      gtsam::PriorFactor<gtsam::Point3>>(g));
