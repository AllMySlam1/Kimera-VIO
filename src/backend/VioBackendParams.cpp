...
bool BackendParams::parseYAMLVioBackendParams(const YamlParser& yaml_parser)
{
    ...
    switch (linearization_mode_id) {
        case 0: linearizationMode_ = gtsam::HESSIAN; break;
        case 1: linearizationMode_ = gtsam::IMPLICIT_SCHUR; break;
        case 2: linearizationMode_ = gtsam::JACOBIAN_Q; break;
        case 3: linearizationMode_ = gtsam::JACOBIAN_SVD; break;
        default:
    ...
    switch (degeneracy_mode_id) {
        case 0: degeneracyMode_ = gtsam::IGNORE_DEGENERACY; break;
        case 1: degeneracyMode_ = gtsam::ZERO_ON_DEGENERACY; break;
        case 2: degeneracyMode_ = gtsam::HANDLE_INFINITY; break;
        default:
...
void BackendParams::printVioBackendParams() const
{
    ...
    PipelineParams::print(
        out, std::string(kCenter, '.') + "** Initialization parameters **", "",
        "Automatic Initialization", autoInitialize_, "Round on autoinit",
        roundOnAutoInitialize_, "Sigmas:", "", "Initial Position Sigma",
        initialPositionSigma_, "Initiail Roll Pitch Sigma",
        initialRollPitchSigma_, "Initial Yaw Sigma", initialYawSigma_,
        "Initial Velocity Sigma", initialVelocitySigma_,
        "Initial Acc Bias Sigma", initialAccBiasSigma_,
        "Initial Gyro Bias Sigma", initialGyroBiasSigma_,
        std::string(kCenter, '.') + "** Vision parameters **", "",
        "Linearization Mode: hessian, implicit_schur, jacobian_q, jacobian_svd",
        linearizationMode_,
        "Degeneracy Mode: ignore_degeneracy, zero_on_degeneracy, "
        "handle_infinity",
        degeneracyMode_, "Rank Tolerance", rankTolerance_,
        "Landmark Distance Threshold", landmarkDistanceThreshold_,
        "Outlier Rejection", outlierRejection_, "Retriangulation Threshold",
        retriangulationThreshold_, "Add Btw Stereo Factors",
        addBetweenStereoFactors_, "Btw Rotation Precision",
        betweenRotationPrecision_, "Btw Translation Precision",
        betweenTranslationPrecision_,
        std::string(kCenter, '.') + "** Optimization parameters **", "",
        "Relinearize Threshold", relinearizeThreshold_, "Relinearize Skip",
        relinearizeSkip_, "Zero Velocity Sigma", zeroVelocitySigma_,
        "No Motion Position Sigma", noMotionPositionSigma_,
        "No Motion Rotation Sigma", noMotionRotationSigma_,
        "Constant Velocity Sigma", constantVelSigma_, "Optimization Iterations",
        numOptimize_, "Horizon", horizon_, "Isam Wildfire Threshold",
        wildfire_threshold_, "Use Dog Leg", useDogLeg_);
    LOG(INFO) << out.str();
