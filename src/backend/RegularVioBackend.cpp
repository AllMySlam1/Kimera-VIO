    ...
    boost::optional<gtsam::Pose3>    stereo_ransac_body_pose)
{
    ...
    // Get the landmarks visible in current keyframe. (These are not all the
    // lmks in time horizon used for the optimization!)
    LandmarkIds lmks_kf;
            ...
            if (kfTrackingStatus_mono == TrackingStatus::VALID) {
                ...
                switch (regular_vio_params_.backend_modality_) {
                    ...
                    case RegularBackendModality::PROJECTION_AND_REGULARITY: {
                        // Keep the planes, but change all smart factors to
                        // projection factors.
                        lmk_ids_with_regularity = lmks_kf;
                        break;
                    }
                    case RegularBackendModality::STRUCTURELESS_AND_PROJECTION: {
                        // Transforms to projection factors only the ones that
                        // should have regularities, but do not use the planes
                        // anymore.
                        extractLmkIdsFromPlanes(planes_,
                    ...
                    case RegularBackendModality::
                        ...
                        // Act as usual, keep planes, and transform smart
                        // factors to projj factors for those that will have
                        // regularities.
                        extractLmkIdsFromPlanes(planes_,
                                                &lmk_ids_with_regularity);
                        break;
                    ...
                    default: {
                        LOG(ERROR) << "Backend modality: "
                                   << static_cast<std::underlying_type<
                                          RegularBackendModality>::type>(
                                          regular_vio_params_.backend_modality_)
                                   << " is not supported.";
                        break;
                    }
                }
                VLOG(10) << "Finished extracting lmk ids from set of planes, "
                            "total of "
                ...
                // in addLandmarksToGraph, but here we also make sure we have
                // converted the ones with regularities in time horizon.
                if (FLAGS_convert_extra_smart_factors_to_proj_factors) {
                    VLOG(10) << "Starting converting extra smart factors to "
                                "proj factors...";
                    convertExtraSmartFactorToProjFactor(
                        lmk_ids_with_regularity);
                    VLOG(10) << "Finished converting extra smart factors to "
                                "proj factors...";
                }
                ...
                if (planes_.size() > 0) {
                    ...
                    // WARNING if a plane has been removed by the mesher, then
                    // we will not remove the plane from the optimization, since
                    // it won't be in planes.
                    ...
                    for (const Plane& plane : planes_) {
                        ...
                        addRegularityFactors(
                            plane,
                            // Creates a new entry if the plane key was not
                            // found. So make sure you use [plane_key] instead
                            // of .at(plane_key).
                    ...
                    if (FLAGS_remove_old_reg_factors) {
                        ...
                        removeOldRegularityFactors_Slow(
                            planes_, idx_of_point_plane_factors_to_add,
                    ...
                    // TODO shouldn't we "removeOldRegularityFactors_Slow"
                    // because there are no planes anymore? shouldn't we delete
                    // them or something? Not really because the mesher will
                    // only add planes, it won't delete an existing plane from
                    // planes structure... Log warning only if we are not using
                    // a structureless approach (since it does not require
                    // planes).
                    LOG_IF(WARNING, regular_vio_params_.backend_modality_ !=
                                        RegularBackendModality::STRUCTURELESS)
                        << "We are not receiving planes for the Backend. If "
                           "planes have "
                           "been added to the optimization, we are not "
                           "removing them.";
                }
            }
            break;
    ...
    bool is_smoother_ok = optimize(timestamp_kf_nsec, curr_kf_id_,
                                   backend_params_.numOptimize_, delete_slots);
    ...
    if (is_smoother_ok) {
        ...
        // Update estimates of planes, and remove planes that are not in the
        // state.
    ...
    for (const LandmarkId& lmk_id : lmks_kf) {
        ...
        if (feature_track.obs_.size() >= FLAGS_min_num_of_observations) {
            ...
            if (!feature_track.in_ba_graph_) {
                ...
                // For each landmark we decide if it's going to be a smart
                // factor or not. Here there is a timeline mismatch, while
                // landmarks_kf are only currently visible lmks,
                // lmk_ids_with_regularity contains lmks in time_horizon. To
                // further convert the lmks in time_horizon we use the
            ...
            VLOG(20) << "Feature track is shorter ("
                     << feature_track.obs_.size()
    ...
    const std::pair<FrameId, StereoPoint2>& new_obs)
{
    if (is_lmk_smart) {
        ...
        if (state_.find(gtsam::Symbol('l', lmk_id).key()) == state_.end()) {
            ...
            // Unless we could not convert the smart factor to a set of
            // projection factors, then do not add it because we do not have a
            // right value to use.
            if (is_conversion_done) {
                VLOG(20)
                    ...
                    << new_obs.first << ".\n";
                addProjectionFactor(lmk_id, new_obs,
                                    &new_imu_prior_and_other_factors_);
                // Sanity check, if the conversion was successful, then we
                // should not be able to see the smart factor anymore.
                CHECK(!old_smart_factors_.exists(lmk_id));
                CHECK(new_smart_factors_.find(lmk_id) ==
                      new_smart_factors_.end());
            }
            else {
                LOG(ERROR)
                    << "Not using new observation for lmk: " << lmk_id
                    << " because we do not have a good initial value for it.";
            }
        }
        else {
            VLOG(20)
                << "Lmk with id: " << lmk_id << " has been found in state: "
    ...
    SmartFactorMap*                         old_smart_factors)
{
    ...
    CHECK_NE(lmk_id, -1)
        << "When calling update existing smart factor, the slot "
           "should already be != -1! \n";
    ...
    CHECK(old_smart_factors_it != old_smart_factors->end())
        << "Landmark with id: " << lmk_id
        << " not found in old_smart_factors_\n";
    ...
    gtsam::FactorIndices*        delete_slots_of_converted_smart_factors)
{
    ...
    if (old_factor->point().valid()) {
        ...
        VLOG(10) << "Performing conversion for lmk with id: " << lmk_id
                 << " from "
        ...
        // There is no need to have a threshold for how many observations we
        // want , since it is done already in the updateLmkIsSmart function.
        ...
        for (size_t i = 0; i < old_factor->keys().size(); i++) {
            const gtsam::Symbol& cam_sym =
                gtsam::Symbol(old_factor->keys().at(i));
            ...
            VLOG(20) << "Lmk with id: " << lmk_id
                     << " added as a new projection factor attached to pose "
                        "with id: "
        ...
        if (old_smart_factors_it->second.second != -1) {
            ...
            VLOG(20) << "Remove smart factor by adding its slot to the list of "
                        "slots to "
            ...
            CHECK(new_smart_factors->find(lmk_id) == new_smart_factors->end())
                << "Someone is updating the smart factor while it should be a "
                   "projection factor...";
            // NOTE If the slot is -1, so the smart factor is not in the graph
            // yet, but it is maybe going to be added in this iteration as a
            // new_smart_factor...
        }
        ...
        VLOG(20) << "Erasing lmk with id " << lmk_id
                 << " from old_smart_factors";
        ...
        LOG(WARNING)
            << "Cannot convert smart factor to proj. factor.\n"
    ...
    for (const LandmarkId& lmk_id : lmk_ids_with_regularity) {
        ...
        if (lmk_id_is_smart_.exists(lmk_id)) {
            if (lmk_id_is_smart_.at(lmk_id)) {
                // We have found a smart factor that should be a projection
                // factor. Convert it to a projection factor, so that we can
                // enforce regularities on it, but only if it is valid.
                CHECK(old_smart_factors_.exists(lmk_id));
                VLOG(20) << "We found a smart factor that should be in a "
                            "regularity for "
                            "lmk with id: "
                         << lmk_id;
                // We do FLAGS_min_num_obs_for_proj_factor + 1,
                // because we have to substract the fact that in this iteration
                // we added an extra measurement to the factor. This is to be
                // consistent with the check of isSmartFactor3dPointGood when
                // deciding if the 3d point is valid for changing from smart to
                // projection factor. Well this is only true if lmk_id is in the
                // set of lmks observed now... But if it is not, then there is
                // no new observation... Check nwe_smart_factors_ to see if
                // there is indeed a new observation or not.
                if (isSmartFactor3dPointGood(
                    ...
                    VLOG(20) << "Converting extra smart factor to proj factor "
                                "for lmk"
                                " with id: "
                    ...
                    if (convertSmartToProjectionFactor(
                            ...
                            &delete_slots_of_converted_smart_factors_)) {
                        VLOG(30)
                            << "Converted smart factor to proj factor for lmk"
                               " with id: "
                            << lmk_id;
                        ...
                        VLOG(30) << "NOT converted smart factor to proj factor "
                                    "for lmk"
                                    " with id: "
                                 << lmk_id;
                    }
                    ...
                    VLOG(20) << "Not converting extra smart factor to proj "
                                "factor for lmk"
                                " with id: "
    ...
    gtsam::NonlinearFactorGraph*            new_imu_prior_and_other_factors)
{
    ...
    if (!std::isnan(new_obs.second.uR())) {
            ...
            if (parallax < FLAGS_max_parallax) {
                ...
                new_imu_prior_and_other_factors->push_back(
                    boost::make_shared<
                        ...
                        gtsam::Symbol('l', lmk_id), stereo_cal_, true, true,
                        B_Pose_leftCam_));
            }
            else {
                LOG(ERROR) << "Parallax for lmk_id: " << lmk_id
                           << " is = " << parallax;
            }
        }
        else {
        ...
        // Right pixel has a NAN value for u, use GenericProjectionFactor
        // instead of stereo.
        new_imu_prior_and_other_factors->push_back(
            boost::make_shared<gtsam::GenericProjectionFactor<Pose3, Point3>>(
                gtsam::Point2(new_obs.second.uL(), new_obs.second.v()),
                mono_noise_, gtsam::Symbol('x', new_obs.first),
                gtsam::Symbol('l', lmk_id), mono_cal_, true, true,
                B_Pose_leftCam_));
    }
}
    ...
    LmkIdIsSmart*      lmk_id_is_smart)
{
    ...
    if (std::find(lmk_ids_with_regularity.begin(),
        ...
        // This lmk is involved in a regularity, hence it should be a variable
        // in the factor graph (connected to projection factor).
        VLOG(20) << "Lmk_id = " << lmk_id
        ...
        if (old_smart_factors_it == old_smart_factors_.end()) {
            ...
            SmartStereoFactor::shared_ptr old_factor =
                old_smart_factors_it->second.first;
            // Mind that we are going to add an extra measurement in this
            // iteration so in reality we are checking that the factor number of
            // measurements (without the new measurement) accounts for at least
            // min_num_obs...
            ...
            if (lmk_id_slot == lmk_id_is_smart->end()) {
                ...
                // TODO use an enum instead of bool for lmk_id_is_smart,
                ...
                if (lmk_is_valid) {
                    ...
                    // We cannot allow conversion back from proj to smart
                    // factor. So just check that this is still smart, otherwise
                    // we are stuck.
    ...
    const size_t&                 min_num_of_observations)
{
    ...
    if (!(factor->point().valid())) {
        ...
        if (factor->measured().size() >= min_num_of_observations) {
            VLOG(20) << "Smart factor is valid with: "
                     << factor->measured().size()
    ...
    std::vector<std::pair<Slot, LandmarkId>>* idx_of_point_plane_factors_to_add)
{
    ...
    if (!state_.exists(plane_key)) {
        ...
        // Verify that the plane is going to be fully constrained before adding
        // it. Variables to check whether the new plane is going to be fully
        // constrained or not.
        ...
        for (const LandmarkId& lmk_id : plane_lmk_ids) {
            if (state_.exists(gtsam::Symbol('l', lmk_id)) ||
                ...
                // Lmk id exists either in the state or it is going to be added
                // in the next optimization.
                VLOG(20) << "Lmk id: " << lmk_id
                         << " is in state_ or is going to be added in next "
                            "optimization.";

                const auto& lmk_id_regularity_type =
                    lmk_id_to_regularity_type_map->find(lmk_id);
                if (lmk_id_regularity_type ==
                    lmk_id_to_regularity_type_map->end()) {
                    // Lmk has not been used in a regularity.
                    VLOG(20) << "Lmk id: " << lmk_id
                             << " has not yet been used in a regularity.";

                    // Check that the plane is constrained before adding
                    // factors.
                    if (!is_plane_constrained) {
                        // Still need to check whether the plane is fully
                        // constrained. Add lmk to list of potential
                        // constraints.
                        VLOG(10)
                            << "Checking that plane has enough constraints...";
                        list_of_constraints.push_back(lmk_id);
                    }
                    else {
                        // Act as usual, the plane is already fully constrained,
                        ...
                        // Acknowledge that this lmk has been used in a
                        // regularity.
                    ...
                    if (list_of_constraints.size() >
                        ...
                        // TODO find a way to add initial guess, maybe when
                        // sending the lmk_ids having a regularity we could
                        // regress a plane through it? The mesher should send
                        // the plane!
                        ...
                        // static const gtsam::noiseModel::Diagonal::shared_ptr
                        // prior_noise
                        // =
                        //    gtsam::noiseModel::Diagonal::Sigmas(Vector3(0.5,
                        //    0.5, 0.5));
                        // new_imu_prior_and_other_factors_.push_back(
                        ...
                        // Add the factors for the lmks that we skipped while
                        // checking that the plane is fully constrained.
                        for (const LandmarkId& prev_lmk_id :
                             list_of_constraints) {
                            VLOG(20) << "Adding PointPlaneFactor.";
                            idx_of_point_plane_factors_to_add->push_back(
                                std::make_pair(
                        ...
                        // Clear list of constraints, so that we do not enter in
                        // this condition anymore.
                        list_of_constraints.resize(0);
                    }
                }
                else {
                    LOG(ERROR) << "If this is a new plane, the lmks should not "
                                  "have any "
                                  "regularities...";
                    // And this if condition will prevent multiple regularities
                    // on the same point...
                }
            }
            else {
                ...
                // Erase from map of lmk id to regularity type, since the lmk
                // does not exist anymore, or has never existed. Avoid the world
                // of undefined behaviour by checking that the lmk_id we want to
                // erase is actually there.
        ...
        for (const LandmarkId& lmk_id : plane_lmk_ids) {
            if (state_.exists(gtsam::Symbol('l', lmk_id)) ||
                ...
                if (lmk_id_regularity_type ==
                        lmk_id_to_regularity_type_map->end() ||
                    lmk_id_regularity_type->second !=
                        RegularityType::POINT_PLANE) {
                    // Lmk has not been used in a regularity or is not in a
                    // point plane factor.
                    VLOG(20) << "Lmk id: " << lmk_id
                             << " has not yet been used in a regularity.";

                    // The plane is already in the graph so it must be fully
                    // constrained, keep adding new factors.
                    VLOG(20) << "Adding PointPlaneFactor.";
                    ...
                    // This lmk has already been used in a regularity and is a
                    // point plane factor, avoid duplication of factors.
                    VLOG(20)
                        << "Avoiding duplicated regularity factor for lmk id: "
                        << lmk_id;
                }
            }
            else {
    ...
    gtsam::FactorIndices*  delete_slots)
{
    ...
    for (const Plane& plane : planes) {
        ...
        if (!state_.exists(plane_symbol.key())) {
            // The plane is not in the state, it is probably going to be added
            // in this iteration, so it must be already well constrained and
            // attached to the right lmks. Or it could be that it is not going
            // to be added at all, so we do not need to do anything.
            VLOG(10) << "Plane with id "
        ...
        if (new_values_.exists(plane_symbol.key())) {
            ...
            // this iteration, so it must be already well constrained and
            // attached to the right lmks.
            VLOG(10) << "Plane with id "
                     << gtsam::DefaultKeyFormatter(plane_symbol)
                     << " is in new_values_.";
            i++;
            ...
            if (ppf) {
                ...
                for (const size_t& plane_id : plane_idx_to_clean) {
                    ...
                    if (plane_symbol == ppf->getPlaneKey()) {
                        // We found the plane involved in current
                        // PointPlaneFactor.
                        const LandmarkId& lmk_id =
                            gtsam::Symbol(ppf->getPointKey()).index();
                        const LandmarkIds& plane_lmk_ids =
                            planes.at(plane_id).lmk_ids_;
                        // Try to find this lmk id in the set of lmks of the
                        // plane.
                        if (std::find(plane_lmk_ids.begin(),
                                      plane_lmk_ids.end(),
                                      lmk_id) == plane_lmk_ids.end()) {
                            // We did not find the point in plane's lmks,
                            // therefore it should not be involved in a
                            // regularity anymore, delete this slot. (but I want
                            // to remove the landmark! and all its factors, to
                            // avoid having underconstrained lmks...)
                            VLOG(20) << "Found bad point plane factor on lmk "
                                        "with id: "
                                     << lmk_id;
                            plane_id_to_factor_slots_bad.at(plane_id).push_back(
                                std::make_pair(slot, lmk_id));

                            // Before deleting this slot, we must ensure that
                            // both the plane and the landmark are well
                            // constrained!
                        }
                        else {
                            // Store those factors that we will potentially
                            // keep.
                            plane_id_to_factor_slots_good.at(plane_id)
                                .push_back(std::make_pair(slot, lmk_id));
                        }
                    }
                }
            }
                ...
                for (const size_t& plane_idx : plane_idx_to_clean) {
                    ...
                    if (plane_prior->find(plane_symbol) != plane_prior->end()) {
                        LOG(WARNING)
                            << "Found plane prior for plane: "
                            << gtsam::DefaultKeyFormatter(plane_symbol.key());
                        // Store slot of plane_prior, since we might have to
                        // delete it if the plane has no constraints.
                ...
                for (const size_t& plane_idx : plane_idx_to_clean) {
                    ...
                    if (lcf->find(plane_symbol.key()) != lcf->end()) {
                        VLOG(10)
                            << "Found linear container factor for plane: "
                            << gtsam::DefaultKeyFormatter(plane_symbol.key());
    ...
    for (const size_t& plane_idx : plane_idx_to_clean) {
        ...
        /// delete_slots otherwise delete ALL constraints, both old and new, so
        /// that the plane disappears (take into account priors!). Priors
        /// affecting planes: linear container factor & prior on OrientedPlane3
        ...
        if (total_nr_of_plane_constraints >
            ...
            // We can just delete bad factors, assuming lmks will be well
            // constrained.
            // TODO ensure the lmks are themselves well constrained.
            VLOG(10)
                << "Plane is fully constrained, removing only bad factors.";
            ...
            if (has_plane_a_prior || has_plane_a_linear_factor) {
                ...
                if (FLAGS_use_unstable_plane_removal) {
                    // This should be the correct way to do it, but a bug in
                    // gtsam will make the optimization break.
                    if (total_nr_of_plane_constraints == 0 &&
                        has_plane_a_prior && !has_plane_a_linear_factor) {
                        // Not only the plane is not fully constrained, it has
                        // no constraints at all, and we are going to delete the
                        // bad ones, so plane floating with a plane prior, not
                        // attached to anything else... Delete the prior as
                        // well, to get rid of this plane.
                        LOG(ERROR) << "Plane has no constraints at all, "
                                      "deleting prior as well.";
                    ...
                    // TODO maybe if we are deleting too much constraints, add a
                    // no information factor btw the plane and a lmk!
                    if (total_nr_of_plane_constraints >
                        FLAGS_min_num_of_plane_constraints_to_avoid_seg_fault) {
                        // Delete just the bad factors, since we still have some
                        // factors that won't make the optimizer try to delete
                        // the plane variable, which at the current time breaks
                        // gtsam.
                        ...
                        // Do not delete all factors, otherwise gtsam will
                        // break.
                        VLOG(10) << "Not deleting bad factors attached to "
                                    "plane, or gtsam will "
                                    "break.";
                    }
                }
                ...
                if (FLAGS_use_unstable_plane_removal) {
                    // Delete all factors involving the plane so that iSAM
                    // removes the plane from the optimization.
                    ...
                    // Remove as well the factors that are going to be added in
                    // this iteration.
                    ...
                    // Just add a prior on the plane and remove only bad
                    // factors... Add a prior to the plane.
                    VLOG(10) << "Adding a prior to the plane, delete just the "
                                "bad factors.";
                    gtsam::OrientedPlane3 plane_estimate;
                    CHECK(getEstimateOfKey(state_, plane_symbol.key(),
                                           &plane_estimate));
                    ...
                    new_imu_prior_and_other_factors_.push_back(
                        boost::make_shared<
                            gtsam::PriorFactor<gtsam::OrientedPlane3>>(
    ...
    //  // TODO now the plane could be floating around with a prior attached,
    //  but
        ...
        case 1: {
            ...
            *noise_model_output = gtsam::noiseModel::Robust::Create(
                gtsam::noiseModel::mEstimator::Huber::Create(
                    norm_type_parameter,
                    gtsam::noiseModel::mEstimator::Huber::Scalar),  // Default
                                                                    // is Block
                noise_model_input);
            break;
        }
        case 2: {
            ...
            *noise_model_output = gtsam::noiseModel::Robust::Create(
                gtsam::noiseModel::mEstimator::Tukey::Create(
                    norm_type_parameter,
                    gtsam::noiseModel::mEstimator::Tukey::Scalar),  // Default
                                                                    // is Block
...
void RegularVioBackend::updatePlaneEstimates(std::vector<Plane>* planes)
{
    ...
    for (std::vector<Plane>::iterator plane_it = planes->begin();
        ...
        if (getEstimateOfKey(state_, plane_key, &plane_estimate)) {
            ...
            plane_it->normal_ =
                cv::Point3d(plane_estimate.normal().point3().x(),
            ...
            if (plane_id_to_lmk_id_reg_type_.find(plane_key) !=
                ...
                LOG(WARNING)
                    << "Plane " << gtsam::DefaultKeyFormatter(plane_key) << " "
