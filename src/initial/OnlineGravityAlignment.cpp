    ...
    const bool&      estimate_bias)
{
    ...
    if (estimate_bias) {
        if (!estimateBiasAndUpdateStates(pims_, ahrs_pims_, gyro_bias,
                                         &vi_frames,
    ...
    gtsam::Vector3*             gyro_bias)
{
    ...
    gtsam::SharedNoiseModel noise_init_rot =
        gtsam::noiseModel::Gaussian::Covariance(
            B_Rot_W *
            (FLAGS_rotation_noise_prior * gtsam::Matrix3::Identity()) *
            B_Rot_W.transpose());
    ...
    nfg.push_back(gtsam::PriorFactor<gtsam::Rot3>(
        gtsam::Symbol('R', 0), gtsam::Rot3(vi_frames.at(0).b0Rbk()),
        noise_init_rot));
    ...
    for (int i = 0; i < vi_frames.size(); i++) {
        ...
        nfg.push_back(gtsam::PriorFactor<gtsam::Rot3>(
            gtsam::Symbol('R', frame_id), gtsam::Rot3(frame_i->b0Rbkp1()),
            noise_init_rot));
        // Add AHRS factor between rotations
        nfg.push_back(gtsam::AHRSFactor(
            gtsam::Symbol('R', i), gtsam::Symbol('R', frame_id),
            gtsam::Symbol('b', 0), ahrs_pims.at(i), gtsam::Vector3(0, 0, 0)));
    }
    ...
    gtsam::Velocity3*           init_vel)
{
    ...
    for (int l = 0; l < FLAGS_num_iterations_gravity_refinement; l++) {
        ...
        for (int i = 0; i < vi_frames.size(); i++) {
            ...
            gaussian_graph.add(gtsam::Symbol('V', i), frame_i->A21(),
                               gtsam::Symbol('V', i + 1), frame_i->A22(),
                               gtsam::Symbol('d', 0), A_23_tangent, b_2_tangent,
                               noise);
        }
    ...
    const bool&     use_ahrs_estimator)
{
    ...
    if (!estimateBiasAndUpdateStates(pims_, ahrs_pims_, gyro_bias, &vi_frames,
                                     use_ahrs_estimator)) {
        return false;
    }
    return true;
