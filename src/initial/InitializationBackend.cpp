    ...
    gtsam::NavState*     init_navstate)
{
    ...
    while (!output_frontend.empty()) {
        ...
        inputs_backend.push_back(VIO::make_unique<BackendInput>(
            ...
            init_input_payload.tracker_status_, init_input_payload.pim_,
    ...
    OnlineGravityAlignment initial_alignment(estimated_poses, delta_t_camera,
                                             pims, imu_params_.n_gravity_);
...
{  // TODO: Remove verbosity and use VLOG
    ...
    BOOST_FOREACH (const gtsam::Values::ConstKeyValuePair& key_value,
        ...
        initial_states.push_back(
            initial_values.at<gtsam::Pose3>(key_value.key));
    }
