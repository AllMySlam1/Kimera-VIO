...
class ParallelPlaneRegularFactor
  ...
  protected:
    ...
    /// direction of / the normal of plane2 will have a measured distance of
    /// -2).
  ...
  public:
    ...
    ParallelPlaneRegularFactor(
        ...
        : Base(noiseModel, plane1Key, plane2Key), plane1Key_(plane1Key),
  ...
  private:
    ...
    /// Basically it's to avoid re-defining default parameters in derived
    /// classes. See also Item 37 of Effective_C++.
    virtual Vector
    doEvaluateError(const OrientedPlane3&    plane_1,
                    const OrientedPlane3&    plane_2,
...
class ParallelPlaneRegularBasicFactor : public ParallelPlaneRegularFactor {
  ...
  private:
                                   ...
                                   boost::optional<Matrix&> H_plane_2) const
    {
        ...
        if (H_plane_1) {
            // Jacobian of plane retraction when v = Vector3::Zero(), to
