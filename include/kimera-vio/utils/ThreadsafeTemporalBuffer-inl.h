...
namespace utils {
    ...
    ThreadsafeTemporalBuffer<ValueType,
                             AllocatorType>::ThreadsafeTemporalBuffer()
    ...
    ThreadsafeTemporalBuffer<ValueType, AllocatorType>::
        ThreadsafeTemporalBuffer(Timestamp buffer_length_nanoseconds)
    ...
    template <typename ValueType, typename AllocatorType>
    bool
    ThreadsafeTemporalBuffer<ValueType, AllocatorType>::getNearestValueToTime(
        Timestamp  timestamp,
        ValueType* value) const
    {
        CHECK_NOTNULL(value);
    ...
    template <typename ValueType, typename AllocatorType>
    bool
    ThreadsafeTemporalBuffer<ValueType, AllocatorType>::getValueAtOrBeforeTime(
        Timestamp  timestamp,
        ...
        ValueType* value) const
    {
        ...
        if (!has_exact_match) {
            // has_exact_match could be also false if the buffer is empty, check
            // that. Which would make it_lower_bound a nullptr, and raise an
            // excetption when being dereferenced later.
    ...
    template <typename ValueType, typename AllocatorType>
    bool
    ThreadsafeTemporalBuffer<ValueType, AllocatorType>::getValueAtOrAfterTime(
        Timestamp  timestamp,
        Timestamp* timestamp_of_value,
        ValueType* value) const
    {
            ...
            typename BufferType::const_iterator* it_lower_bound) const
    {
        ...
        // Returns first element with a time that compares not less to
        // timestamp.
    ...
    bool
    ThreadsafeTemporalBuffer<ValueType, AllocatorType>::getValuesBetweenTimes(
        Timestamp           timestamp_lower_ns,
        ...
        const bool          get_lower_bound) const
    {
        ...
        // TODO I don't see why only 100% overlapping query intervals are
        // accepted. Shouldn't it be oldest > higher || latest < lower ? Or at
        // least accept as well when timestamp_lower_ns is smaller than
        ...
        for (; it != values_.end() && it->first < timestamp_higher_ns; ++it) {
            ...
            // lower_bound includes the border so we need to skip them when
            // there are perfect matches, except if the user explicitly asked
            // for it.
        ...
        Timestamp* timestamp_at_value_ns) const
    {
        ...
        // If the lower bound points out of the array, we have to return the
        // last element.
        if (it_lower == values_.end()) {
            typename BufferType::const_iterator it_last =
                std::prev(values_.end());
        ...
        ValueType* value) const
    {
        ...
        return getNearestValueToTime(timestamp, maximum_delta_ns, value,
                                     &timestamp_at_value_ns);
    }

    template <typename ValueType, typename AllocatorType>
    void
    ThreadsafeTemporalBuffer<ValueType, AllocatorType>::removeOutdatedItems()
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
