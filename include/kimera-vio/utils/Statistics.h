...
namespace utils {
    ...
    // A class that has the statistics interface but does nothing. Swapping this
    // in in place of the Statistics class (say with a typedef) eliminates the
    // function calls.
    ...
    class Statistics {
      public:
        ...
        // Writes a csv file, but transposed, each row first element represents
        // the columns headers, and the subsequent values are the data.
