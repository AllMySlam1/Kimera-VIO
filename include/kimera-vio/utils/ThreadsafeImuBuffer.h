...
namespace utils {
    ...
    /// It allows to retrieve a list  of measurements up to a given timestamp.
    /// The data is stored in the order it is added. So make sure to add it in
    /// correct time-wise order.
    class ThreadsafeImuBuffer {
      public:
        ...
        enum class QueryResult {
            ...
            kDataAvailable,
            /// The required data is not (yet) available. The query timestamp is
            /// above
            /// the last IMU sample's time.
            kDataNotYetAvailable,
            /// The queried timestamp lies before the first IMU sample in the
            /// buffer.
            /// This request will never succeed considering chronological
            /// ordering
        ...
        //      getImuDataStrictlyBtwTiemstamps(2, 5, ...) returns elements at
        //      3, 4.
        ...
        /// timestamps. The IMU values get interpolated if the queried timestamp
        /// does not match a measurement.
        /// @param[in] timestamp_from Try to get the IMU measurements from this
        /// time [ns].
        /// @param[in] timestamp_to Try to get the IMU measurements up this time
        /// [ns].
        ...
        QueryResult
        getImuDataInterpolatedBorders(const Timestamp& timestamp_from,
                                      const Timestamp& timestamp_to,
                                      ImuStampS*       imu_timestamps,
                                      ImuAccGyrS*      imu_measurements);

        /// \brief Return a list of the IMU measurements between the specified
        /// timestamps. ONLY the IMU newest value gets interpolated (if the
        /// queried timestamp does not match a measurement, otw no
        /// interpolation). The oldest IMU value is always an actual measurement
        ...
        /// In this case the output matrices will be of size 0.
        QueryResult
        getImuDataInterpolatedUpperBorder(const Timestamp& timestamp_ns_from,
                                          const Timestamp& timestamp_ns_to,
                                          ImuStampS*       imu_timestamps,
                                          ImuAccGyrS*      imu_measurements);
        ...
        // This can be done be checked using the isDataAvailableUpToImpl
        // function.
        void
        interpolateValueAtTimestamp(const Timestamp& timestamp_ns,
      ...
      private:
        ...
        /// Is data available up to this timestamp? Note this function does not
        /// lock the buffers, the caller must hold the lock.
        /// TODO I believe it should be:
        /// a) checks that the requested interval of data is a subset of the
        /// data in
        ...
        ///       ii) Query from timestamp < 2 (e.g. 0) or timestamp to < 2
        ///       (e.g. 1)
        ///           , returns kDataNeverAvailable.
        ///       iii) Query from timestamp 3 to 4, returns kDataAvailable.
        /// And I don't think there is a need to lock any mutex, as buffer_ is
        /// already threadsafe...
        QueryResult
        isDataAvailableUpToImpl(const Timestamp& timestamp_ns_from,
        ...
        typedef ThreadsafeTemporalBuffer<ImuMeasurement, BufferAllocator>
            Buffer;
