...
template <typename T> class ThreadsafeQueueBase {
  public:
     ...
     * @param max_queue_size if the queue is filled with more than
     * max_queue_size messages, it will not push to the queue, and it will wait
     * for a consumer to remove messages.
     * @return false if the queue has been shutdown
     */
    virtual bool pushBlockingIfFull(T      new_value,
                                    size_t max_queue_size = 10u) = 0;
...
template <typename T> class ThreadsafeQueue : public ThreadsafeQueueBase<T> {
  public:
    ...
    /**
     * @brief popBlockingUpToTime Same as Pop blocking, but further returns
     * early if the given duration has passed...
     * @param value Returned value
     * @param duration_ms Time to wait for a msg [in milliseconds]
     * @return Returns false if the queue has been shutdown or if it was
     * timeout.
     */
 ...
 */
template <typename T> class ThreadsafeNullQueue : public ThreadsafeQueue<T> {
  public:
    ...
    virtual bool pushBlockingIfFull(T, size_t)
    {
        return true;
    };
    virtual bool popBlocking(T&) override
    {
        return true;
    }
    virtual std::shared_ptr<T> popBlocking() override
    {
        return nullptr;
    }
...
ThreadsafeQueueBase<T>::ThreadsafeQueueBase(const std::string& queue_id)
    : queue_id_(queue_id), mutex_(), data_queue_(), data_cond_(),
      shutdown_(false)
{
}
...
template <typename T> bool ThreadsafeQueue<T>::push(T new_value)
{
    if (shutdown_)
    ...
    if (queue_size_stats_)
        queue_size_stats_->AddSample(queue_size);
...
template <typename T> bool ThreadsafeQueue<T>::popBlocking(T& value)
{
    std::unique_lock<std::mutex> lk(mutex_);
...
template <typename T> std::shared_ptr<T> ThreadsafeQueue<T>::popBlocking()
{
    std::unique_lock<std::mutex> lk(mutex_);
    data_cond_.wait(lk, [this] { return !data_queue_.empty() || shutdown_; });
    if (shutdown_)
...
template <typename T> bool ThreadsafeQueue<T>::pop(T& value)
{
    if (shutdown_)
        return false;
    std::unique_lock<std::mutex> lk(mutex_);
...
template <typename T> std::shared_ptr<T> ThreadsafeQueue<T>::pop()
{
    if (shutdown_)
        return std::shared_ptr<T>(nullptr);
    std::unique_lock<std::mutex> lk(mutex_);
