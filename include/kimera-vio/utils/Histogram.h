...
class Histogram {
  public:
     ...
     * 8-bit array of the same size as images[i] . The non-zero mask elements
     * mark the array elements counted in the histogram.
     * @param dims Histogram dimensionality that must be positive and not
     * greater than CV_MAX_DIMS (equal to 32 in the current OpenCV version).
     ...
     * each dimension i it is enough to specify the lower (inclusive) boundary
     * \f$L_0\f$ of the 0-th histogram bin and the upper (exclusive) boundary
     * \f$U_{\texttt{histSize}[i]-1}\f$ for the last histogram bin histSize[i]-1
     * . That is, in case of a uniform histogram each of ranges[i] is an array
     * of 2 elements. When the histogram is not uniform ( uniform=false ), then
     * each of ranges[i] contains histSize[i]+1 elements: \f$L_0, U_0=L_1,
     * U_1=L_2, ..., U_{\texttt{histSize[i]}-2}=L_{\texttt{histSize[i]}-1},
     ...
     * cleared in the beginning when it is allocated. This feature enables you
     * to compute a single histogram from several sets of arrays, or to update
     * the histogram in time.
     */
    ...
    struct PeakInfo
    {
        ...
        float  support_ = 0;
        double value_   = 0;
        // < Operator will compare the support (# of points) of one peak vs the
        // other. nothing else. Used for std::max_element.
    ...
    struct PeakInfo2D
    {
        PeakInfo2D(const cv::Point& pos,
            ...
            : pos_(pos), support_(support), x_value_(x_value), y_value_(y_value)
        {
        }
        cv::Point pos_;
        double    support_ = 0;
    ...
    // the number of peaks found. Min_support specifies the minimal number of
    // votes for a bin to be considered a peak.
    std::vector<PeakInfo>
    getLocalMaximum1D(const cv::Size& smooth_size       = cv::Size(9, 9),
                      int             neighbor_size     = 3,
                      float           peak_per          = 0.5,
    ...
    // then makes the difference and checks centers of countours to get
    // maximums.
  ...
  private:
     ...
     */
    PeakInfo
    peakInfo(int pos, int left_size, int right_size, float value) const;
    ...
    void
    visualizeHistogram2DWithPeaks(cv::Mat                        histogram,
