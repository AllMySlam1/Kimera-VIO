...
class UtilsOpenCV {
  public:
    ...
    {
        return cv::Point3d(unit3.point3().x(), unit3.point3().y(),
                           unit3.point3().z());
    }
    ...
    static gtsam::Pose3
    poseVectorToGtsamPose3(const std::vector<double>& vector_pose);
    ...
    static gtsam::Pose3
    openGvTfToGtsamPose3(const opengv::transformation_t& RT);
    ...
    ComputeRotationAndTranslationErrors(const gtsam::Pose3& expectedPose,
    ...
    /* ------------------------------------------------------------------------
     */
    // reorder block entries of covariance from state: [bias, vel, pose] to
    // [pose vel bias]
    ...
    cv::Scalar getColorFromKeypointStatus(const KeypointStatus& kpt_status);

    /* ------------------------------------------------------------------------
     */
    static cv::Mat
    DrawCircles(const cv::Mat              img,
     ...
     */
    static void
    DrawCornersMatchesOneByOne(const cv::Mat                   img1,
    ...
    static std::vector<std::pair<KeypointCV, double>>
    FindHighIntensityInTriangle(const cv::Mat    img,
...
struct Plane
{
  ...
  public:
    Plane(const gtsam::Symbol& plane_symbol,
        ...
        : plane_symbol_(plane_symbol), normal_(normal), distance_(distance),
          lmk_ids_(lmk_ids)
    {
                        ...
                        const double& distance_tolerance) const
    {
        return (isNormalStrictlyEqual(rhs.normal_, normal_, normal_tolerance) &&
                // TODO implement a better test for distance tolerance... as it
                // can be large for small normal difference and distances are
                // big.
                isPlaneDistanceStrictlyEqual(rhs.distance_, distance_,
                                             distance_tolerance)) ||
               // Check also the other possible case.
               (isNormalStrictlyEqual(rhs.normal_, -normal_,
                                      normal_tolerance) &&
                isPlaneDistanceStrictlyEqual(rhs.distance_, -distance_,
                                             distance_tolerance));
    }
  ...
  private:
                       ...
                       const double&         tolerance) const
    {
        ...
        // Dot product should be close to 1 or -1 if axis is aligned with
        // normal.
