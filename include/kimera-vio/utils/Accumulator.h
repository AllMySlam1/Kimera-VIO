...
namespace utils {
    ...
    // If the window size is set to -1, the vector will grow infinitely,
    // otherwise, the vector has a fixed size.
    template <typename SampleType, typename SumType, int WindowSize>
    class Accumulator {
      public:
        Accumulator()
            : sample_index_(0), total_samples_(0), sum_(0), window_sum_(0),
              min_(std::numeric_limits<SampleType>::max()),
              max_(std::numeric_limits<SampleType>::lowest()), most_recent_(0)
        {
        ...
        /* ------------------------------------------------------------------------
         */
        const std::vector<SampleType>& GetAllSamples() const
        {
            return samples_;
        }
