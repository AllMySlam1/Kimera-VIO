...
namespace utils {
    ...
    class ThreadsafeTemporalBuffer {
      public:
        typedef std::
            map<Timestamp, ValueType, std::less<Timestamp>, AllocatorType>
                BufferType;
        ...
        bool getNearestValueToTime(Timestamp  timestamp_ns,
        ...
        // Get all values between the two specified timestamps excluding the
        // border values. Example: content: 2 3 4 5
        ...
        //          getValuesBetweenTimes(2, 5, ...) returns elements at 2,
        //          3, 4.
        ...
        // The container is exposed so we can iterate over the values in a
        // linear fashion. The container is not locked inside this method so
        // call lockContainer()/unlockContainer() when accessing this.
        ...
        inline bool operator==(const ThreadsafeTemporalBuffer& other) const
        {
            return values_ == other.values_ &&
                   buffer_length_nanoseconds_ ==
                       other.buffer_length_nanoseconds_;
        }
