...
namespace UtilsNumerical {
    ...
    template <class T1, class T2> size_t hashPair(const std::pair<T1, T2>& p)
    {
        size_t seed = 0u;
    ...
    //! If you want order invariance just sort your values before
    //! accessing/checking Used for hashing faces in a mesh.
    ...
    template <typename T> void VectorUnique(std::vector<T>& v)
    {
    ...
    // Generate random float using random number generator between -sigma and
    // sigma
