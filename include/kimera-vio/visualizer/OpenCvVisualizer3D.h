...
class OpenCvVisualizer3D : public Visualizer3D {
  public:
     ...
     */
    inline void
    registerMesh3dVizProperties(Mesh3dVizPropertiesSetterCallback cb)
    {
  ...
  public:
    // Visualization calls are public in case the user wants to manually
    // visualize things, instead of running spinOnce and do it automatically.
     ...
     * @param id Optional string id in case you want to display multiple 3D
     * meshes in the same visualization window
  ...
  private:
    ...
    static cv::Mat
    visualizeMesh2D(const std::vector<cv::Vec6f>& triangulation2D,
                    const cv::Mat&                img,
                    const KeypointsCV& extra_keypoints = KeypointsCV());

    //! Visualize 2d mesh.
    static cv::Mat
    visualizeMesh2DStereo(const std::vector<cv::Vec6f>& triangulation_2D,
  ...
  private:
    ...
    /// it is in no cluster. [in] clusters: a set of triangle clusters. The ids
    /// of the triangles must
    ...
    // @param wait_time Amount of time in milliseconds for the event loop to
    // keep running.
    ...
    void drawLinearContainerFactor(const gtsam::LinearContainerFactor& lcf,
                                   ...
                                   WidgetsMap*         widgets_map);
    void
    drawVelocityPrior(const gtsam::PriorFactor<gtsam::Vector3>& velocity_prior,
                      const gtsam::Values&                      state,
                      WidgetsMap*                               widgets_map);
  ...
  private:
    ...
    const cv::Matx33d K_ = {458.0, 0.0, 360.0, 0.0, 458.0,
                            240.0, 0.0, 0.0,   1.0};
    ...
    cv::viz::Color cam_with_linear_prior_frustum_color_ =
        cv::viz::Color::pink();
    double         cam_with_pose_prior_frustum_scale_ = 0.20;
    cv::viz::Color cam_with_pose_prior_frustum_color_ =
        cv::viz::Color::yellow();
