...
class StereoCamera {
  ...
  public:
     ...
     * @param lmk 3D landmark resulting from the backProjection of the 2D
     * keypoint
     */
     ...
     * @param disparity_img
     * Input single-channel 8-bit unsigned, 16-bit signed, 32-bit signed or
     * 32-bit floating-point disparity image. If 16-bit signed format is used,
     * the values are assumed to have no fractional bits.
     * @param depth
     * Output 3-channel floating-point image of the same size as disparity .
     * Each element of _3dImage(u, v) contains 3D coordinates of the keypoint
     * (u, v) computed from the disparity map. WARNING The output 3D landmarks
     * (x, y, z) are given in camera coordinates!! rather than body coordinates!
     */
    ...
    /**
     * @brief getBodyPoseLeftCamRect Get left camera pose after rectification
    ...
    inline cv::Rect getROI1() const
    {
        return ROI1_;
    }
    inline cv::Rect getROI2() const
    {
        return ROI2_;
    }
    ...
    inline cv::Mat getP1() const
    {
        return P1_;
    }

    inline cv::Mat getP2() const
    {
        return P2_;
    }

    inline cv::Mat getR1() const
    {
        return R1_;
    }

    inline cv::Mat getR2() const
    {
        return R2_;
    }

    inline cv::Mat getQ() const
    {
        return Q_;
    }

    inline Baseline getBaseline() const
    {
        return stereo_baseline_;
    }
     ...
     */
    void
    undistortRectifyLeftKeypoints(const KeypointsCV& keypoints,
                                  StatusKeypointsCV* status_keypoints) const;

    void
    distortUnrectifyRightKeypoints(const StatusKeypointsCV& status_keypoints,
     ...
     */
    static void
    computeRectificationParameters(const CameraParams& left_cam_params,
                                   const CameraParams& right_cam_params,
                                   cv::Mat*            R1,
                                   cv::Mat*            R2,
                                   cv::Mat*            P1,
