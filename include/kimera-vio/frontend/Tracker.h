...
class Tracker {
  ...
  public:
    ...
    static void
    findMatchingStereoKeypoints(const StereoFrame& ref_stereoFrame,
    ...
    static std::pair<Vector3, Matrix3>
    getPoint3AndCovariance(const StereoFrame&              stereoFrame,
  ...
  private:
    ...
    // StereoCamera object for the camera we are tracking. We use the left
    // camera.
    // TODO(marcus): this should be general to all camera types!
    Camera::ConstPtr camera_;

    // Feature tracking uses the optical flow predictor to have a better guess
    // of where the features moved from frame to frame.
