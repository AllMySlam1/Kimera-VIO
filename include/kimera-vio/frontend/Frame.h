...
class Frame : public PipelinePayload {
  public:
    ...
    Frame(const FrameId&      id,
          ...
          const cv::Mat&      img)
        : PipelinePayload(timestamp), id_(id), cam_param_(cam_param), img_(img),
          isKeyframe_(false), keypoints_(), keypoints_undistorted_(), scores_(),
          landmarks_(), landmarks_age_(), versors_(), descriptors_()
    {
    }
    ...
    Frame(const Frame& frame)
        : PipelinePayload(frame.timestamp_), id_(frame.id_),
          cam_param_(frame.cam_param_), img_(frame.img_),
          isKeyframe_(frame.isKeyframe_), keypoints_(frame.keypoints_),
          keypoints_undistorted_(frame.keypoints_undistorted_),
          scores_(frame.scores_), landmarks_(frame.landmarks_),
          landmarks_age_(frame.landmarks_age_), versors_(frame.versors_),
  ...
  public:
                                         ...
                                         size_t* idx_in_keypoints = nullptr)
    {
        ...
        for (size_t i = 0; i < keypoints.size(); i++) {
            ...
            // landmark id of the keypoint and return the index of it at
            // keypoints_.
