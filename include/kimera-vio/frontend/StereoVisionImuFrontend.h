...
class StereoVisionImuFrontend : public VisionImuFrontend {
  ...
  private:
    ...
    /**
     * @brief nominalSpin SpinOnce used when in nominal mode after
    ...
    // structures.
    StereoFrontendOutput::UniquePtr
    bootstrapSpinStereo(StereoFrontendInputPayload::UniquePtr&& input);

    /* ------------------------------------------------------------------------
     */
    // Used when in nominal mode after init, operates on Stereo
    // Frontend-specific structures.
    StereoFrontendOutput::UniquePtr
    nominalSpinStereo(StereoFrontendInputPayload::UniquePtr&& input);

    /* ------------------------------------------------------------------------
     */
    // Frontend main function.
    StatusStereoMeasurementsPtr
    processStereoFrame(const StereoFrame& cur_frame,
  ...
  private:
    ...
    // We use this to calculate the rotation btw reference frame and current
    // frame Whenever a keyframe is created, we reset it to identity.
    ...
    // This is not const as for debugging we want to redirect the image save
    // path where we like
