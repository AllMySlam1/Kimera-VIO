...
class UndistorterRectifier {
  ...
  public:
     ...
     */
    static void
    UndistortRectifyKeypoints(const KeypointsCV&       keypoints,
    ...
    UndistortKeypointAndGetVersor(const KeypointCV&        keypoint,
                                  const CameraParams&      cam_param,
                                  boost::optional<cv::Mat> R = boost::none);

    /**
     * @brief undistortRectifyImage Given distorted (and optionally
     * non-rectified) image, returns a distortion-free rectified one.
     ...
     *
     * It uses the internal camera parameters, and the optional R_ and P_
     * matrices as provided at construction (by cv::stereoRectify). If R_/P_ are
     * identity no rectification is performed and the resulting keypoints are in
  ...
  protected:
     ...
     * of a stereo camera, it is the output of stereo rectification
     * (cv::stereoRectify)
     *
