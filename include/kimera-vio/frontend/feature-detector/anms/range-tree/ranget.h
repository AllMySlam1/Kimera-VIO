...
template <class point, class data> class rangetree {
  public:
    ...
    std::vector<data*>*
    search(point xmin, point xmax, point ymin, point ymax) const
    {
        if (!init)
            return NULL;
  ...
  private:
    ...
    u32
    binarynext(const pty* const arr, const u32 ycount, const point goal) const
    {
        if (ycount == 0)
            return 0;
...
#ifdef LR_VISUALIZE
    void
    visualize(const node* const n, graph_t* const g, Agnode_t* const n1) const
    {
        Agnode_t* temp;
        Agedge_t* e;
