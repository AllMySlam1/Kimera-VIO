...
class VisionImuFrontendFactory {
  public:
    ...
    static VisionImuFrontend::UniquePtr
    createFrontend(const FrontendType&     frontend_type,
                   ...
                   bool                    log_output)
    {
        switch (frontend_type) {
            case FrontendType::kMonoImu: {
                return VIO::make_unique<MonoVisionImuFrontend>(
                    imu_params, imu_initial_bias, frontend_params, camera,
                    display_queue, log_output);
            }
                   ...
                   bool                          log_output)
    {
        switch (frontend_type) {
            ...
            case FrontendType::kStereoImu: {
                return VIO::make_unique<StereoVisionImuFrontend>(
                    imu_params, imu_initial_bias, frontend_params,
                    stereo_camera, display_queue, log_output);
            }
            default: {
                LOG(FATAL) << "Requested frontend type is not supported.\n"
                           << "Currently supported frontend types:\n"
