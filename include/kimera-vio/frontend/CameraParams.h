...
class CameraParams : public PipelineParams {
  public:
    ...
    CameraParams()
        : PipelineParams("Camera Parameters"), camera_id_(), camera_model_(),
          intrinsics_(), K_(), body_Pose_cam_(), frame_rate_(), image_size_(),
          distortion_model_(), distortion_coeff_(), distortion_coeff_mat_()
    {
    }
  ...
  public:
     ...
     */
    static const DistortionModel
    stringToDistortion(const std::string& distortion_model,
                       const std::string& camera_model);

  private:
    void        parseDistortion(const YamlParser& yaml_parser);
    static void parseImgSize(const YamlParser& yaml_parser,
                             cv::Size*         image_size);
    static void parseFrameRate(const YamlParser& yaml_parser,
                               double*           frame_rate);
