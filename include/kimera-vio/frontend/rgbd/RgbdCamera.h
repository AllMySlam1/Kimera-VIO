                                 ...
                                 cv::Mat*         colors)
    {
        ...
        if (depth_type == CV_16UC1) {
            return convert<uint16_t>(rgbd_frame.intensity_img_->img_,
                                     ...
                                     cam_params_.intrinsics_, depth_factor_,
                                     cloud, colors);
        }
        else if (depth_type == CV_32FC1) {
            return convert<float>(
                rgbd_frame.intensity_img_->img_,
                rgbd_frame.depth_img_->depth_img_, cam_params_.intrinsics_,
                static_cast<float>(depth_factor_), cloud, colors);
        }
        else {
