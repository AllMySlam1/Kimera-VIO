...
class RgbdDataProviderModule : public MonoDataProviderModule {
  public:
    ...
    inline void
    fillDepthFrameQueueBlockingIfFull(DepthFrame::UniquePtr depth_frame)
    {
        CHECK(depth_frame);
  ...
  protected:
    ...
    // Any Rgbd frames that appear before the first IMU packet will be
    // discarded. If a rgbd frames appears after another rgbd pair with no IMU
    // packets in between, it will be discarded. The first valid pair is used as
    // a timing fencepost and is not published.
