...
class DataProviderInterface {
  public:
     ...
     * NOTE: this is only setting the shutdown_ flag to any class inheriting
     * from this one will have to implement the logic around the usage of
     * shutdown_! See example usage in other DataProvider classes. Other classes
     * might want to override the simple behavior of shutdown();
     */
    virtual void shutdown();

    // Register a callback function for IMU data
    inline void
    registerImuSingleCallback(const ImuSingleInputCallback& callback)
    {
    ...
    inline void
    registerDepthFrameCallback(const DepthFrameInputCallback& callback)
    {
        depth_frame_callback_ = callback;
    }

  protected:
    // Vio callbacks. These functions should be called once data is available
    // for processing.
