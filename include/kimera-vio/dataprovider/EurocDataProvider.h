...
class EurocDataProvider : public DataProviderInterface {
  ...
  public:
    /**
     * @brief spin Spins the dataset until it finishes. If set in sequential
     * mode, it will return each time a frame is sent. In parallel mode, it will
  ...
  protected:
     ...
     * does not need to be called by the user. Left in public for
     * experimentation.
     */
    void parse();
     ...
     * @return if k is larger than the number of frames, returns false, otw
     * true.
     */
    ...
    gtsam::Pose3
    getGroundTruthRelativePose(const Timestamp& previousTimestamp,
    ...
    getInitializationPerformance(const std::vector<Timestamp>&    timestamps,
    ...
    bool
    sanityCheckCamTimestamps(const CameraImageLists::ImgLists& left_img_lists,
