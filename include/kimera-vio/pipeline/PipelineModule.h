...
class PipelineModuleBase {
  ...
  public:
     ...
     * when done (this is typically a while(!shutdown()) loop, see
     * PipelineModule derived class.
  ...
  protected:
    ...
    // (kind of like visitor pattern), and use the queue synchronizer base
    // class.
    /**
...
class PipelineModule : public PipelineModuleBase {
  public:
    ...
    //! The output is instead a shared ptr, since many users might need the
    //! output
     ...
     * the input is taken from an input queue and processed into an output
     * packet which is sent to the output queue. If the module returns a
     * nullptr, then we don't push to the output queue to save computation time.
    ...
    bool spin() override
    {
        ...
        while (!shutdown_) {
            ...
            if (input) {
                ...
                // Transfer the ownership of input to the actual pipeline
                // module. From this point on, you cannot use input, since
                // spinOnce owns it.
                ...
                if (output) {
                    ...
                    if (!pushOutputPacket(std::move(output))) {
                        ...
                        VLOG(2)
                            << "Module: " << name_id_ << " - Pushed output.";
                    }
                }
                else {
                    VLOG(1) << "Module: " << name_id_
                            << "  - Skipped sending an output.";
  ...
  protected:
     ...
     * when spinning. The user must implement this to feed input payloads to the
     * spinOnce.
     * The typical usage of this function just pops from a threadsafe queue that
     * contains the input packets to be processed. Alternatively, one may
     * consider synchronizing different queues and generating a custom packet.
     * @param[out] input_packet Parameter to be filled that is then used by the
     * pipeline module's specific spinOnce.
     * @return a boolean indicating whether the generation of the input packet
     * was successful.
     */
    ...
    /**
     * @brief pushOutputPacket Sends the output of the module to other
     * interested parties, potentially other pipeline modules. The typical use
     * case would be to just push to a threadsafe output queue the newly
     * generated output. Alternatively, one may override this function to send
     * the output to multiple registered queues or callbacks.
     ...
     * output (for example the data provider module does this, since it just
     * syncs input and forwards it to the output).
...
class MIMOPipelineModule : public PipelineModule<Input, Output> {
  ...
  protected:
    /**
     * @brief pushOutputPacket Sends the output of the module to other
     * interested parties, potentially other pipeline modules. Just push to a
     * threadsafe output queue the newly generated output.
     * @param[out] output_packet  Parameter to be sent to others
     * @return boolean indicating whether the push was successful or not.
     */
    bool
    pushOutputPacket(typename PIO::OutputUniquePtr output_packet) const override
    {
        ...
        static constexpr auto kTimeLimitCallbacks =
...
class MISOPipelineModule : public MIMOPipelineModule<Input, Output> {
  ...
  protected:
    ...
    {
        return output_queue_ ? output_queue_->push(std::move(output_packet)) :
                               true;
    }
...
class SISOPipelineModule : public MISOPipelineModule<Input, Output> {
  public:
    ...
    SISOPipelineModule(InputQueue*        input_queue,
                       ...
                       const std::string& name_id,
                       const bool&        parallel_run)
        : MISOPipelineModule<Input, Output>(output_queue,
                                            name_id,
                                            parallel_run),
          input_queue_(input_queue)
    {
