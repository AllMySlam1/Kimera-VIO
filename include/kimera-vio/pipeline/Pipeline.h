...
class Pipeline {
  ...
  public:
     ...
     * if working nominally (it does not return unless shutdown in parallel
     * mode).
     */
    virtual bool spin();
    ...
    /**
     * @brief printStatus Returns a string with useful information to monitor
     * the status of the pipeline, in particular, whether the pipeline's modules
     * are working and if their queues are filled.
    ...
    //! Register external callback to be called when the VIO pipeline shuts
    //! down.
    virtual void
    registerShutdownCallback(const ShutdownPipelineCallback& callback)
    {
  ...
  protected:
    ...
    virtual void setDeterministicPipeline() const
    {
        srand(0);
    }
    ...
    //! Shutdown for in case Backend fails (this is done for a graceful
    //! shutdown).
    ...
    inline void
    registerMesherOutputCallback(const MesherModule::OutputCallback& callback)
    {
        if (mesher_module_) {
    ...
    inline void
    registerLcdOutputCallback(const LcdModule::OutputCallback& callback)
    {
        if (lcd_module_) {
  ...
  protected:
    ...
    // Pipeline Modules
    // TODO(Toni) this should go to another class to avoid not having
    // copy-ctor...
    //! Frontend.
