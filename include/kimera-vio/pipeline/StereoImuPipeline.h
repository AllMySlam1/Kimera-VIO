...
class StereoImuPipeline : public Pipeline {
  ...
  public:
    inline void fillRightFrameQueue(Frame::UniquePtr right_frame)
    {
        ...
        // TODO(marcus): this is not a good solution. The problem is the above
        // code doesn't work in online/parallel because other threads are
        // accessing data_provider_module_ when it's been temporarily released
        // to the stereo version. Checks fail for that reason. This fix is
        // really bad because it totally bypasses the rules of unique_ptr
