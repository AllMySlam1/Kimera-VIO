...
class MeshOptimization {
  ...
  public:
     ...
     */
    virtual MeshOptimizationOutput::UniquePtr
    spinOnce(const MeshOptimizationInput& input);

    static void
    draw2dMeshOnImg(const Mesh2D&         mesh_2d,
                    cv::Mat*              img,
  ...
  public:
    ...
    /** Faster version (should be faster using parallelization and
     * vectorization) both in hardware (GPUs) or in software...
     *
    ...
    static cv::Point2f
    generatePixelFromLandmarkGivenCamera(const cv::Point3f&    lmk,
    ...
    static float
    sign(const cv::Point2f& p1, const cv::Point2f& p2, const cv::Point2f& p3);
  ...
  private:
    ...
    solveOptimalMesh(const cv::Mat& noisy_point_cloud,
  ...
  private:
    ...
    //! Mesh count: just for visualization to change the ids of the 3d mesh
    //! widget
