...
struct MesherInput : public PipelinePayload
{
  public:
    ...
    MesherInput(const Timestamp&                 timestamp,
        ...
        : PipelinePayload(timestamp), frontend_output_(frontend_payload),
...
struct MesherOutput : public PipelinePayload
{
  public:
    ...
    explicit MesherOutput(const Timestamp& timestamp)
        : PipelinePayload(timestamp), mesh_2d_(3), mesh_3d_(3),
          mesh_2d_for_viz_()
    {
    }
    ...
    MesherOutput(
        ...
        : PipelinePayload(timestamp), mesh_2d_(std::move(mesh_2d)),
          mesh_3d_(std::move(mesh_3d)), mesh_2d_for_viz_(mesh_2d_for_viz),
    ...
    explicit MesherOutput(const MesherOutput::Ptr& in)
        : PipelinePayload(in ? in->timestamp_ : Timestamp()), mesh_2d_(3),
          mesh_3d_(3),
          // yet another copy...
