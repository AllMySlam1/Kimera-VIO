  ...
  public:
    ...
    static cv::Mat CreateMesh3D_MapPointId(cv::Mat pointsMatcv)
    {
        ...
        for (size_t i = 0; i < pointsMatcv.rows; i++)
            points.push_back(Point_cgal(double(data[i].x), double(data[i].y),
                                        double(data[i].z)));

        std::cerr << "read points: " << points.size() << " points."
                  << std::endl;
        ...
        // ...) where n is the number of points in the polygon, and id is a
        // zero-offset index into an associated cloud.
        cv::Mat polygon(0, 1, CV_32SC1);
        for (auto it = reconstruct.facets_begin();
