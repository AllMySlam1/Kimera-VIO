...
template <typename VertexPosition = cv::Point3f> class Mesh {
  public:
  ...
  public:
    template <typename PositionType = cv::Point3f> struct Vertex
    {
      public:
        Vertex()
            : lmk_id_(-1), vertex_position_(), vertex_normal_(),
        ...
        Vertex(const LandmarkId&     lmk_id,
            ...
            : lmk_id_(lmk_id), vertex_position_(vertex_position),
              vertex_normal_(vertex_normal), vertex_color_(vertex_color)
        {
        }
  ...
  public:
    ...
    {
        return static_cast<size_t>(polygons_mesh_.rows /
                                   (polygon_dimension_ + 1));
    }
                                              ...
                                              const cv::Mat&            tf_3_2)
{
    ...
    // The important point is that we re-use the lmk ids, rather than
    // position...
    return Mesh3D::VertexType(vtx_2d.getLmkId(), vtx_position_3d,
