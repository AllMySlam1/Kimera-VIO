...
class Mesher {
  ...
  public:
    ...
    // visualization. It also returns a mesh_2d which represents the
    // triangulation in the 2d image.
    //
     ...
     * later (Note: the function is not const bcs serialize needs to be
     * non-const) But we do not modify the meshes.
     */
    ...
    static std::vector<cv::Vec6f>
    createMesh2D(const Frame&               frame,
    ...
    static void createMesh2D(const KeypointsCV& keypoints, Mesh2D* mesh_2d)
    {
        ...
        for (size_t i = 0u; i < tri.size(); i++) {
            ...
            for (size_t j = 0u; j < triangle_2d.rows / 2u; j++) {
                const cv::Point2f vtx(triangle_2d[j * 2u],
                                      triangle_2d[j * 2u + 1u]);
                polygon_2d.push_back(
                    Mesh2D::VertexType(tri_vtx_indices[j], vtx));
            }
     ...
     */
    static std::vector<cv::Vec6f>
    computeDelaunayTriangulation(const KeypointsCV& keypoints,
  ...
  private:
    ...
    // compute ratio between largest side and smallest side (how elongated it
    // is).
    ...
    // The 3D mesh is constructed by finding the 3D landmark corresponding to
    // the pixel in the 2d mesh. The correspondence is found using the frame
    // parameter. The 3D mesh contains, at any given time, only points that are
    // in points_with_id_map.
    ...
    /* ------------------------------------------------------------------------
     */
    // Calculate normal of a triangle, and return whether it was possible or
    // not. Calculating the normal of aligned points in 3D is not possible...
    ...
    /* ------------------------------------------------------------------------
     */
    // Clusters normals perpendicular to an axis. Given an axis, a set of
    // normals and a tolerance. The result is a vector of indices of the given
    // set of normals that are in the cluster.
    void
    clusterNormalsPerpendicularToAxis(const cv::Point3f&              axis,
                                      const std::vector<cv::Point3f>& normals,
                                      const double&                   tolerance,
    ...
    /* --------------------------------------------------------------------------
     */
    // Update plane lmk ids field, by looping over the mesh and stoting lmk ids
    // of the vertices of the polygons that are close to the plane. It will
    // append lmk ids to the ones already present in the plane.
    void
    updatePlanesLmkIdsFromMesh(std::vector<Plane>*    planes,
    ...
    // It can either associate a polygon only once to the first plane it
    // matches, or it can associate to multiple planes, depending on the flag
    // passed.
    ...
    // And walls perpendicular to the ground, using a cv::Mat which is expected
    // to be a cv::Mat walls (0, 0, CV_32FC2), with first channel being theta
    // (yaw angle of the wall) and the second channel the distance of it.
    ...
    // Extract lmk ids from triangle cluster.
    void
    extractLmkIdsFromTriangleCluster(const TriangleCluster& triangle_cluster,
                                     const PointsWithIdMap& points_with_id_vio,
                                     LandmarkIds*           lmk_ids) const;
     ...
     * @param vtx_indices Returns each vertex id of the vertices of the
     * triangles in order
     * @return
     */
    static std::vector<cv::Vec6f>
    createMesh2dImpl(const cv::Size&    img_size,
                     const KeypointsCV& keypoints_to_triangulate,
                     MeshIndices*       vtx_indices = nullptr);

    static void
    createMesh2dVIO(std::vector<cv::Vec6f>*            triangulation_2D,
                    const LandmarkIds&                 landmarks,
                    const std::vector<KeypointStatus>& keypoints_status,
                    const KeypointsCV&                 keypoints,
