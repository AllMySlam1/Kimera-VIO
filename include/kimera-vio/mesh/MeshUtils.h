...
{
    // Check both for disparities explicitly marked as invalid (where OpenCV
    // maps pt.z to MISSING_Z) and zero disparities (point mapped to infinity).
 ...
 */
inline float
edgeFunction(const KeypointCV& a, const KeypointCV& b, const KeypointCV& c)
{
                                   ...
                                   BaryCoord*        w2)
{
    ...
    *w2 = edgeFunction(v0, v1, p);
    ...
    if (!(has_neg && has_pos)) {
        // barycentric coordinates are the areas of the sub-triangles divided by
...
inline cv::viz::Color rainbowColorMap(double h)
{
    ...
    switch (i) {
        case 6:
        case 0: color = cv::Scalar(255 * m, 255 * n, 255 * v, 255); break;
        case 1: color = cv::Scalar(255 * m, 255 * v, 255 * n, 255); break;
        case 2: color = cv::Scalar(255 * n, 255 * v, 255 * m, 255); break;
        case 3: color = cv::Scalar(255 * v, 255 * n, 255 * m, 255); break;
        case 4: color = cv::Scalar(255 * v, 255 * m, 255 * n, 255); break;
        case 5: color = cv::Scalar(255 * n, 255 * m, 255 * v, 255); break;
        default: color = cv::Scalar(127, 127, 255, 255); break;
    }
