  ...
  public:
        ...
        const gtsam::NonlinearFactorGraph& graph) const
    {
        for (const gtsam::Values::const_iterator& key_value = values_.begin();
             key_value != values_.end(); ++key_value) {
  ...
  public:
                      ...
                      const gtsam::PreintegratedCombinedMeasurements& pim)
    {
        imu_factors_.push_back(boost::make_shared<gtsam::CombinedImuFactor>(
            ...
            gtsam::Symbol(kSymbolImuBiasKey, to_id), pim));
    }
                      ...
                      const gtsam::PreintegratedImuMeasurements& pim)
    {
        imu_factors_.push_back(
            boost::make_shared<gtsam::PreintegratedImuMeasurements>(
                ...
                gtsam::Symbol(kSymbolPoseKey, to_id),
                gtsam::Symbol(kSymbolVelocityKey, to_id),
                gtsam::Symbol(kSymbolImuBiasKey, from_id), pim));
    }

    void
    addConstantImuBiasFactor(const FrameId&                      from_id,
                             const FrameId&                      to_id,
    ...
    void
    updateSmartFactorsSlots(const gtsam::FactorIndices& new_factors_indices)
    {
        // Since the first added factors where the smart ones, and since ISAM2
        // keeps a 1-to-1 correspondence between newFactorsIndices and the new
        // factors passed, we can simply iterate over the number of smart
        // factors and update the delete slot ids.
    ...
    /**
     * @brief addZeroVelocityPrior Add no motion factors in case of low
     * disparity.
     * @param frame_id
     */
                          ...
                          const gtsam::SharedNoiseModel& noise_model)
    {
        between_factors_.push_back(
            boost::make_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
                ...
                from_id_POSE_to_id, noise_model));
    }
    ...
    /**
     * @brief cleanFactorsToDeleteIndices Cleans the books containing the
     * indices of the smart factors to be removed in the graph. You should call
     * this after an ISAM2 update.
     */
    void cleanFactorsToDeleteIndices()
    {
        smart_factors_to_delete_.clear();
    }
...
class FactorGraphBuilder {
  ...
  public:
    ...
    FactorGraphManager& getGraphManager()
    {
