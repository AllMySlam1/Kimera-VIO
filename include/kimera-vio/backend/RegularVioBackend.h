...
class RegularVioBackend : public VioBackend {
  ...
  private:
    ...
    // We need to make a shared queue with the MeshSegmenter which will push
    // plane hypothesis while the Backend pulls them.
  ...
  private:
     ...
     */
    void
    updateExistingSmartFactor(const LandmarkId&                       lmk_id,
                              const std::pair<FrameId, StereoPoint2>& new_obs,
     ...
     */
    virtual void
    deleteLmkFromExtraStructures(const LandmarkId& lmk_id) override;
     ...
     */
    void
    fillDeleteSlots(const std::vector<std::pair<Slot, LandmarkId>>&
                                              point_plane_factor_slots,
                    LmkIdToRegularityTypeMap* lmk_id_to_regularity_type_map,
                    gtsam::FactorIndices*     delete_slots);
    ...
    /* ------------------------------------------------------------------------
     */
    // Update plane normal and distance if the plane could be found in the
    // state. Otherwise, erase the plane.
