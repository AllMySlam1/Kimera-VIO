...
class VioBackend {
  ...
  public:
    ...
    // Register (and trigger!) callback that will be called as soon as the
    // Backend comes up with a new IMU bias update. The callback is also
    // triggered in this function to update the imu bias for the callee of this
    // function.
    ...
    bool initializeFromGt(const BackendInput& input)
    {
        ...
        CHECK(
            !backend_params_.initial_ground_truth_state_.equals(VioNavState()))
    ...
    bool initializeFromIMU(const BackendInput& input)
    {
        ...
        const VioNavState& initial_state_estimate =
            InitializationFromImu::getInitialStateEstimate(
                input.imu_acc_gyrs_, imu_params_.n_gravity_,
  ...
  private:
    ...
    void
    cleanCheiralityLmk(const gtsam::Symbol&         lmk_symbol,
    ...
    void
    findSlotsOfFactorsWithKey(const gtsam::Key&                  key,
                              const gtsam::NonlinearFactorGraph& graph,
    ...
    void
    setFactorsParams(const BackendParams&                vio_params,
    ...
    void
    setNoMotionFactorsParams(const double&            rotation_sigma,
    ...
    void
    printPointPlaneFactor(boost::shared_ptr<gtsam::PointPlaneFactor> ppf) const;
    ...
    void
    printSelectedGraph(const gtsam::NonlinearFactorGraph& graph,
                       const bool& print_smart_factors            = true,
                       const bool& print_point_plane_factors      = true,
                       const bool& print_plane_priors             = true,
                       const bool& print_point_priors             = true,
  ...
  public:
    ...
    inline Timestamp getTimestampLkf() const
    {
    ...
    inline ImuBias getImuBiasPrevKf() const
    {
        return imu_bias_prev_kf_;
    }
    inline Vector3 getWVelBLkf() const
    {
        return W_Vel_B_lkf_;
    }
    inline Pose3 getWPoseBLkf() const
    {
        return W_Pose_B_lkf_;
    }
    inline gtsam::Matrix getStateCovarianceLkf() const
    {
        return state_covariance_lkf_;
    }
    inline int getCurrKfId() const
    {
        return curr_kf_id_;
    }
    inline gtsam::Values getState() const
    {
        return state_;
    }
    inline int getLandmarkCount() const
    {
        return landmark_count_;
    }
    inline DebugVioInfo getCurrentDebugVioInfo() const
    {
        return debug_info_;
    }
