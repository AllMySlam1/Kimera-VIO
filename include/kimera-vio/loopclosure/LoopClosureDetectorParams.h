...
class LoopClosureDetectorParams : public PipelineParams {
  ...
  public:
    ...
    // NOTE: we cannot parse width, height principe pt and focal length from
    // here. Those are done at initialization of the LoopClosureDetector.
    ...
    bool equals(const PipelineParams& obj) const override
    {
        ...
        return image_width_ == rhs.image_width_ &&
               ...
               max_nrFrames_between_islands_ ==
                   rhs.max_nrFrames_between_islands_ &&
               max_nrFrames_between_queries_ ==
                   rhs.max_nrFrames_between_queries_ &&
               ...
               ransac_inlier_threshold_stereo_ ==
                   rhs.ransac_inlier_threshold_stereo_ &&
               ...
               scale_factor_ == rhs.scale_factor_ && nlevels_ == rhs.nlevels_ &&
               ...
               betweenTranslationPrecision_ ==
                   rhs.betweenTranslationPrecision_ &&
  ...
  public:
    ...
    //////////////////////////////
    bool use_nss_;  // If true, normalize score wrt score achieved across
                    // consecutive queries
    float alpha_;   // Alpha similarity threshold for matches
    int   min_temporal_matches_;  // Min consistent matches to pass temporal
                                  // check; if set to 1, no temporal check
    int recent_frames_window_;  // Number of recent frames we don't consider for
                                // loop closures
    ...
    int   max_intraisland_gap_;  // Max separation between matches of the same
                                 // island
    int max_nrFrames_between_islands_;  // Max separation between groups
    int max_nrFrames_between_queries_;  // Max separation between two queries
                                        // s.t. they count towards
