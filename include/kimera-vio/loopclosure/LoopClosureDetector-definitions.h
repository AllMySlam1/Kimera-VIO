...
struct LCDFrame
{
    ...
    LCDFrame(const Timestamp&                   timestamp,
        ...
        : timestamp_(timestamp), id_(id), id_kf_(id_kf), keypoints_(keypoints),
          keypoints_3d_(keypoints_3d), descriptors_vec_(descriptors_vec),
          descriptors_mat_(descriptors_mat), versors_(versors),
...
{
    MatchIsland()
        : start_id_(0), end_id_(0), island_score_(0), best_id_(0),
          best_score_(0)
    {
    }

    MatchIsland(const FrameId& start, const FrameId& end)
        : start_id_(start), end_id_(end), island_score_(0), best_id_(0),
          best_score_(0)
    {
    }

    MatchIsland(const FrameId& start, const FrameId& end, const double& score)
        : start_id_(start), end_id_(end), island_score_(score), best_id_(0),
          best_score_(0)
    {
    }
...
struct LoopResult
{
    ...
    {
        return status_ == LCDStatus::LOOP_DETECTED;
    }
...
struct LoopClosureFactor
{
    LoopClosureFactor(const FrameId&                 ref_key,
                      ...
                      const gtsam::SharedNoiseModel& noise)
        : ref_key_(ref_key), cur_key_(cur_key), ref_Pose_cur_(ref_Pose_cur),
          noise_(noise)
    {
    }
...
struct LcdInput : public PipelinePayload
{
    ...
    LcdInput(const Timestamp&                     timestamp,
             ...
             const gtsam::Pose3&                  W_Pose_Blkf)
        : PipelinePayload(timestamp), frontend_output_(frontend_output),
          cur_kf_id_(cur_kf_id), W_Pose_Blkf_(W_Pose_Blkf)
    {
...
struct LcdOutput : PipelinePayload
{
    ...
    LcdOutput(bool                               is_loop_closure,
        ...
        : PipelinePayload(timestamp_kf), is_loop_closure_(is_loop_closure),
          timestamp_query_(timestamp_query), timestamp_match_(timestamp_match),
          id_match_(id_match), id_recent_(id_recent),
          relative_pose_(relative_pose), W_Pose_Map_(W_Pose_Map),
          states_(states), nfg_(nfg)
    {
    }

    LcdOutput(const Timestamp& timestamp_kf)
        : PipelinePayload(timestamp_kf), is_loop_closure_(false),
          timestamp_query_(0), timestamp_match_(0), id_match_(0), id_recent_(0),
          relative_pose_(gtsam::Pose3::identity()),
          W_Pose_Map_(gtsam::Pose3::identity()), states_(gtsam::Values()),
