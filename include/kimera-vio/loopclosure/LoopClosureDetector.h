...
class LoopClosureDetector {
  public:
    ...
    /* ------------------------------------------------------------------------
     */
    /** @brief Processed a single input payload and runs it through the
     * pipeline.
    ...
    /** @brief Runs all checks on a frame and determines whether it a
     ...
     information.
     * @param[in] stereo_frame A stereo_frame that has already been "rewritten"
     by
     ...
     * @return True if the frame is declared a loop-closure with a previous
     frame,
     *  false otherwise.
     */
    ...
    /** @brief Verify that the geometry between two frames is close enough to be
        considered a match, and generate a monocular transformation between
     them.
     ...
     * @param[out] camMatch_T_camQuery_mono The pose between the match frame and
     the
     *  query frame, in the coordinates of the match frame.
     * @return True if the verification check passes, false otherwise.
     */
    bool
    geometricVerificationCheck(const FrameId&        query_id,
     ...
     * @param[in] camMatch_T_camQuery_mono The relative pose between the match
     * frame and the query frame, in the coordinates of the match frame.
     * @param[out] bodyMatch_T_bodyQuery_stereo The 3D pose between the match
     * frame and the query frame, in the coordinates of the match frame.
     ...
     * @param[in] query_id The frame ID of the query image in the database.
     * @param[in] match_id The frame ID of the match image in the database.
     * @param[in] camMatch_T_camQuery_stereo The relative pose between the match
     * frame and the query frame, in the coordinates of the match frame.
     ...
     */
    gtsam::Pose3
    refinePoses(const FrameId               query_id,
                const FrameId               match_id,
     ...
     *
     * WARNING: This is a potentially dangerous method to use because it
     * requires a manual deletion of the pointer before it goes out of scope.
     */
    inline const OrbDatabase* getBoWDatabase() const
    {
        return db_BoW_.get();
    }
    ...
    // TODO(marcus): can this be static even though it requires id? Maybe feed
    // it descriptors instead
    ...
    void
    transformCameraPoseToBodyPose(const gtsam::Pose3& camMatch_T_camQuery,
     ...
     */
    void
    transformBodyPoseToCameraPose(const gtsam::Pose3& bodyMatch_T_bodyQuery,
  ...
  private:
     ...
     * @param[in] query_id The frame ID of the query frame in the database.
     * @param[in] match_id The frame ID of the match frame in the database.
     * @param[out] camMatch_T_camQuery_mono The relative pose between the two
     * frames, with translation up to a scale factor.
     * @return True if the verification passes, false otherwise.
     */
    bool
    geometricVerificationNister(const FrameId&        query_id,
                                const FrameId&        match_id,
                                gtsam::Pose3*         camMatch_T_camQuery_mono,
                                std::vector<FrameId>* inlier_id_in_query_frame,
                                std::vector<FrameId>* inlier_id_in_match_frame);
     ...
     * @param[in] query_id The frame ID of the query frame in the database.
     * @param[in] match_id The frame ID of the match frame in the database.
     * @param[out] bodyMatch_T_bodyQuery The relative pose between the two
     * frames.
     * @return True if the verification passes, false otherwise.
     */
...
class LcdFactory {
  public:
    ...
    static LoopClosureDetector::UniquePtr
    createLcd(const LoopClosureDetectorType&   lcd_type,
        ...
        switch (lcd_type) {
            case LoopClosureDetectorType::BoW: {
                return VIO::make_unique<LoopClosureDetector>(
                    lcd_params, stereo_camera, stereo_matching_params,
                    log_output);
            }
            default: {
                LOG(FATAL)
                    << "Requested loop closure detector type is not "
                       "supported.\n"
...
class LcdModule : public MIMOPipelineModule<LcdInput, LcdOutput> {
  public:
    ...
    LcdModule(bool parallel_run, LoopClosureDetector::UniquePtr lcd)
        : MIMOPipelineModule<LcdInput, LcdOutput>("Lcd", parallel_run),
          ...
          backend_queue_("lcd_backend_queue"), lcd_(std::move(lcd))
    {
    }
  ...
  protected:
    ...
    {
        // TODO(X): this is the same or very similar to the Mesher
        // getInputPacket.
        ...
        return VIO::make_unique<LcdInput>(timestamp, frontend_payload,
    ...
    //! Checks if the module has work to do (should check input queues are
    //! empty)
    bool hasWork() const override
    {
        // We don't check Frontend queue because it runs faster than Backend
        // queue.
